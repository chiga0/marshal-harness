...
- **Mac-first 实证与 rework 最小化**：为避免 Qoder/Codex 类适配问题在每个 Run 中重复消耗 token，Lead 在每个 heartbeat 依次执行以下 admission 清单：
  ...
  9. **零 Attempt admission 与待办去重**：`task run` 前必须先完成零 Attempt 预检：`task status` 处于 `READY`、plan approval 与当前 `specDigest/policyDigest/capabilityDigest` 匹配、`doctor` 显示 Adapter 已配置且 supported、Mac 普通用户模式显式声明、scope/worktree 不冲突、容量与 Provider 背压检查通过。任一项失败都不得启动 Worker、不得消耗 Attempt/retry 预算；只记录 admission finding 并修复输入或 plan。
     - **Qoder v5 transcript attestation（独立 reviewer 前强制）**：真实 Attempt 进程终态后、派发任何独立 reviewer 前，从 `templates/transcript-attestation-preflight.json` 生成 operator-local manifest；把当前 Attempt 的 `control/output/qoder-transcript.jsonl`、`control/output/qoder-transcript-meta.json`、`worker-request.json`、权威 `worker-result.json`、Run 根 `task-spec.json`、`capability-snapshot.json` 与当前 source tree 的冻结 `references/transcript-attestation-profile.json` 原始字节复制到 source worktree 和 `.marshal` 之外的紧凑临时目录，逐项填写 raw `sha256`/`maxBytes` 与 Attempt/Adapter/协议/permission identity。allow/deny tool、required constraint、forbidden command 与 command digest binding 只从已验证的 TaskSpec/profile 机械导出，manifest 不得维护人工副本。禁止读取或提交 prompt，禁止把 raw transcript、WorkerResult 自由文本或绝对用户路径带入仓库。先执行 `go build -o "$OPERATOR_DIR/transcript-attestation-checker" .agents/skills/marshal/references/tests/transcript_attestation_core_probe.go`，再执行 `python3 -I -B .agents/skills/marshal/references/validate-transcript-attestation-preflight.py --root "$ATTESTATION_DIR" --manifest transcript-attestation-preflight.json --checker "$OPERATOR_DIR/transcript-attestation-checker"`；仅退出码 0 且 `status=pass`、`reasonCode=transcript-attestation-pass`、subject 与当前 Attempt 完全相符时，才能继续 freshness preflight 和 reviewer 派发。Validator 必须让 checker 在 stdin 未发送时保持阻塞；Mac 预期身份直接从复制前持有的 raw bytes 解析 thin 64-bit Mach-O 唯一 SHA-256 CodeDirectory，实际身份再由固定系统 `codesign -dvvv +PID` 取得 full SHA-256 CDHash 并精确比较，Linux CI 使用 `/proc/PID/exe` raw SHA-256；只有匹配才可发送 evidence，pathname/inode 复核只是防御纵深，不能描述成实际执行身份。Checker 与 `codesign` 的 stdout/stderr 都必须增量读取并分别及合计受硬上限约束；overflow/deadline 固定 fail closed，只终止、等待并回收 validator 自己创建的对应 `Popen`。只允许保存 validator 的脱敏 identity/digest/observation/reasonCode 摘要；摘要必须绑定 validator/schema/checker/profile、checker expected/actual process identity method/digest、Qoder event contract、result transport、Capability admission evidence 与 executable digest。
     - transcript attestation 是 **pre-review/operator-local** 机械门禁，只核对 Adapter 保存的实际 tool/command、`declaredCommands`、TaskSpec 与 final tee 纪律；它不是 Worker 自证，不替代 Core 对生命周期、持久化、failure/retry、ReviewPacket、ReviewDecision 或 freshness 的权威，也不得写入 `.marshal` 或直接改变 Run 状态。任一非零退出或 `status=fail` 都原样记录固定 `reasonCode` 并阻断 reviewer：`input-*`、`path-boundary-invalid`、`invalid-json`、`duplicate-json-key`、`manifest-schema-invalid`、`schema-document-invalid`、`policy-invalid` 先修 operator input/validator，不消耗 Worker Attempt；`source-head-mismatch`、`subject-mismatch`、`task-constraint-mismatch`、`task-tool-policy-mismatch`、`transcript-meta-mismatch` 重新取得当前 Attempt 权威输入，确认真实漂移后修 TaskSpec/Adapter 并建 fresh-base successor；其余 tool/command/transcript/tee/terminal/transport/WorkerResult 失败均按结构性 `protocol-invalid/do-not-retry` 处理，停止该 Adapter 晋升与新派发，按稳定 failure signature 只裁决一次，先修 Adapter/契约再建 successor，禁止原 Run retry 或 Worker rework。相同输入摘要和 `reasonCode` 不得重复运行或重复派 reviewer。
  ...
  10. **Adapter 上线阶梯与失败预算**：新 Adapter 或版本升级必须按 `registry/doctor → fake conformance → 真实只读 live probe → 单个低风险写任务 → 常规调度` 逐级晋升；任何一级失败都停在该级修 adapter/契约，不得直接把真实产品任务当探针。每一级只保留一份按 executable digest、平台、authority mode 与协议摘要索引的证据，后续 Run 命中相同身份时复用，避免重复烧 token。
      - Qoder v5 的真实只读 live probe、首个低风险写任务与独立 conformance 都必须分别通过上面的 transcript attestation，且 subject/input/attestation digest 纳入该级脱敏摘要；缺失、失败或身份不匹配时不得晋升、复用证据或进入常规调度。
