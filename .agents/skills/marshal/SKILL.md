...
- 新建 TaskSpec 未收到用户的显式 Worker 顺序时，生产派发者必须把默认顺序冻结为 `preferredAdapter: qoder`、`fallbackAdapters: [codex, qwen, pi]`；收到显式 `preferredAdapter`/`fallbackAdapters` 时逐字保持其顺序。不得把 OpenCode 加入新 Task 的候选列表。Planning 只消费冻结后的显式顺序，不存在隐式默认或运行时重排。
