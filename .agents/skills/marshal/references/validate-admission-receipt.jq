...
.format == "marshal-skill/operator-admission-receipt-v3" and
...
([.bindings.specDigest, .bindings.policyDigest, .bindings.capabilityDigest,
  .bindings.runStateDigest, .bindings.planApprovalDigest] | all(.[]; sha256)) and
...
(.adapter.mode == "host-user" or .adapter.mode == "ordinary-user") and
...
(.checks | type == "object" and
  keys == ["capacityAvailable", "currentPlanApproved", "doctorConfigured",
           "doctorSupported", "providerBackpressureAbsent", "stateReady",
           "worktreeClean"] and
  all(.[]; . == true)) and
.decision == "observe" and
