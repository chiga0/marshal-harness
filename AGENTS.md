...
- **ADR 与架构演进**：ADR 记录的是当时的决策上下文，不冻结后续优化与重构。旧 ADR 只有 `Superseded` 链上的位置；改变其约束的合法路径是新增 ADR 并标注 `Superseded by`。任何人不得引用历史 ADR 拒绝合法的新方案。
- **工程工具链豁免（2026-09-15 维护者明确）**：不改变信任边界、持久化契约、生命周期、发布权限、HTTP 合同与业务语义的纯工程变更——构建、类型/lint、格式化、测试基建与门禁、CI 配置、机械性批量迁移与重构——无需 ADR，决策依据留档于 PR/commit 即可。
- 修改合同 `packages/task-api/openapi.json` 后必须验证 JSON 语法、示例和 `git diff --check`。
- 不得为了简化 Adapter 而静默放宽强制门禁。

## 实施门禁（universal）

### UI 三条验收（2026-09-11 用户明确要求）

所有涉及 UI 的变更（含影响界面的 API、状态、文案和配置行为）必须按 [UI 验收规范](docs/ui-1/acceptance.md#三条独立验收线) 分别审查：功能与可靠性、视觉与交互、产品可用性，三者不可互相替代。PR/交付记录逐条列出范围、候选版本、证据、结论与缺口；未测不能写通过，代码合并不等于产品体验验收完成。

- 功能与可靠性：相关自动化测试加真实服务操作；fixture 证据与真实 Agent/业务场景分开。
- 视觉与交互：真实浏览器截图、点击、键盘、缩放及动态刷新检查；不能只看代码或空态。
- 产品可用性：由未参与该流程开发的目标用户或代表完成指定任务，尽量不指导；代码 reviewer 或 Agent 模拟不能替代真实用户测试。
- 每次变更按影响范围验收，不为纯文档或未受影响页面机械重跑全量；不适用需说明理由并由独立 reviewer 确认。证据缺失标记待验及具体补验任务，不以不适用隐藏缺口。完整 UI 能力/版本验收要求三线必需范围均通过。
- 缺陷集中记录、修复、复测；重复问题前移到测试、组件或设计规范。沿用现有 PR/验收文档，不恢复旧 Marshal skill，不新建审批平台或微切片流程。
...
- **ADR 触发条件**：改变信任边界、持久化契约、生命周期或发布权限时，必须先新增或替代 ADR。不改变上述边界与业务语义的纯工程变更（构建、类型/lint、格式化、测试基建、CI、机械性迁移重构）无需 ADR；ADR 不冻结后续演进，替代一律走 `Superseded by` 链。
