...
- verifier 不得根据 request 或预期常量自行产生三项通过 verdict。隔离 sandbox 必须以固定 policy 锚定的独立 Ed25519 receipt authority 对每轮实际执行签发 canonical、closed `qoder-probe-execution-receipt-v1`，逐项绑定 held executable fd 的 device/inode/digest、实际 argv、完整替换 environment、held scratch/credential/write/deny root identities、执行时 topology digest、variant challenge、transcript/session/model/version/protocol/permission、marker digest 与执行时间。verifier 只持 receipt public key，逐项与持续持有的 invocation handles 和实际 transcript/marker 对账；`hermetic-fixture` receipt、调用者自带 key、未知/缺失字段、签名错误、任一替换或跨 variant replay 都不能生成可签名的 live observation。
...
候选 verifier 不提供普通宿主进程 fallback：它必须由隔离 sandbox 实现运行四种真实 argv 组合，sandbox 的唯一写域是 verifier-owned scratch，并显式拒绝全部业务仓库 root；环境为完整替换且 observation 没有 credential、stderr、transcript 正文或 private key 字段。`1.1.23` fake fixture 仅约束解析、capability/profile/argv 和 signer fail-closed 机制，不得标记为 credentialed live probe。
