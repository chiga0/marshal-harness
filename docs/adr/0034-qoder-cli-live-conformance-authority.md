...
- authority config 精确绑定一个 current evidence digest、probe artifact digest 与递增 authority generation，并可列出 revoked evidence digests。相同 key 的新 generation 不改写旧 evidence。

### 3. 文件与路径边界

`MARSHAL_QODER_CONFORMANCE_CONFIG`、authority root 与 evidence leaf 必须是绝对 clean path。consumer 从 `/` 开始逐段使用 dirfd + `openat(O_NOFOLLOW)`，不跟随任何父级或 leaf symlink；config/evidence 必须是当前 uid 或 root 拥有的私有 regular file，authority root 必须是当前 uid 或 root 拥有且权限 `0700` 的真实目录。JSON 拒绝未知字段并执行严格字节上限；digest 命名 leaf 只通过已打开的 authority root dirfd 解析。

### 4. 撤销、恢复与可观察性

生产 Adapter 不把一次成功变成永久内存授权。每次 `Probe` 和每个 Worker launch guard 都重新安全读取 current config、trust roots、generation、revocation set 与 content-addressed evidence，并重新验证签名、freshness、profile、host 和 executable identity。删除、撤销、权限变化、key rotation、generation/identity 漂移立即使当前操作 fail closed；长寿命 `marshal-server` 无重启豁免。
...
负向 fixture 至少覆盖：缺失配置/evidence、未知字段、过期/超长 TTL、错误 executable realpath/digest/version、同 OS/arch 不同 host、错误 suite/artifact/challenge/profile/argv/env/tool policy/event/protocol/permission、任一 verdict 为 false、未知/轮换 key、错误 generation、已撤销 digest、config/root/evidence symlink（含父级）、错 owner/权限、运行中撤销后 Probe 与 launch guard 拒绝。
