...
**证据门禁式 Coding Agent 编排器。** 主 Agent 负责规划与审查，可替换的 Worker Agent 负责实现，确定性的 Harness 负责验证、留痕和发布。
...
> 状态：Milestone 0–6 全部通过，Local MVP `USABLE`。详见 [路线图状态](roadmap-status.md)。
...
3. [ADR 索引](adr/README.md)（0001–0014）；
4. [验收报告](roadmap-status.md)（Milestone 0–6）；
5. [研究与审计](research/herdr-comparison.md)（herdr 对照、fan-out 决策、A/B 设计）。

## 定位边界

Marshal 不让 Agent 更聪明，而是让 Agent 的工作**可验证、可审计、可安全委派**。它不是终端运行时（见 [herdr 对照](research/herdr-comparison.md)），不是恶意代码沙箱，不提供自动 merge。适用与不适用场景见 README。
