# Runtime 架构历史参考（2026-09-07 归档）

> 本文是旧 file-backed/多账本 Runtime 设计与长期扩展的追溯记录，不是当前服务方案的总规范。下文“当前 B1/B2 不迁库”、固定 proof/AST/锁序和 M7–M13 顺序仅描述旧方案；当前目标见[Runtime 架构](runtime-architecture.md)及[合同适用性](design-contract-map.md)。历史字节和旧 profile 的安全/恢复合同不因归档失效。
