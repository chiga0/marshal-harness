...
- 未关闭 Blocking Finding：1 项 P1（状态 `OPEN`，关联 [Issue #25](https://github.com/chiga0/marshal-harness/issues/25)/[PR #24](https://github.com/chiga0/marshal-harness/pull/24)，见下文 Issue #25 审计增补节的发布合并后 head reconcile finding；S-A1–S-A7、Round 2 六项歧义、Round 3 C-A1–C-A6、Round 4–9 独立评审与复核各轮 P1/残留的关闭记录见下文增补节）
- 门禁状态（分层）：
...
| ID | 级别 | 状态 | 关联 | 问题 | 定位 |
| --- | --- | --- | --- | --- | --- |
| `PUBLICATION-MERGED-HEAD-RECONCILE-P1` | P1 | `OPEN` | [Issue #25](https://github.com/chiga0/marshal-harness/issues/25) / [PR #24](https://github.com/chiga0/marshal-harness/pull/24) | 已合并 PR 的 head 与 merge commit 缺乏权威 reconcile 路径，`accept` 无法识别 merge 完成态，Run 被永久置为 `BLOCKED` | 分两层：**nonterminal implementation bug**——`accept` 以 PR OPEN/Draft 为前置条件，未区分“merge 前需 OPEN”与“merge 后已合并”两种语义，是可修复的实现缺陷；**terminal contract gap**——当前 Schema/命令缺少不可变 `SCMMergeReceipt` 与 append-only `PublicationReconcileRecord`，无法把已合并 Run 从 `BLOCKED` 安全迁移到 `ACCEPTED`，属契约级缺口，需新 ADR 定义 |

临时 operational 护栏见 [Operator Runbook §7](operator-runbook.md)。当前没有安全的 legacy merged-PR reconcile 命令；`doctor --repair` 只修复权威 Journal 能唯一证明的 snapshot 损坏，不能把 `BLOCKED` 改成 `ACCEPTED`。Run 的恢复必须等待 [ADR 0026](adr/0026-scm-merge-receipt-and-publication-reconcile.md) 定义的不可变 `SCMMergeReceipt` 与 append-only `PublicationReconcileRecord` 及其实现。处置进展（2026-08-12）：ADR 0026 已接受并合入 main（PR #49），冻结 `SCMMergeReceipt` 与 `PublicationReconcileRecord` 契约，契约层关闭；实现层（采集 SCMMergeReceipt、append-only reconcile 命令与 `BLOCKED`→`ACCEPTED` 迁移）仍待后续任务。本 finding 状态保持 `OPEN` 至实现合入，级别与状态不变，不改变 Local MVP `APPROVED_FOR_IMPLEMENTATION` / `USABLE` 结论，也不改变任何已冻结的信任边界、持久化契约、生命周期或发布权限；它是契约级 reconcile 缺口的审计记录，实现状态由后续实现任务与对应 Milestone 退出门禁关闭。
