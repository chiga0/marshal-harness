...
提案保留的硬门禁是current-ledger recheck、exact successful resume、ADR0065 sealed proof、唯一Run successor、fixed Supervisor mechanics、ADR0064 control-directory identity、ADR0066 canonical factory。S1′只允许runstore内部窄lease shared-guard/borrow、descriptor-bound strict journal与read-only projection，唯一exported mutation seam仍为`WithPreparedRunStartAuthority`；S2′必须立即用现有authority API产生真实`attempt-opened→owner/current Attempt binding→allocation provision intent/receipt→launch-authorized/StoredClosure→PreparedExecution`，不允许seed/Fake/legacy`execution.Run`。Mac ordinary-user的ambiguous窗口不转silent fallback；只有pre-intervention exact no-effect证明可走封闭abort/cleanup链，其余永久intervention。ADR0067仍为Proposed，接受前不改变现行ADR状态；实现不得以提案名义提前绕过0059/0060/0063。
