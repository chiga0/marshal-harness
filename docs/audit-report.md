...
| `AGENT-AUTHORITY-PEER-EXEC-SWAPBACK` | P1 | `OPEN`（ADR 0039 Proposed） | held `/proc/<pid>/exe`、pidfd 与收包前后 pathname sampling 不能排除 `exec-away → send → exec-back`。关闭要求先接受 ADR 0039，再实现 root trusted launcher、`no_new_privs`、不可放宽 seccomp exec deny、一次性 signed launch receipt 与双向 transport bootstrap；真实 Linux 负测必须证明 application handler 调用数为零且无 response。 |
