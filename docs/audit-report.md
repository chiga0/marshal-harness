# 产品目标与标准契约文档收敛（2026-09-14）

本轮按用户明确要求分离产品定义、目标设计、标准契约和实现进展。README/首页不再承载Milestone；目标架构、生命周期、安全和Leader职责给出完整叙述，历史原文归档；标准API连接客户端与执行、业务验收、交付和存储边界，现有OpenAPI保持唯一HTTP机器真值。目标扩展义务不自动开放远端执行、外部ETL或新权限，不改变Node状态/持久格式。

公开站点按产品、使用、架构、API/扩展、路线组织；OpenAPI按原字节发布并生成HTTP参考，构建检查站内目标和片段链接。修正的是设计表达和发布一致性，不将原产品待验、真实ETL或UI真人验收标为通过。完整候选的独立审查与构建证据见本轮PR；旧记录保留原结论与候选。
...
## 2026-09-14：示例业务侵入默认产品接入（OPEN，修复中）

用户已安装发行包，却因 DataWorks Qwen 的 `bin/qwen` 入口不是 `cli-entry.js` 被拒绝。进一步核对发现通用团队只有布局函数，`serve` 必须提供业务 JavaScript，主要可运行配置固定地区/输入/输出，无法直接承接原设计中的普通自然语言任务。这是部署装配与通用出口缺失，不是用户应重装 Qwen 或反复补提示词的问题。

修正按 ADR0100：协议适配接受真实可执行入口，不绑定 npm 目录；内置动态文件团队+独立 Review+受管字节验收；init/serve/升级保留旧状态并给出真实连接。验收必须用同一配置完成不同意图/不同 DAG，并消费真实下载，不能只数组件测试。未知工具作用域、原生 ambient 权限和冷崩溃能力继续明示；真实 ETL 外部操作须另有明确授权/回执/查证闭环，不用文件报告伪造平台成功。
...
以下为历史审计检查点，保留失败原因、成本和当时结论；其中 intake、第二需求失败和恢复 P1 的待办口径已由上文取代，不据历史记录重新打开已关闭问题。
...
第二次真实 Pi 的原输出已由 `c52f9c89` 私有诊断保留，确认字符串 `ask.options` 与解析器要求的 `{value,label}` 不符；此前提示只给空数组，没有传达非空元素合同。第一次输出未保留，其原因仍未知。此次不是登录失败，不应重新配置账号或盲目重跑模型；修复方向是向模型完整传达已有合同，并对捕获返回做确定性反例回放，同时聚合核对其余嵌套字段。保持 Core 原闭集、摘要、预算和授权检查，不用静默格式降级掩盖接入问题。提示测试通过只能证明覆盖的例子，不能代替真实模型整链。

部署审计另确认：生产 CLI 与 v7 Core 已接通，但完整 Leader 配置仍主要存在于 fixture，随包旧配置不是 v7。后继补非 fixture 部署示例并经原包 CLI→HTTP→冷开验证，不新增配置平台，也不将驱动成功当产品一键启动。以下记录保留各自时点。
...
根因是正向阶段实现覆盖了顺序调用，却未完整覆盖并发事实变化、硬停止与已发生效果的正交关系，以及失败分类在接缝上的保留。纠偏为同一作者一次聚合修正、同一 reviewer 复核，并保留原时序反例；不拆成四个新治理任务、不启动付费模型重试来探索确定性缺陷。整合源 `da2e9bd2` 原 HTTP 正向 2/2 通过不能冲销这些负例。后继原 HTTP 取消通过；真实进程的受控缺陷修复测试在第二次 Review 接纳后原 verify 失败，尚待区分夹具假设与产品缺陷，未宣称完整局部修正通过。详见 [Roadmap 当前证据](roadmap-status.md#业务交付当前表)。

后继已定位 repair 失败为产品准备接缝：独立 Review 的 finding 正确只指 east，verify 是已批准修正闭包的后继，却被当成也必须直接命中 finding 的作者拒绝；原 checker 未启动，不能更改业务 oracle 来变绿。另从真实安装消费发现 driver 纯函数测试未覆盖生产组合：复制 FileBusiness 丢失原私有身份、默认容量与Task容量矛盾。后继 `8eec88e5` 保留原对象/预算并通过独立14/14，将真实create/open、准备/权限/采集前置为无模型检查。安装消费者 `1657f6cc` 的独立同包两Task4/4证明正常闭环，不替代上述故障修正；不再仅凭helper测试通过就进入付费模型。外部受控ACP在end_turn后由原Runtime停止，不能把SIGTERM清理事实篡改成exit0；命令检查者与发布执行仍检验exit0。
...
## 2026-09-09：以业务交付校正 Leader 设计与实施节奏（文档修订）

基于 `3f359fd0fd88987b8ea2f3d3fbb36058834b003b` 的架构、Leader 机制、Milestone 与既有失败记录，结论是保留现有底座，补齐业务正确性和持续集成约定，而非重建编排平台。采用：Leader 对业务目标负责，Agent 发挥专业能力，Core 保证执行边界与事实可靠，最终交付接受独立且贴近真实需求的检查。Supervisor 只观察、聚合与通知；硬规则仍由 Core 决定执行，不等待模型判断。

本轮落实四项纠偏：一是原始需求、确认的交付约定、可变实现计划分开，需求对应工作包、成果与独立证据，不能将测试绿等同于业务要求全部满足；二是可信验证机制与任务验收内容分开，在同一声明业务族内用不改 Core/部署配置的不同任务检验通用性；三是明确 Leader 原授权内自主调整、重大目标/标准/预算/副作用变更需确认的边界，不把非阻塞优化变成返工理由；四是 B2-L 每一可运行检查点就接入原 HTTP/Application/SQLite，不等末尾一次大集成。既有上下文遗漏与验收盲区记录保留，用工作包完整性和需求覆盖反例前移检查。

正常任务可以首次通过；另用明确标注的真实缺陷修复案例证明实际独立拒收→局部修正→重新验收，用确定性注入证明故障接缝。三类证据分开，不伪造拒收、不污染候选，也不反复付费等模型自然失败。修正链未真实发生则该项保持未关闭。Milestone 历史日志[原样归档](agent-team-service-milestones-history-2026-09-09.md)，当前页面保留出口与实施顺序。

细化见 [ADR0094 §7](adr/0094-trusted-single-user-role-team.md#7-2026-09-09-业务正确性与效率细化)和[Leader 执行机制](node-leader-execution-design.md)。本轮仅文档变更，不修改 API、Schema、Store、运行权限或历史证据；B2-L 仍 DESIGN，原 API-STABLE 保留原范围，新 Leader 语义须随正式候选重新核验。文档作者自检不等于独立设计审查；没有新增实现、实机通过或生产可用声明。

集成时保留主线 `5ac6d33d` 的 ADR0095、机器合同及其原独立审查记录；本轮只补业务覆盖与效率约定，不改其闭集字段、v7 权限或机器语义。本次文档差量按用户明确批准免独立 review，由作者完成链接、归档及差量自检后合入；该一次性例外不作为独立验证证据，不扩展到产品或后续研发。
...
效率约束明确：原 DAG 后继自动调度、批次通知合并；精确发布授权回复只继续原已提交动作，不再花一次模型调用重复批准。17 Attempts/9 calls 是代表验收场景可用上限，不是固定消费，更不制造返工凑证据。B2-L 仍 DESIGN，合同接纳不等于运行时完成；后续三个互斥实施范围为 Core、API/客户端、本机报告端口。
...
Runtime修正 `285a3516` 在原清理预算内观察Darwin瞬时EPERM，只有后续原真实ESRCH才确认，持续拒绝/未知后代仍不通过；独立18/18通过34.668秒，无P0/P1，不改变ADR0089权威边界。精确ce55eed的macOS CI两项失败仍保留，实机已确认该OS机制但没有原CI errno，不冒充历史根因已确诊。完整[来源、复现、失败成本与剩余边界](node-task-service-status-2026-09-08.md)独立记录，B1/B2/B3不升级。

## 2026-09-09：同计划局部修正合同收敛，避免失败后重跑整队

[ADR 0091](adr/0091-node-same-plan-local-repair.md) 经维护者独立审阅候选 `5c2d82e` 后接受。它补当前 B2 的实际缺口：只有父验证器产生的、带原始负面报告的业务内容拒收，才允许用户在原计划、预算和期限内显式修正受影响依赖闭包；保留无关作者的精确成果，并重新执行完整独立验收。旧失败不改写，结构性失败不允许借此盲目重试，不增加第二控制器、重规划或发布权限。

实现重点是统一所有成果消费者使用明确选择，而非从历史 Worker 猜选“最新成功”；问答仍绑定原执行，旧答案不重投递。新持久语义独立声明 v4-repair，旧根不隐式升级。该合同只是后继实施依据，当前尚无局部修正运行证据；运行中问答、故障验证及正式权限/发行出口各自保持真实状态。

## 2026-09-09：真实混合 Git 业务通过，权限证据继续独立判断

`76f9c36` 一次实际 Pi0.84.4＋Qwen0.22.3 双作者业务通过：两个原仓库不变，实际 patch 在独立验收和第三消费者工作区应用，正常重开保留回执/成果、无重复派发；已合并推送 `7294878`。这是固定完整业务方案的实际交付，不是模型握手或纯夹具。独立组合48/48通过，实机56.914秒、作者重叠11.631秒；[原始摘要](node-task-service-status-2026-09-08.md)保留精确版本。

不能将成功交付等同权限隔离：本次 Pi 原生授权3允许/1拒绝，Qwen没有权限回调。后者不提供越界拒绝或 Publisher 凭据隔离证据；不按品牌、环境变量清理或 publication:none 宣称分权。最短后继是在受限执行身份下运行整条Node服务与Worker链，Publisher保留另一个明确权限域，使用无秘密 sentinel 与明确入口验证；不先增加跨UID控制平台。该身份自己的模型登录仍需合法配置，不复制维护者凭据。

问答 API 完成即续派原问答故障链，Core/Provider生产作者与独立测试作者分开；Git驱动完成即续派局部修正的最小完整方案，不占槽等CI、不恢复旧skill。提前对齐 public options 投影和原409错误码，避免冻结后跨层返工。
...
## 2026-09-08：正式发布目标与 Qwen ACP 接入纠偏

用户明确要求完整 stable 正式发行，不能让 Node 启动可行性实验替代产品。当前真实双 Qwen 已启动并收集，但独立业务验收失败；确定性组合测试 46/46 不填补该失败，也不授予 B2/B3。保留一次真实失败，不原样付费重试。Qwen 原生 ACP 已在本机帮助/实现中确认，正式接入应使用能力协商、原生工具与配置及真实进度/交互/取消，不继续将固定无工具 JSON 样例作为主线。正式 Node 存储/生命周期的取代合同与完整恢复仍开放；不是把实验更名为 production。事实与下一步边界见[接入记录](qwen-integration-status-2026-09-08.md)。
