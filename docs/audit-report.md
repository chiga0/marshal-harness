...
| `MARSHAL-BUILD-INPUT-ATTESTATION` | P1 | `CONTRACT-ACCEPTED/AMENDMENT-PROPOSED/IMPLEMENTATION-OPEN` | Issue #212 第四个候选的独立审查证明 `HEAD/status/HEAD` 只观察 Git 元数据，不能绑定 Go 实际读取的 ignored `.go`/embed，且没有关闭 provenance 后 mutation 与 linked worktree `.git`/`gitdir`/`commondir` ABA；build 后再次 `git status` 也不充分。ADR 0048 原合同在 sourceHead `5de09997f5260c672f297496290b567815162bb1` 经独立 reviewer 复审 `ACCEPT`（P0/P1=0）并随 PR #215 合入后由维护者接受；它冻结 protected builder、sealed source/compile/external-material manifest 与 signer 分权。后续审计发现 compile-root object、authenticated build-record carrier、shared code-sign identity/observation 与跨对象相等关系仍未冻结，现以 amendment Proposed 补充，原接受证据不追溯覆盖这些新语义。关闭仍需要 amendment 接受、production producer hostile matrix、schema/validator、protected build→sign→install 最短纵切、外部 provision 与独立 reviewer P0/P1=0。Accepted/Proposed 文档、纯函数测试或 40-hex `sourceHead` 任一单项均不构成关闭，也不表示 R3-D/E/F 完成。 |
