...
## 2026-09-14：Leader 模型运输身份复制失败（OPEN）

通用文件实机出现动作形状合法但 `callId` 不匹配，原端口拒绝结果；不删除失败，不把重新拼接旧结果当作恢复。新增 [ADR0101 草案](adr/0101-generic-leader-model-wire-binding.md) 将模型建议与受信运输封套分开，仅拟用于显式新配置：保留 Core 动作／证据／当前性门禁，旧 profile 错误绑定继续拒绝。当前仅设计，不修改默认、权限、数据库或 HTTP，不宣称模型与产品验收通过。先独立审查，再做原 SQLite 正负例与有限真实验证；原始输出不可用时不能把映射决定冒称逐字模型证据。
