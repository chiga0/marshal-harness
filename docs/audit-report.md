...
| `MARSHAL-BUILD-INPUT-ATTESTATION` | P1 | `OPEN-PROPOSED-ADR` | Issue #212 第四个候选的独立审查证明 `HEAD/status/HEAD` 只观察 Git 元数据，不能绑定 Go 实际读取的 ignored `.go`/embed，且没有关闭 provenance 后 mutation 与 linked worktree `.git`/`gitdir`/`commondir` ABA；build 后再次 `git status` 也不充分。ADR 0048 提议以 protected builder、sealed source/compile manifest 和独立 `MarshalArtifactBuildAttestationV1` signer 补齐 ADR 0047 的 artifact input authority。关闭需要 ADR 接受、production producer hostile matrix、schema/validator、protected build→sign→install 最短纵切与独立 reviewer P0/P1=0；Proposed 文档、纯函数测试或 40-hex `sourceHead` 均不构成关闭。 |
