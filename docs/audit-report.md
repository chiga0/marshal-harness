...
临时 operational 护栏见 [Operator Runbook §7](operator-runbook.md)。当前没有安全的 legacy merged-PR reconcile 命令；`doctor --repair` 只修复权威 Journal 能唯一证明的 snapshot 损坏，不能把 `BLOCKED` 改成 `ACCEPTED`。Run 的恢复必须等待规划中的 ADR 0026 定义不可变 `SCMMergeReceipt` 与 append-only `PublicationReconcileRecord`。本 finding 状态 `OPEN`，不改变 Local MVP `APPROVED_FOR_IMPLEMENTATION` / `USABLE` 结论，也不改变任何已冻结的信任边界、持久化契约、生命周期或发布权限；它是契约级 reconcile 缺口的审计记录，实现状态由后续 ADR 与对应 Milestone 退出门禁关闭。
