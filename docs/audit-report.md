...
| `AGENT-AUTHORITY-PEER-EXEC-SWAPBACK` | P1 | `OPEN`（ADR 0039 Proposed） | held `/proc/<pid>/exe`、pidfd 与收包前后 pathname sampling 不能排除 `exec-away → send → exec-back`。关闭要求先接受 ADR 0039，再实现 root trusted launcher、USER_NOTIF 只放行一次经 `pidfd_getfd` 验证的初始 held-FD `execveat`、随后永久 exec deny、独立 launch attestation/逐连接 nonce receipt、不可转移 client/server helper 与双向 bootstrap；真实 Linux 负测必须证明 application handler 调用数为零且无 response。 |
