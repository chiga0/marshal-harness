...
## 2026-09-10：从安装成功到默认团队之间的接入缺口

用户在 v1.0.2 安装成功后运行 `marshal status` 得到 `connection_unavailable`。根因不是再次缺少环境变量：安装只做发现与 init，首次 serve 仍要求部署者写业务配置；已有区域报告装配并非通用业务。此前把“发现路径”和“可执行团队”分批交付，却没有证明用户最短链，造成安装成功后继续阻塞。

按用户明确确认的[ADR0099](adr/0099-generic-file-team-default.md)，默认能力收敛为文件交付：独立语义 Review 加固定文件完整性核验，产品代码负责装配、启动与连接；不向 Skill 转移部署实现，不宣称 SQL 发布或补数已完成。默认根与旧业务根分开，显式配置优先。该改进正在同一纵切完成实现、定向负例和两需求真实消费，未发布前 v1.0.2 原行为不变。

后续验收以“安装后的 init→serve→Task→独立检查→下载→重启”作为单位，不再仅以路径发现或模块测试通过称接入完成。模型语义意见、确定性文件证据与外部效果证据分别标注，不互相替代。
