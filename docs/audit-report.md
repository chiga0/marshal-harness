...
| `V1-CORE-RESTART-PROCESS-MECHANICS` | P0 | `OPEN-DESIGN` | RB2/B2 预审证明直接 `PT_TRACE_ME` 启动者的 wait right、tracer、held FD 与 pipe 无法转移给重启 Core；PID/路径重验不能重建 mechanics。ADR 0059 提议固定 `marshal internal process-supervisor` 持有 per-Attempt mechanics、Core 只经 current authority 命令重连。关闭条件是 ADR 接受、真实接线、两次 Core restart/lost-response 通过；Supervisor crash 保持 intervention。 |
