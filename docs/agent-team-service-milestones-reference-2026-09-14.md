> 历史快照：基线 `12db5f90`，2026-09-14 文档收敛前原文。当前出口见 [Milestone](agent-team-service-milestones.md)，完成事实见 [Roadmap](roadmap-status.md)。
...
2026-09-14 用户接入纠偏按 [ADR0100](adr/0100-generic-team-default-and-agent-entry.md) 前移：先验收安装→默认通用服务→已安装 Qwen 可执行入口→同配置不同任务的完整文件交付；不得用专用地区汇总工厂关闭通用入口。下一条独立出口是 DataAgent 工具的真实授权外部操作、查询回执和生产结果验收，文件成果通过不能关闭该出口。新发行必须包含实际 runtime 闭包及启动升级脚本，保留旧安装/配置/数据库，不让用户手写业务 JavaScript 才能创建普通任务。
...
**2026-09-10 实施状态**：B2-L 四个实机证据缺口（安装 Pi 整轮、真实局部修正、声明面故障恢复、Linux 实机+长期运行样本+same-bytes 候选材料）已按主线 `fc2cdc92` 闭合，全部细节与收窄口径见 [Roadmap 当前表](roadmap-status.md#业务交付当前表)。B3 仍 `IN_PROGRESS`：正式受保护发行与签名/notarized 未执行，不把上述证据视为 B3 通过。
