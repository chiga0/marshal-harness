...
显式配置的 [Leader 格式纠错](adr/0104-bounded-leader-protocol-correction.md)只适用于有可信语法分类、已确认清理且仍具当前性和原预算的调用。原 Worker 保持失败，整个 Task 最多保留一个格式后继；调用、Attempt 和期限均按原账本继续计算。`LeaderView.protocolCorrection` 给出原失败与后继关联，后继尚未保留、执行中或已结束均以实际记录判断。格式被接纳不表示计划已批准或业务结果通过，关闭该配置的旧任务保持原失败出口。
