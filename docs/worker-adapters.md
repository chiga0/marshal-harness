...
Pi Adapter `0.4.0` 当前精确闭集支持 Pi CLI `0.84.1` 与 `0.84.3`。`0.84.3` 在 2026-08-26 的 Mac 真实非交互 canary 中，候选 executable digest 为 `sha256:1c3a5094b54aae9ae98c66516ce8c6578140363d081471ca7e91f9cb8c23dc8a`；`env -i` 仅显式保留 Node bin `PATH` 与必要宿主变量时，冻结 argv 表面保持有效，实际 stdout 使用 session JSONL v3。`agent_end(willRetry=false)` 只关闭一次低层 run，之后仍可合法进入有序 `compaction_start` / `compaction_end` 与 continuation；`agent_settled` 或进程 EOF 才关闭会话。Adapter 对 compaction reason、结果 shape、summarization retry、usage、重复/乱序 closure 和 overflow 单次恢复执行 fail-closed 校验，不以忽略未知事件换取兼容。其他版本继续在 Worker 启动前 fail closed。该结论只是普通宿主进程兼容性，不构成恶意代码隔离或新的权威边界。
