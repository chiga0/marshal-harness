...
| 原包证据与发行 | 本次受限预发布签名/分发待执行；完整 UI stable 仍须三线必需范围通过 | 673原CI包已恢复并完成9月13日新消费2/2 PASS；9月11日限定升级/回退历史通过，旧临时原件不可获取，不伪造旧JSON |
