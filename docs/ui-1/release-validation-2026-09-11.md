...
更新日期：2026-09-14。本次补录9月13日真实浏览器证据及实际发行回执，不修改产品行为；不以文档合并关闭 UI 三条验收线。用户明确批准后，Chrome 优先 `v1.1.0-rc.1` 已预发布，签名、GitHub/OSS分发和公开安装核验通过，继续发布后实机测试。stable 保持 `v1.0.2`；范围见 [预发布记录](../v1.1.0-rc.1-release-dossier-2026-09-14.md)。

[PR #297](https://github.com/chiga0/marshal-harness/pull/297) 已合并为 `67317d7b8d08c59b82d7fffc4ad934666c8e3ca8`，精确候选的13项 Node/UI/同包消费及安全检查通过；长需求标题密度修复已进入产品主线。后继 PR #298 与 #304 仅更新文档，产品仍为673版本。该原 CI 包于9月11日已完成接纳与限定同根升级/回退；9月13日本机临时原件不可获取，已从同一CI资产恢复原包并新执行消费2/2 PASS，历史通过与当前证据可获取性分开记录。`61a0048e` 原包与源码构建证据各自保留范围，不替代673原包。功能与可靠性、视觉交互仍为 PARTIAL，真实目标用户 NOT_RUN；Chrome 原包隐藏退避与实际 Safari 受控任务已补验，Safari 下载、真人尚未完成；签名预发布已完成。以下带具体时间的“尚在运行/待合并”描述保留为历史记录；当前状态以上表及9月13日补验为准。
...
| E21 与性能 | 原包真实隐藏退避已限定通过，不外推持续负载或通用SLO | 9月13日 Chrome 列表隐藏约31秒轮询、详情隐藏46.699秒零请求，恢复分别约5秒/2秒；真实尾页复读去重及新进度≤3秒等原样本保留 |
| E24/E25 交互 | 长标题受影响范围已补证；原单Task样本未覆盖跨另Task原文隔离 | 经典选项与确认纯键盘、标题六组、列表/卡片四组及完整200%视口四图；不重跑所有页面的笛卡尔组合 |
| 实际 Safari | 浏览器下载后消费待许可；不阻塞本次明确批准的 Chrome 优先预发布 | 9月13日实际 Safari 原安装包受控 Task completed，问答/计划批准/双作者/独立验收/本机发布及后验通过；本地93字节报告独立核对通过，不冒充浏览器下载或真实模型 |
| 真实目标用户 | 至少一位非开发参与者无指导完成关键任务并记录理解/误操作 | Agent模拟仅作补充，不冒充真人 |
| 原包证据与发行 | 本次受限预发布已签名/分发且公开安装通过；完整 UI stable 仍须三线必需范围通过 | 673原CI包已恢复并完成9月13日新消费2/2 PASS；9月11日限定升级/回退历史通过，旧临时原件不可获取，不伪造旧JSON |

### 9月13日原包浏览器补验与9月14日发行安排

本节覆盖下文较早的「真实隐藏退避、实际 Safari 未登录」待办口径，原失败与各自样本限制仍保留。Chrome 用原安装包673、正常 Bearer 登录和真实标签可见性变化；关闭工具 focus emulation 后能观察 hidden，未覆写 DOM 属性或伪造事件。列表前台三次 GET 时间 `64247.55011/64252.56068/64257.57153`，隐藏 `64300.002032/64331.001035`，恢复 `64358.633309/64363.6441/64368.653391`；分别约5.01秒/31秒/5.01秒，恢复可见 Safari 新建的原 Task。该 Task 经 Safari 正式取消后，用其详情观察 hidden 46.699秒、请求0，恢复七组详情请求约2.01秒，原记录未截断。两项均 `PASS_LIMITED`，不扩大为活动任务长运行或完整性能 SLO。

实际 Safari 后继原包 Task `task-646ac3f1-cff9-4008-a9f2-e81e68b2c7f8` 已走完正常登录、创建、回答 north、批准精确计划、双作者、独立 review/验收、明确授权本机发布及后验，最终 completed。独立只读核对本地 report.json 为93字节，SHA-256 `6f0187a48bde1dad9fc2938f4212dfe7bb8c04a8f011471815589f78001c829c`，east10/west20/region north；9次ACP执行均清理，发布启动1次。实际点下载后等待 Safari 首次下载许可，因此下载消费仍未完成；不把本地文件读取冒充浏览器下载。使用确定性 ACP fixture，非新真实模型/真人测试。先前无 requirements 合法输入触发私有 fixture 假设错误的失败被保留，原 Task 正式取消确认，后继仅修私有 fixture，没有改产品包或降低验收。

私有证据：`.marshal/ui-e21-authenticated-result-20260913.json`、`.marshal/ui-e21-detail-result-20260913.json`、`.marshal/ui-safari-compatible-20260913/browser-result.json`、`local-report-result.json`。认证沿用正常入口，经用户授权系统剪贴板传递，操作后清空，记录不含 token。9月14日用户批准先发布再实机测试，本次限定预发布不等待 Safari 下载或真人测试；完整 UI 三线仍保持 PARTIAL/PARTIAL/NOT_RUN，不因预发布自动通过。
