...
候选 `00393413` 后端三组通过，Linux Node 24 的唯一失败位于既有 Supervisor ACK 时序测试：fixture 使用真实时钟加 offset，却固定推进 4900ms，5 秒问题期限只剩不足 100ms；CI 该用例耗时 344ms，Core 正确拒绝已过期问题。后继修复只移除该人为推进，等待原 ACK 定时器，并给测试局部有界观察窗口；不延长产品期限或跳过取消/兄弟执行断言。具体复现及后继测试须另记，不将其误报为本次真实模型任务失败。

时序测试源修复 `69f57940` 经独立审查无 P0/P1：注入 150ms 迟滞时，旧测试 224.6ms 同栈失败，新测试等待原定时器约 5.08 秒通过；迟滞注入未提交。作者 Node 22/24 完整文件各 29/29 通过，独立 reviewer 精确提交及同条件迟滞各 1/1 通过。只改测试局部观察上限为 8 秒，原产品 5 秒期限及兄弟未停止/原取消回执断言保留；最终 CI 按后继 head 重新核验。
