...
更新日期：2026-08-30（`main@a7e9f93` implementation checkpoint；RC1 canary 已切换到闭集校验与固定 Go toolchain）
...
3. ADR 0056、0059–0066 的 process、terminalization、Supervisor、PreparedExecution、sealed proof 与 production factory 合同继续有效；[ADR 0067](adr/0067-darwin-ordinary-user-launch-and-attach-recovery.md) 已接受其 Mac ordinary-user 减法与精确取代范围。旧候选 `a6a0d63`、`506a647`、`6298eae` 冻结且不合入。RB1-authoritative existing-worktree Bind/Receipt/Release 与可重建 projection 已于 `main@259edd3` 实现；Linux staticcheck 的 U1000 跨平台 build-graph 修复已于 `main@60291e8` 推送，ResultIngress fail-closed 顺序修复随 `main@04c8fa9` 合入，provider held-descriptor 稳定性修复随 `main@a73c82b` 合入。该 exact-head CI 的 Secret scan 已通过，`CI #33290685508` 的 Ubuntu/macOS quality 仍在运行，整体尚未宣称全绿；本地 `go test ./internal/runstore -count=1` 当前仍暴露 canonical journal 与 prepared Run-start seam 回归，不能将质量门禁视为通过。完整 S1′（S1′-A reservation/full Attempt + S1′-B held descriptor/prepared proof/sealed successor，含 item 5 borrow seam/门禁）尚未进入 `main`，`3abed5a` 仍只是未合入候选；S2′ production composition、Attach/rebind 与 terminalization 同样未完成。
...
补充：`main@ceb8b39`、`main@822dcd3`、`main@a7e9f93` 已分别修复 canonical journal、sealed Run-start 测试 fixture 与带 authoritative-time/expiry 校验的 dispatch lease lookup；runstore 定向 test、race、vet 已通过。它们均为组件门禁修复，不改变 `COMPONENT` 成熟度，也不能替代 execution producer chain。
