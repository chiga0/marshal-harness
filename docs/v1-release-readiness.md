...
| 3 | 文件型耐久 authority ledger、幂等提交、重启恢复、旧 generation fencing、单一恢复模型 | `COMPONENT` | AttemptBinding creation-once + dispatch 冻结 lease expiry 已实现；agent/sandbox current-ledger recheck 已实现；**但 lease 是内存态（非耐久），跨进程恢复未覆盖**；embedded canary（`MARSHAL_EMBEDDED_SANDBOX=1`）首次跑通：AttemptBinding 落盘、lease 从 BindDispatch 正确恢复、pi 在 allocation 内执行（`634937b`）。 |
| 4 | 每 Attempt 双 binding + 接纳前 current-ledger recheck | `COMPONENT` | admission anchor 持久化 + 双侧 recheck 代码已实现；agent/sandbox capability digest 已分离（`686ee61`）；**`TestRealPiStrictE2E` 未跑通**（受 pi API rate limit 外部阻塞），embedded canary 验证了 AttemptBinding 接纳链圈路。 |
...
## 当前结论（2026-08-28，composition root 纠偏 + embedded fencing 修复后）

- `R0: PASSED`；`R1: IN_PROGRESS（INTEGRATED，real Agent in allocation 纵切）`；`R2/R3: IN_PROGRESS（COMPONENT，composition root 修复 + embedded canary 通过，真实 pi 严格 E2E 受 rate limit 阻塞）`；`R4: IN_PROGRESS（COMPONENT）`；`R5: IN_PROGRESS（COMPONENT，embedded canary 通过 AttemptBinding 且 pi 在 allocation 内执行——但严格 E2E 未跑通，受 pi API rate limit 外部阻塞）`；`R6: IN_PROGRESS（COMPONENT）`。
- 最短剩余路径：`TestRealPiStrictE2E` 跑通（等 pi API rate limit 重置后重跑）→ #3/#4 升级 INTEGRATED → v1.0-rc prerelease。
- 外部阻塞：pi API rate limit（qwen3.8-max via idealab，10 次/60 分钟）；Issue #212（macOS 签名身份，企业策略，仅影响 stable 通道）。
- embedded fencing 修复完成：exec-chain 在 embedded 模式下复用 BindDispatch lease（`634937b`）；marshal-server restart 测试重写为真实 Run 恢复（`da8cccd`）。
