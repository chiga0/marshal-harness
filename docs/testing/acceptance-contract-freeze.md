...
本测试包不接入 `TaskApplication`、HTTP、Provider、VerificationPort 或 Core，不登记 capability，不写入产品运行态，也不把测试结果变成 `ReviewDecision`、Verification receipt 或业务完成。测试中使用的 Store 是临时的真实 SQLite Store；fixture 只提供可重复的事实关系，不能证明生产运行时已经支持 ADR0107。
...
- 当前 Node 没有独立耐久 `attemptId`。测试使用 `task-attempt-ref/v1` 的复合身份：`workerId`、`commandId`、`generation`、`reservationDigest` 和 `reservationEvent`。五者必须能从同一 Task 的 `attempt` projection、输入快照和唯一 `worker.reserved` 事件重算；`Worker.worker.attempt` 不参与唯一身份。
...
- 任一合同、计划、选果、候选、Attempt、reservation event 或 source Artifact 变异都会拒绝；不能把缺证据、重复事件或未知字段降级为通过。
...
当前测试覆盖规范摘要与原始字节摘要、真实 SQLite 事件/投影绑定、完整证据通过、十类绑定变异拒绝、精确输入/制品字节变异拒绝以及未知字段、重复事件和检查目录缺失拒绝。它不替代真实 Agent、业务后验、冷故障恢复、UI 三线验收或 S01→M01→C01 E2E。
