# 实施计划历史参考（2026-09-07 归档）

> 本文保留 M0–M13、I186、S1′/S2′、T1/T2 的历史顺序与验收资产。下文“当前执行顺序”“v1.0 权威实施表”“不得新建状态库”等不再作为当前目标的排期或架构否决条件。当前方案只读[实施计划](implementation-plan.md)、[Milestone](agent-team-service-milestones.md)和[合同适用性](design-contract-map.md)；不得据归档提前放行新运行时、删除失败证据或重复建设已完成组件。
...
> **2026-09-04 ADR 0079 S1 / fixed server T2 runtime checkpoint**：`feat/fixed-server-t2-lifecycle@138a49c` 已把同一 resident fixed server surface 延伸到 collect/verify/review/Decision；真实 Run `run:fixed-server-final-138a49c` 经 response-loss 与 server restart 收敛到唯一 `RUNNING` Attempt，但 signed Node 在旧 `PT_TRACE_ME → exec → PtraceDetach` mechanics 后以 `SIGTRAP` terminal，fixed Marshal server 本身持续存活，直接 Node/Pi probe 正常。候选 `f71232c` 已完成 [ADR 0079](adr/0079-darwin-posix-spawn-setexec-barrier.md) 的 S1 dormant mechanics：固定 Marshal 内置、Darwin arm64/amd64、`CGO_ENABLED=0` 的 libSystem `posix_spawn(SETEXEC | START_SUSPENDED)` bridge 与纯负测，零匿名 helper，producer 仍 disabled，v1 journal 不混入新 mechanics。双架构 fixed build/package compile-only、Linux package compile-only、vet、staticcheck、diff-check 与 fixed-image symbol/dependency 检查已通过。下一步严格是 S2 closed Supervisor v2 protocol/journal 与 v1 read-only routing，再做 S3 new-session-only production cutover并以相同 fixed bytes 重跑真实 Pi T2 到 `ACCEPTED`；Issue #212 signing/notarization 与 stable gate仍开放。

> **2026-09-04 fixed server T1 integration checkpoint**：PR [#252](https://github.com/chiga0/marshal-harness/pull/252) 已以 `main@b39c346` 关闭 S4 runtime-root observation、current RB1/projection join、exact StartRun response adoption、pure-read InspectRun 与 resident sealed RUNNING recovery 缺口；[required CI 33850189142](https://github.com/chiga0/marshal-harness/actions/runs/33850189142) 全绿。[T1 run 33851302323](https://github.com/chiga0/marshal-harness/actions/runs/33851302323) 从同一 head build-once 固定 candidate，以真实 Pi `0.84.4` 证明 `StartRun→RUNNING→InspectRun`、response-loss、受控 crash、owner epoch `1→2` recovery-before-ready 与 strict-successor same-key receipt replay；spawn/resume/Attempt/worker/delivery 均唯一，`cliFallback=false`。ADR 0076 fixed transport/T1 capability 现为 `INTEGRATED`。下一步不再重复 T1，而是扩展同一 resident `PublicApplicationPort`，把 collect/verify/review/独立 Decision 接到 T2 并到 `ACCEPTED`；旧 CLI mutation 不能在 server 持有 repository owner 时作为第二 writer。T2 后依次完成完整 recovery/fault matrix、managed signing/notarization、Linux stable gate 与受保护 stable candidate。

> **2026-09-04 main exact-head CLI closure checkpoint**：`main@c2198e3` 已用 build-from-head 固定 candidate 与真实 Pi `0.84.4` 完成单 Attempt sealed CLI 纵切；[run 33788766642](https://github.com/chiga0/marshal-harness/actions/runs/33788766642) 到 `REVIEW_PENDING`，独立 reviewer `P0=0/P1=0`，[finalize 33790168049](https://github.com/chiga0/marshal-harness/actions/runs/33790168049) 到 `ACCEPTED` 且 immutable carrier 校验通过。该 marker canary 关闭 #226；#224/#225 仍须按 ADR 0075 用 `m13-e2e-dogfood` 的真实复杂任务补齐 default umask、“散文 + 单 JSON”与时间/token 外部证据。该 checkpoint 本身不升级 I186 成熟度，也不替代 fixed server 或 recovery/fault matrix；fixed server T1 已由上方后继 checkpoint 关闭，当前顺序为 T2 server 路径到独立 Decision/`ACCEPTED` → recovery/fault matrix → managed signing/notarization → Linux stable gate → 受保护 stable candidate。macOS 26.6.2 的 ptrace exec-stop 兼容性须以窄 ADR 替换底层机制，固定 `marshal` 单二进制与上层 lifecycle 不变。

> **2026-09-03 fixed server S4 candidate checkpoint**：`main@0761ad3` 已包含 ADR 0076 的 S1–S3。该时点的 S4 候选新增固定 `marshal control-plane serve|status|inspect|start`、recovery-before-ready 的 resident production composition、独立只读 current-owner client view，以及有界 accept-stop/drain/cancel/exact-unlink shutdown；没有临时 executable、第二 writer/root 或 CLI fallback。mutation client 必须显式携带并在 response-loss/restart 时原样复用 canonical UTC deadline，server 不重新起算。候选已通过唯一独立 reviewer（一次 aggregate rework 后 `P0=0`、`P1=0`）以及 Darwin/Linux 定向 compile-only、vet、staticcheck 与 architecture/diff 门禁；当时 exact-head macOS 动态 CI 和同一 fixed bytes + 真实 Pi 的 RUNNING/restart/response-loss replay canary 尚未完成，所以只能标记 `COMPONENT`。该门禁现已由上方 2026-09-04 checkpoint 关闭；详细审计见 [设计审计报告](audit-report.md)。
...
> **2026-08-29 implementation checkpoint**：RB1-authoritative existing-worktree Bind/Receipt/Release 与 recovery projection 已于 `main@259edd3` 实现；Linux staticcheck U1000 修复已于 `main@6176868` 推送。该 exact-head CI 的 Ubuntu quality 与 secret scan 已通过，但 macOS quality 失败，整体仍未全绿。RC1 build-once distribution contract（`main@2d7da6a`）、installer exact opt-in/fail-closed guard（`main@e6a78a3`）与 immutable carrier checker/receipt Schema/hostile matrix（`main@66523d9`）均已实现并独立审查。完整 S1′（S1′-A reservation/full Attempt + S1′-B held descriptor/prepared proof/sealed successor，含 item 5 borrow seam/门禁）尚未进入 `main`，`3abed5a` 仍只是未合入候选；S2′、Attach/rebind、terminalization、最终 fixed-bin Pi→独立 Decision→`ACCEPTED`、真实 same-bytes canary/carrier、tag、GitHub prerelease与release asset仍开放；不升级R2–R6。
...
1. 在已通过 T1 的 authenticated fixed `marshal control-plane serve` 与 durable delivery ledger 上完成 T2：collect/verify/review/独立 Decision 全部经同一 resident `PublicApplicationPort` 到 `ACCEPTED`，保持一个 fixed binary 和一条 authority write path；
2. 在真实 fixed server 路径补齐 T1/T2 之外的 kill/restart/cancel/timeout/retry、重复 start、stale/replayed result、process reuse 与 source/cwd/process/binding drift，形成唯一可回放 recovery decision；
...
- 默认每个切片必须在同一变更中接到 `cmd/marshal` 的真实 composition root 并取得真实路径证据，禁止无界铺 package/schema 后延期集成；ADR 0063/0065 曾允许的 **S1 sealed proof component → 立即相邻 S2 fixed composition** 窄例外已由 `main@3819462` 的真实链闭合，不得再作为新增 component 的依据；`marshal control-plane serve` 属于 RC1 后继；
