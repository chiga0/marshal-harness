...
| `I186-R3` | `IN_PROGRESS` | `COMPONENT` | production selector已于`main@d4b9647` fail closed；ADR0056/0063/0065合同已接受，ADR0067正在提议Mac ordinary-user减法。S1′ fresh-start proof、S2′ fixed composition、只读Attach与Darwin process/terminalization真实负测均未完成。 |
...
当前最短剩余路径是：审查并决定ADR0067；若接受，从当前main重建S1′ fresh-start sealed proof（不合入`a6a0d63`/`506a647`/`6298eae`）→立即相邻S2′ fixed`cmd/marshal`本地CLI composition，证明exact resume后唯一`RUNNING`与两边response-loss只查自身ledger→单独实现只读`Attach`和`bind-authority(owner-successor)`，只恢复`process-started`已耐久且无pending的同一Supervisor→按ADR0056/0061接入terminalization/transcript/cleanup（R2/R3/R4）→按ADR0062接入authenticated`marshal control-plane serve`与durable delivery ledger→当前主线fixed-bin E2E经独立Decision进入`ACCEPTED`并证明无successor双活（R5）→发布可验证unsigned RC，再provision macOS signing/notarization并通过Linux stable gate（R6）。pre-start/跨owner pending/identity ambiguity保持typed intervention；合同接受本身、候选分支、单次live pass或reviewer verdict都不能单独升级阶段。
