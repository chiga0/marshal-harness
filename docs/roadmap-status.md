...
**默认团队接入修复（实施中，未发行）**：v1.0.2 已安装但无业务配置时尚不能启动通用团队，`status=connection_unavailable` 不应让用户反复安装或填写同一连接变量。用户已接受 ADR0099 的通用文件交付范围；候选 `49041af6` 已集成默认 Qwen 装配并通过独立机器验证。首次真实 Task 两作者并行完成、独立 Review 接受，但后续 Leader 返回 `invalid_leader_decision`，尚未最终交付；正在定位，不重复已完成作者工作来掩盖故障。见[实机记录](generic-files-live-validation-2026-09-10.md)。两种真实文件需求出口仍未通过，安装器仍固定已发布 v1.0.2；不把 DataWorks 发布/补数列为已实现。
