...
**最新主线**：[PR #297](https://github.com/chiga0/marshal-harness/pull/297) 已合并为 `67317d7b8d08c59b82d7fffc4ad934666c8e3ca8`，精确候选13项 Node/UI/同包消费及安全检查通过；长需求标题密度修复已进入主线。`61a0048e` 原包接纳、同根升级/回退及 E12 控制补验已完成，但不含本次修复；新 main 原始资产及最终同包验收仍待完成，UI尚未正式发布。

**当前 UI 验收**：功能与可靠性 PARTIAL、视觉交互 PARTIAL、真实目标用户 NOT_RUN。延迟ACK、Git patch 独立消费、8 MiB边界、E09/E12原控制回执、E13/E14限定反例及经典选项键盘已有证据，不重新排为完全未测。`c14df00e` 构建的长标题六组（1440/375/200%各浅深）、独立列表/卡片四组及完整200%视口四图已限定通过；源码构建不冒充新main原包，单Task不证明跨另Task原文隔离。剩余真实隐藏退避、重复页与组合性能、新main资产、实际Safari及真实目标用户仍待验。所有原失败、工具/脚本限制和各项范围见[验收记录当前结论](ui-1/release-validation-2026-09-11.md#当前结论)。以下已发布API的B1–B3状态不被UI待验撤销。
...
| UI-1 完整界面交付 | `IN_PROGRESS`（PR #297 修复已合入 main，未发布新版） | 真实隐藏退避、重复页与组合性能、实际Safari、真实目标用户及新main原包/最终发行待验；已测标题/控制等范围不重复排期，不重新打开已发布 API 的 B1–B3 出口 |
