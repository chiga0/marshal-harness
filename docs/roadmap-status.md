...
- 最新实施候选 `feat/team-resident-progress` 由 `00d3749439d6a6c55aa3dc309361073f6612948e` 推进，已接自动 Collect/Verify、公平调度和冷启动 Verify 屏障。`495ab02` 完整 CI `34144298657` 五项绿；真实双 Pi `34145704791` 进入 Collect 后报 `pi-result-final-object-invalid` 并持久 halt，未形成团队 Decision/集成成果。`912b5f1` 的 Darwin 定向 `34146665644` 13 项通过，仅补齐安全诊断，不是业务修复。当前后继实现显式 `native-terminal/v1`，让 Adapter 从真实终态生成控制结果、模型只交付业务成果；旧 Run 不重释、不原样重试。新候选须独立审查及动态验证，B1/B2/B3 状态不升级。
