...
**Chrome 优先 UI 预发布准备中**：用户明确批准先发布再实机测试，拟发行 `v1.1.0-rc.1`，复用 source `67317d7b` 的原 CI 包，状态 `PREPARING`；尚未记录实际签名/tag/分发成功。`v1.0.2` 保持 stable，UI-1 不关闭。9月13日 Chrome 原包真实隐藏轮询已限定通过；实际 Safari 完成受控团队交付及本机后验，浏览器下载待许可。真人可用性仍 NOT_RUN，完整 UI 三线仍未全部通过；不再等待 Safari 下载或真人测试才准备本次显式预发布。身份、范围与后续实机任务见 [预发布记录](v1.1.0-rc.1-release-dossier-2026-09-14.md)。

**最新主线**：[PR #297](https://github.com/chiga0/marshal-harness/pull/297) 已合并为 `67317d7b8d08c59b82d7fffc4ad934666c8e3ca8`，精确候选13项 Node/UI/同包消费及安全检查通过；长需求标题密度修复已进入主线。`61a0048e` 原包接纳、同根升级/回退及 E12 控制补验已完成，但不含本次修复；后继文档主线为PR #298的`b999607b`，产品未漂移。673原CI包接纳与限定同根升级/回退于9月11日已通过；9月13日本机临时原件不可获取，已从同CI资产恢复原包并重新完成消费测试2/2 PASS，新证据落持久目录，不伪造旧证据或声称重跑升级。UI尚未签名发行。

**当前 UI 验收**：功能与可靠性 PARTIAL、视觉交互 PARTIAL、真实目标用户 NOT_RUN。延迟ACK、Git patch 独立消费、8 MiB边界、E09/E12原控制回执、E13/E14限定反例及经典选项键盘已有证据，不重新排为完全未测。`c14df00e` 构建的长标题六组（1440/375/200%各浅深）、独立列表/卡片四组及完整200%视口四图已限定通过；源码构建不冒充新main原包，单Task不证明跨另Task原文隔离。103Task缓存＋500事件的新进度显示≤3秒及真实尾页复读去重已限定通过，低样本、混合构建不外推全部性能或通用SLO。9月13日原包 Chrome 隐藏退避与实际 Safari 受控交付的新证据见上文；Safari 下载、真实目标用户和最终签名发行仍待完成。所有原失败、工具/脚本限制和各项范围见[验收记录当前结论](ui-1/release-validation-2026-09-11.md#当前结论)。以下已发布API的B1–B3状态不被UI待验撤销。
...
| UI-1 完整界面交付 | `IN_PROGRESS`；Chrome 优先 `v1.1.0-rc.1` 为 `PREPARING` | Chrome 原包隐藏退避限定通过，Safari 受控任务 completed/独立验收及本机后验通过、浏览器下载待许可；真实目标用户 NOT_RUN。用户批准先预发布再实机测，签名/分发待执行；不关闭完整三线，不重开 API B1–B3，stable 仍 v1.0.2 |
