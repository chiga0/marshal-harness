...
- 2026-09-08 Task HTTP 候选为 `8543878cc9cc9b095e94c06c0cc41987611149f2`，已推 [PR #271](https://github.com/chiga0/marshal-harness/pull/271)，非 main 合并。同 RB1 草稿/确认/查询、resident 原 writer lane、真实 CLI 封闭参数与 revision 400/409 已接线；原 reviewer 聚合修正及复核无 P0/P1。首次完整 CI 暴露 `productionruntime→planning` 新依赖，后继改为真正的组合根 DI，未扩大架构白名单。精确新 SHA 的 ECS 四包定向 race、Mac held Session 冷重放和 17 项客户端测试通过；[完整 CI 34184969456](https://github.com/chiga0/marshal-harness/actions/runs/34184969456) 截至本次核对为 Linux 两架构 conformance/secret scan 通过，两平台 quality 仍在运行。Mac 部分固定测试程序被安全检查终止，保留未过结果；Linux 不替代 Darwin。公网 ECS 使用独立受限账号；内网机器不接 GitHub CI，模型额度不足期间不发起真实 Agent 重试。精确证据与缺口见 [Task HTTP 入口](task-http-preview.md)及 PR 验证记录。
- CI 等待期间已从精确 `8543878` 创建独立 `feat/task-b1-closure` 与 `feat/task-delivery-consumer` worktree，分别开发自动独立 Decision/完整制品下载和安全下载消费；第三路只读准备 Task cancel 的原提交点/停止链方案。主 Agent 负责整合与候选 CI。上述为正在实施的后继，不是已完成能力；没有新真实团队交付、取消完成或正式发布证据。
