...
**最新主线**：PR #295 已于21:37:33合并为 `61a0048e`，本地/远端 main 同源；其精确候选 `9cdfc074` 全13项 Node/UI/同包消费及安全检查通过。已消费记录修复进入主线。最终 main 资产 run `34605383372` 在途，UI仍未正式发布；下段保留合并前的验收范围，已测项不重复排期。

**后继验收更新（PR #295，尚未合并）**：`9cdfc074` 已通过最终独立产品/安全审查，整合496项UI测试、4项浏览器护栏与构建通过；远端完整 Node 矩阵仍在运行。延迟 ACK、Git patch 独立消费与8 MiB制品边界已取得限定证据，不再列为完全未测；已消费记录修复包含在该候选。当前继续补并行分工与双回执的实际画面、剩余交互反例，以及最终 main 原包验收。真实隐藏页/Safari及目标用户验收仍未完成。下段 PR #294 检查点保留当时范围，具体后继证据以[验收记录末节](ui-1/release-validation-2026-09-11.md#最后修复与制品边界补验2026-09-11)为准；代码审查通过不等于 UI 正式发行。

**UI 完整验收进行中，尚未发布新版**：[PR #294](https://github.com/chiga0/marshal-harness/pull/294) 于 21:06:16（Asia/Shanghai）经独立审查及精确 source `3ed0d3ed` 的13项 Node/UI/原包消费、双平台准入与安全检查通过，合入 main `0549aa82`；此前 PR #293 的 `9852e4da` 保留为原包验收基线。本轮关闭暂停成功后按钮仍锁住和创建引用超限误报未知的两项功能 P2；整合483项UI测试及4项护栏通过。第三轮真实 Pi 团队及四项原输入/交付/回执下载已核对；`9852e4da` 原包消费与限定同根升级回滚通过，但不替代新 main 最终资产。问题/方案/授权窄屏与200%键盘、错误下载/陈旧响应和100Task＋500事件组合滚动已取得限定证据，不外推全范围或长期稳定。三线仍为功能 PARTIAL、视觉交互 PARTIAL、真人可用性 NOT_RUN；剩余延迟ACK、Git样例、真实隐藏页/Safari、目标用户及最终签名资产出口。所有原失败、脚本前提错误及支持范围见 [本轮验收记录](ui-1/release-validation-2026-09-11.md)。以下已发布 API 状态不被 UI 待验撤销。
