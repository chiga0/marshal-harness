...
- 最新实施候选由 `00d3749439d6a6c55aa3dc309361073f6612948e` 推进，已接自动 Collect/Verify、公平调度、冷启动 Verify 屏障与显式 `native-terminal/v1`。`a5418f4` 实机 service 已通过独立 Verify，但 ReviewPacket 暴露 request context 丢身份；`88883d9` 修复后完整 CI `34152276913` 五项与 Darwin 定向 33 项通过，真实团队 `34153526402` 却先因 Provider `length` 拒绝，再因停止清理重复 Collect 卡住，未到 ReviewPacket。当前修复已收集结果→Stop→Close 收口，补冷重开/零重复收集回归，动态结果待验证；不把旧 RUNNING 投影当活跃进程，不原样重跑，不宣称已有团队 Decision/集成/下载。B1/B2/B3 状态不升级，详细证据见[审计报告](audit-report.md)。
