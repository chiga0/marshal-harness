...
## 当前唯一状态与关键路径（2026-09-15）

**2026-09-15 逐项业务验收仍未整体通过**：后继 `4f68bd27` 已将批准前约定、逐项文本证据、原 receipt/事务绑定、返工消费及 UI 连通并打包。实际 HTTP 暴露的附加证据接线 P1 已修复；最新同包的实际服务、375/1440 浏览器与坏响应拒绝复验通过。原 S02/C01 失败及最小正例已独立冻结：`42ce94f3` 四例模型组件均在180秒超时、零正文，语义无法验收；同配置短请求约22秒成功，单次600秒窗口的 S02 负例在373.3秒返回合法返工，后续S02修正材料因遗漏结构化字段拒收，C01原负例虽合法却被业务误收；最后C01修正材料合法返回且恢复项有依据，但操作效果被不充分地豁免；四条延长诊断均已结束，整体仍不通过。独立审查还发现原“正例整体accept”忽略过程证据缺失，后继须先分清内容、过程与后验责任。原超时记录不改写，也不能将单条正确判定当从零 E2E 通过。真人可用性仍 NOT_RUN，本批不发布。身份、范围与缺口见[后继候选](ui-1/experience-e2e-2026-09-14.md#批准前约定与逐项评审的后继候选)。

**2026-09-15 确定性后验实验已集成**：`fb1f21a1`/`c13a878d` 加入离线 `verifyOrders`、`verifyGuide` 与 `verifyRecoveryModel`，13 项测试通过。它们分别验证冻结订单的整数结果、有限批准文案模板和显式四步恢复模型；所有结果标记 `authority:false`，不接入 Core、不改变权限/预算/状态机，也不代替真实业务后验。详细适用范围见[确定性业务后验实验](testing/business-postconditions.md)。

**执行透明度与从零业务验收进行中**：`1eccece1` 已直接接通固定作者指导，独立回归/HTTP/同包消费通过；真实S01一轮有限通过，S02仍将未提供的日程与直接到场方式写成既定事实，原Review误收；C01真实返工修复标签问题后，仍缺导入两步失败后的提交/跳过规则。固定作者指导漏传已排除，业务语义缺口继续开放，不以换模型或重跑偶然成功关闭。UI后继 `2846a631` 收敛长评审原文与技术标识的概览展示，集成UI612项及4浏览器护栏通过，实际服务有限补验通过（原已完成Task、零模型，非空评审行动项HTTP未覆盖）。尚未发布本批，不改变既有发行完成事实，也不关闭UI-1；当前真实用户验收未执行。范围与失败证据统一见[验收记录](ui-1/experience-e2e-2026-09-14.md)与[审计报告](audit-report.md)。
...
**最新主线**：PR #307 产品 source `2fde5038c7238894aefd11e802cbe9b8d16f07c6` 的 [CI 34806468018](https://github.com/chiga0/marshal-harness/actions/runs/34806468018) 13/13成功；PR #308 合并主线 `4a86a5e9` 仅锁定该原包发行 pins。RC.2 原 ZIP、manifest、helper 已独立核对，Decision 的签名批准已由维护者落实，RC.2 tag `4a2c2ed3` 指向该产品 source，GitHub 8项资产已公开；OSS run34811596878成功，公开安装包升级、自动连接、ready/UI200及正常停服通过；这不是公开安装后新一次真实模型验证。历史 PR #297 的 `67317d7b` 原包已作为 RC.1 签名预发布，以下历史证据不改：`61a0048e` 原包接纳、同根升级/回退及 E12 控制补验完成；后继PR #298文档主线 `b999607b` 未改变产品。673原CI包接纳与限定同根升级/回退于9月11日通过；9月13日本机临时原件不可获取，从同CI资产恢复原包并重新消费2/2 PASS，未伪造旧证据或声称重跑升级。

**当前 UI 验收**：功能与可靠性 PARTIAL、视觉交互 PARTIAL、真实目标用户 NOT_RUN。延迟ACK、Git patch 独立消费、8 MiB边界、E09/E12原控制回执、E13/E14限定反例及经典选项键盘已有证据，不重新排为完全未测。`c14df00e` 构建的长标题六组（1440/375/200%各浅深）、独立列表/卡片四组及完整200%视口四图已限定通过；源码构建不冒充新main原包，单Task不证明跨另Task原文隔离。103Task缓存＋500事件的新进度显示≤3秒及真实尾页复读去重已限定通过，低样本、混合构建不外推全部性能或通用SLO。9月13日原包 Chrome 隐藏退避与实际 Safari 受控交付的新证据见上文；Safari 下载、真实目标用户仍待完成；本次限定预发布签名分发已完成。所有原失败、工具/脚本限制和各项范围见[验收记录当前结论](ui-1/release-validation-2026-09-11.md#当前结论)。以下已发布API的B1–B3状态不被UI待验撤销。
...
**正式发行前的历史未闭问题（已被上文 2026-09-10 收口记录和下表覆盖）**：当时仍缺正式安装后的真实 Pi 全程验收、真实局部修正及保留无关成果、完整故障恢复及 B3 发行证据，B2-L 当时为 `IN_PROGRESS`。这些不是当前重新打开的待办；失败和成本保留，不重做 B1、不改变 API-STABLE 原范围，也不把业务交付当 Marshal 软件正式发行。
...
| UI-1 完整界面交付 | `IN_PROGRESS`；Chrome 优先 `v1.1.0-rc.1` 已预发布 | 签名、GitHub/OSS分发及公开原包安装核验通过；Chrome 隐藏退避限定通过，Safari 受控交付及本机后验通过、浏览器下载待许可；真人 NOT_RUN。继续发布后实机测试，不关闭完整三线，不重开 API B1–B3，stable 仍 v1.0.2 |
