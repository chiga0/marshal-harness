...
**默认团队接入修复（实施中，未发行）**：v1.0.2 已安装但无业务配置时尚不能启动通用团队，`status=connection_unavailable` 不应让用户反复安装或填写同一连接变量。用户已接受 ADR0099 的通用文件交付范围；本轮将产品内默认 Qwen 装配与本地启动集成，同一配置实测两种文件需求。仅文件成果，不把 DataWorks 发布/补数列为已实现。后继源码与测试尚待完整冻结审查及实机，安装器仍固定已发布 v1.0.2。
