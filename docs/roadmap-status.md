...
| UI-1 完整界面交付 | `IN_PROGRESS`（PR #297 修复已合入 main，未发布新版） | 真实隐藏退避、实际Safari、真实目标用户及最终签名发行待验；673原CI包已恢复，9月13日新消费测试2/2 PASS，旧临时证据未恢复；Safari仅打开连接页、未登录。已测标题/控制/组合新进度与尾页复读等限定范围不重复排期，不重新打开已发布 API 的 B1–B3 出口 |
