...
RC1 只能在 Darwin arm64 上使用以下精确命令：
...
GitHub 的 `latest` API 不返回 prerelease。`v1.0.0-rc1` 必须使用上文的精确 tag + preview opt-in 命令，不会由 latest/stable channel 自动获得。RC 是候选/预览资产，不代表已满足 Issue #212 的 macOS 签名/notarization 或 v1.0 `RELEASED` 门禁。
