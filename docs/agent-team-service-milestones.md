...
**2026-09-09 当前目标调整**：[ADR0094](adr/0094-trusted-single-user-role-team.md) 设计已接受。当前交付面是可信单用户的 Leader＋开发/Reviewer/验收角色团队，Leader 先组合已有职责；强 OS/凭据隔离证明从当前 profile 的阻断项转为后继加固，不放松独立证据、命令授权、预算、恢复和单写者。B1/B2 已有工作保留，本次不自动升级阶段；B2 补一个明确授权的业务发布及发布后验证闭环，B3 的受保护软件发行和可靠性继续保留。不是协议 breaking change：本次无 API/角色枚举/格式修改或迁移，API-STABLE 原范围不撤回；未来真实协议/持久语义增量需明确兼容策略。旧普通用户实机不改成 production。
...
| **B2：本地 API 可用** | 保留问答/计划、制品/审计/恢复与简启动；补 Leader 协调和明确授权的代表性业务发布/发布后验证 | 原业务/API 证据继续计入；新增闭环具有精确授权、独立验收、原发布回执及后验，取消/丢响应不重复外部写 | 强 OS/凭据隔离证明、动态角色/Workflow 平台、任意高风险流程、全部 Agent 增强、U1 |
...
当前可信单用户 profile 按 ADR0094 以产品职责与权威分离解释分权，不要求先完成 OS 账号/凭据强隔离证明。七项既有功能证据继续保留，本次只是目标调整、不整体宣布 B1 通过，也不把 B2 新业务发布能力倒填进旧 PoC。
...
### B2 新增代表性闭环：角色协调与授权业务交付

按 ADR0094 复用现有规划/开发/Review/Verification 接缝，不增加 Leader 业务实体；当前 reviewer 枚举和 phase 映射不等于绑定成果的发布批准。新增一个完整纵切：Leader 必要澄清/有限分工→一次精确确认→互补作者→计划要求的独立 Reviewer/整体验收→下载→明确授权的单一测试目标发布→发布后独立检查与回执汇总。默认未授权仍只交付；发布/后验不是当前已实现能力。

出口必须包含无授权/越目标零发布、陈旧成果/作者自证拒绝、原回执丢响应/重开不重复写、cancel/期限竞争，以及发布成功但后验失败的诚实状态。未来新增枚举/终态/副作用事实需显式版本与兼容策略，不能改旧 completed 含义。所有高风险业务、任意动态角色和通用 Workflow 平台不作首发前置；成功流程重复后才抽成有界模板。
