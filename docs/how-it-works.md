...
上图是当前单个 Run 的核心交付链。面向复杂 Goal 的目标流程还会在前后增加两个阶段：

- **执行前**：先澄清问题，再由不同视角做代码勘查、竞品调研、安全或架构审计；Lead 汇总共识、分歧和证据后提出 Plan，由 Marshal 接纳，而不是从原始需求直接派发实现。
- **执行后**：对 Outcome、Evidence、返工、预算、人工介入和 Provider 故障做复盘；经验先成为带来源的改进提案，经 review/批准后才进入 runbook、模板、测试、Policy 或 ADR。

Worker 可以通过目标中的受控协调面交换问题、进度和不可变 Artifact 引用，但不能借此修改 scope、预算、lease 或权威状态。详细设计见[前期研讨、Worker 协作与复盘](agent-collaboration-and-learning.md)。这些 Goal 级自动化能力仍在建设中；当前可以按操作手册人工组织调研队、评审团和复盘。
