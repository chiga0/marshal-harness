...
实现 sourceHead 为 `027d35622577bcd3ed957985309a7f59a51b248c`，PR 为 [#230](https://github.com/chiga0/marshal-harness/pull/230)。本地 Darwin/Linux compile-only、`go vet`、双平台 staticcheck、architecture check、diff-check 与 source scan 已通过；动态 hostile matrix 和 secret scan以 exact-head required CI 为准。测试覆盖有效认证与应用字节往返、exact cleanup、oversize、伪造 HMAC、proof replay、token ABA 和 short write。
