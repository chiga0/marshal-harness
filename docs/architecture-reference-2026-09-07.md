# 整体架构历史参考（2026-09-07 归档）

> 本文保留归档前的完整设计与实现映射，不再定义当前目标架构或排期。下文“当前”“终态”“冻结”“唯一”均限原记录时点及对应旧 profile。请先读[当前架构](architecture.md)与[合同适用性](design-contract-map.md)；旧 Accepted ADR 在旧运行路径仍有效，不能用归档删除证据门禁，也不能用历史实现形状否定新方案。
