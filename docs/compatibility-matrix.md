...
- Pi 精确兼容闭集：Adapter `0.3.0` 支持 Pi `0.84.1` 与 `0.84.3`，保持 session protocol v3 与 raw JSONL 审计语义；`0.84.1` 由具名协议 fixture 锁定，`0.84.3` 另经 2026-08-26 Mac 真实非交互 canary 与缩减后的真实事件 fixture 锁定。
...
| `pi` | `0.3.0` | Pi `0.84.1` / `0.84.3` | `MARSHAL_PI_PATH` | 两个精确版本代码锁定 `supported`；`0.84.3` Mac 非交互 canary 已执行 |
...
Pi Adapter `0.3.0` 与 Pi `0.84.1` / `0.84.3` 保持 session protocol 版本精确为 3 且 raw JSONL 审计语义不变：Marshal 不删除、不压缩、不改写 Provider transcript 字段，compact LF JSONL 按原始字节保留。两个版本的最小 `message_update` wire 均保持 `assistantMessageEvent` 内的 `text_delta`/`contentIndex`/`delta`，不含消息标识字段、`partial` 或顶层累积 `message` 快照。`0.84.1` 仍由 `TestCaptureJSONLAcceptsPi0841NormalizedMessageUpdate` 锁定；`0.84.3` 的真实 Mac canary 额外确认结构化 usage/cost、`agent_end(willRetry=false)` 及单次 `agent_settled` 闭合，并由 `TestRunAcceptsPi0843ObservedSessionV3` 的缩减 fixture 锁定。这些证据不扩展到其他 Pi 版本，也不构成恶意代码隔离。
