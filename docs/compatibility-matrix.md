...
更新时间：2026-08-20。本矩阵由共享 Conformance Suite、Live Probe 与受监督 Pilot 证据生成，只记录已验证事实；未验证组合一律不承诺。
...
| `qoder` | `0.1.5` | Qoder CLI `>=1.1.23 <1.2.0`（逐 binary evidence） | `MARSHAL_QODER_PATH` | 严格模式 `pending live evidence`；显式 Mac `ordinary-user` 可配置但新 `v6` transport 尚待真实 conformance，不能迁移 `0.1.2/v3`、`0.1.3/v4` 或 `0.1.4/v5` 摘要，也不提供 hardened authority |
