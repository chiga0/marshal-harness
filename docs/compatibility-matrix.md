...
更新时间：2026-08-26。本矩阵由共享 Conformance Suite、Live Probe 与受监督 Pilot 证据生成，只记录已验证事实；未验证组合一律不承诺。
...
- Pi 精确兼容闭集：Adapter `0.4.0` 支持 Pi `0.84.1` 与 `0.84.3`，保持 session protocol v3 与 raw JSONL 审计语义；`0.84.1` 由具名协议 fixture 锁定，`0.84.3` 另经 2026-08-26 Mac 真实非交互 canary、长任务 overflow compaction 现场证据与缩减后的真实事件 fixture 锁定。
...
| `qwen` | `0.1.1` | Qwen Code `>=0.21.5 <=0.22.0` | `MARSHAL_QWEN_PATH` | `0.21.x` 全画像 `supported`；`0.22.0` 仅 `workspace-write` |
...
| `pi` | `0.4.0` | Pi `0.84.1` / `0.84.3` | `MARSHAL_PI_PATH` | 两个精确版本代码锁定 `supported`；`0.84.3` Mac 普通闭合与 overflow compaction 证据已执行 |
...
Pi Adapter `0.4.0` 与 Pi `0.84.1` / `0.84.3` 保持 session protocol 版本精确为 3 且 raw JSONL 审计语义不变：Marshal 不删除、不压缩、不改写 Provider transcript 字段，compact LF JSONL 按原始字节保留。两个版本的最小 `message_update` wire 均保持 `assistantMessageEvent` 内的 `text_delta`/`contentIndex`/`delta`，不含消息标识字段、`partial` 或顶层累积 `message` 快照。`0.84.1` 仍由 `TestCaptureJSONLAcceptsPi0841NormalizedMessageUpdate` 锁定；`0.84.3` 的真实 Mac 证据额外确认结构化 usage/cost、普通 `agent_end → agent_settled`，以及长任务 `agent_end(length) → compaction_start(overflow) → compaction_end → continuation → agent_settled` 合法链。Adapter 延迟到 session closure 才提交 Provider failure，并拒绝 compaction outcome 伪造、reason 漂移、嵌套、summarization retry 乱序和未知事件。这些证据不扩展到其他 Pi 版本，也不构成恶意代码隔离。

Qwen `0.22.0` 仅声明 `workspace-write`：真实 canary 表明其 `edit` 元权限会同时移除用于写回 `WorkerResult` 的 `write_file`，因此 `read-only` 在拥有路径限定规则或独立结果传输前于 Worker 启动前 fail closed。

## 能力矩阵

| 能力 | opencode 1.18.13 | qwen >=0.21.5 <=0.22.0（0.22.0 仅 workspace-write） | qoder >=1.1.27 <1.2.0 | pi 0.84.1 | codex 0.145.x |
| --- | --- | --- | --- | --- | --- |
