# 历史参考：vision-and-scope（2026-09-14 文档收敛前）

> 本页保存基线 `12db5f908e1a15331e4edb06e9db3cd8c8c16d80` 的原文。下文的“当前”、实现缺口及阶段状态均属于原记录，不是当前支持声明或重新打开的待办；现行设计见 [vision-and-scope](vision-and-scope.md)，实际进展见 [Roadmap](roadmap-status.md#业务交付当前表)。原 ADR 的接受事实不因归档撤回。
