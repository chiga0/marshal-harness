...
macOS 上不要直接执行 `go test`：Go 会在随机 `go-build` 路径直接启动临时测试二进制，容易被企业端点策略反复拦截。先运行 `make build`，再用 `bash scripts/stable-go-test.sh ./internal/<包>/...` 做定向测试；`make test` 与 `make check` 已自动走同一入口，并把绝对 `MARSHAL_RUNNER=$(BINARY)` 显式传给脚本。Go 仍可在临时目录编译测试文件，但这些文件只作为输入，由指定的固定 Marshal `__go-test-exec` 校验并复制到其同目录的 `test/current` 后执行。锁覆盖完整测试进程生命周期；输入、`incoming` 与 `current` 的类型、权限、所有者和 SHA-256 不一致时 fail closed。测试进程内再次启动 `go test` 会经同一固定 Marshal 提前拒绝，避免递归争用单一锁或绕回匿名执行；需要子级测试时应把它提升为顶层显式 Gate。`GOFLAGS` 按普通用户工具兼容配置继承，调用者可显式覆盖普通 Go flag；已有 `-exec` 会被拒绝以避免两套执行器混用。该注入不是强制 authority，也不构成安全证明。生产 Worker 与 verifier 使用当前固定 Marshal image 及用户级 `~/.marshal/test-exec` 槽位，不在业务仓库写测试可执行文件；这只是 Mac ordinary-user 兼容机制，不是 hardened sandbox。
