...
当前状态：Milestone 0–6 已全部通过，Local MVP 标记 `USABLE`（2026-08-07）；M7（架构与契约）已于 2026-08-11 通过退出门禁；M8（Sandbox SPI/Fake/Local conformance + embedded/local 纵切）于 2026-08-13 `PASSED`；M9（`marshal-server`、Public API 与 Durable Runtime 基础）于 2026-08-14 `PASSED`。M8/M9 的通过不表示主执行路径已经收敛为 production Agent-in-Sandbox 链。2026-08-24 起按 [Issue #186](https://github.com/chiga0/marshal-harness/issues/186) 的 I186-R0→R6 纵切路线重整：R0 合同与基线、R1 最薄 Agent-in-Sandbox walking skeleton 已完成，下一步 R2 收敛唯一 production Command/Result authority path；M10–M13 暂停直接推进，等待 R6 后按真实证据重排。状态取值定义与验收证据见 [Roadmap 状态](roadmap-status.md)，目标架构见 [Runtime 架构](runtime-architecture.md)。
