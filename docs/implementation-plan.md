...
>
> **2026-08-29 S1′/S2′ producer P0 提案**：[ADR 0069](adr/0069-attempt-reservation-and-existing-worktree-allocation.md) 仍为Proposed。它要求ResultIngress在任何allocation/bootstrap/child副作用前以`attempt-opened/v2` creation-once预留Attempt；Run保持READY且不消费预算，只有同reservation/exact READY head的sealed successor才写入Attempt并消费一次预算。Mac ordinary-user workspace-write必须使用`bind-existing-worktree/v1` sidecar ledger诚实绑定既有Git worktree。ResultIngress path reopen及`OpenOwner`后才可`ObserveCurrentCore`是ADR0066已有实施P0。本提案尚未审查、接受或实现，不改变R2–R6状态。
...
2. 先完成ADR0069的独立审查与接受同步；随后由ResultIngress实现`attempt-opened/v2`，对同一exact READY head creation-once预留Attempt且不消费Run预算；只有sealed READY→RUNNING successor接受同一reservation、写入Attempt并恰好消费一次预算；legacy无reservation记录仅历史重放；
...
4. S2′必须立即相邻：实现existing-only Run open/acquire（未知Run不得`MkdirAll`）与`bind-existing-worktree/v1` sidecar ledger；既有Git worktree只做held identity逻辑绑定/释放，不创建、替换、clean、reset或删除；再按ADR0066接两阶段owner、canonical repository`.marshal`、controller、唯一factory/composition、fixed`cmd/marshal`窄application adapter与真实Pi fresh E2E；
5. S2′ concrete authority 必须在`internal/productionruntime/authority.go`与必要窄adapter内，按`attempt-opened/v2 reservation→existing-worktree bind intent/outcome→owner/reserved Attempt binding→allocation binding receipt→launch-authorized/StoredClosure→PreparedExecutionV2→sealed READY→RUNNING`产生同一reservation的真实链；fixed CLI/Pi必须亲历全链，seed/Fake/memory-only/legacy `execution.Run`/create-empty receipt皆不可达；S1′后立即相邻执行S2′；
