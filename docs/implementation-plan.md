...
**实施进展（2026-08-12，供接手 agent 参考）**：M8 硬门禁前两个 gate 已落地 main——gate-1（authority 双键空间 AuthorityNamespaceId/SecurityDomainId + SideEffect authority-record Schema，`internal/authority/`，PR #42）与 gate-2（ProviderRegistration/ProviderCapabilitySnapshot/ConformanceEvidence 类型 + Draft 2020-12 Schema + attestation 全链绑定，`internal/provider/`，PR #45）。同期合入基础设施修复：digest-fix（PR #41）、pi-auto-retry（PR #43）、publish commit-tree 修复（PR #44）。**下一步**：gate-3（legacy v1alpha1 CapabilitySnapshot → ProviderCapabilitySnapshot 的 fail-closed mapper）、gate-4（durable embedded registration + ledger recovery）、gate-5（snapshot/evidence validation）三者互不依赖、可并行 fan-out，均只依赖 gate-2 的 `internal/provider/` 类型；gate-6（enable DispatchLease match）必须最后。各 gate 的 taskSpec 草案曾存于 `.marshal/input/`（git 忽略，仅作本地参考），接手 agent 可按上述范围重新生成。**acceptance 设计教训**：(1) 不要要求精确测试函数名（诱发 worker rename 震荡），用功能性验收（查实现符号 + 测试通过）；(2) git 密集包的回归测试用 `-count=3` 而非 `-count=10`（后者撞 go 默认 10m 超时）；(3) worker 无 shell 权限跑不了 gofmt，格式问题需给精确 diff；(4) opencode 二进制自动升级会变 unsupported，备用 pi。
