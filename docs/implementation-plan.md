...
2. 先完成ADR0069最新定向修订的独立复审与接受同步；随后由ResultIngress/RB1实现`attempt-reserved` active→consumed|cancelled closed union，对同一RunID+exact READY seq/head lookup-before-mint；dispatch authority对reservation digest+RunID+reserved AttemptID lookup-before-claim并same-bytes replay；budget从held Run authority三次重验，只有sealed successor写Attempt并恰好计数一次；
