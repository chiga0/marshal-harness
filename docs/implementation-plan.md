...
>
> **2026-08-29 S1′/S2′ producer P0 提案**：[ADR 0069](adr/0069-attempt-reservation-and-existing-worktree-allocation.md)保持Proposed；首版`ebbfd86`独立审查`REQUEST CHANGES`（`P0=3`、`P1=4`）后已完成唯一aggregate rework。ResultIngress/RB1先以`attempt-reserved`按exact READY head lookup-before-mint；它不等于完整Attempt authority。随后依次生成dispatch lease/full identity、`attempt-opened`、owner binding，才可写RB1 existing-worktree bind intent/receipt。唯一sealed Run successor写Attempt并消费一次budget。Existing-worktree sidecar只是在固定layout中的可重建projection，RB1是唯一allocation authority。ResultIngress path reopen及`OpenOwner`后才可`ObserveCurrentCore`是ADR0066已有实施P0。提案尚未复审、接受或实现，不改变R2–R6状态。
...
| `I186-R2` | `IN_PROGRESS` | `COMPONENT` | command/result authority收敛到现有durable journal；ResultIngress事务化接纳；ADR0063/0065/0067合同已接受，ADR0069 Proposed登记fresh Attempt reservation与held-descriptor P0 | `main@912f659`已合入crash-atomic admission/worker-result/Run journal；ResultIngress须在副作用前creation-once预留Attempt但不消费Run预算，且不再path reopen；sealed successor才写Attempt/计数；旧候选不计进展 |
| `I186-R3` | `IN_PROGRESS` | `COMPONENT` | production selector已收紧到`LaunchCapable`；保留PreparedExecution、exact resume、sealed proof与fixed Supervisor；ADR0067已冻结Supervisor唯一承担mutation-adjacent source gate | ordinary-user明确N/A hardened；任一binding/process/source/cwd漂移拒绝；S2′须以existing-only Run open、同一reservation和`bind-existing-worktree` profile真实产生全`PrepareRunStart`链；ADR0069接受前不得实施该新持久化合同 |
...
| `I186-R6` | `PLANNED` | `DESIGN` | ADR0068已接受CLI-only RC1合同；failure conformance、精确opt-in安装、签名/notarization、升级/回滚、release均未实现 | 先以same-bytes发布Darwin arm64 unsigned RC1；其后接server、Issue #212 managed signing/notarization与Linux stable gate，能力成熟度才可升级为`RELEASED` |
...
2. 先完成ADR0069 aggregate rework的独立复审与接受同步；随后由ResultIngress/RB1实现`attempt-reserved` active→consumed|cancelled closed union，对同一RunID+exact READY seq/head lookup-before-mint；budget从held Run authority三次重验，只有sealed successor写Attempt并恰好计数一次；
3. 按已接受 ADR0067，从当前main重建S1′ fresh-start sealed proof：Core只做无副作用current source/cwd admission，Supervisor`spawn`做唯一mutation-adjacent exact-set gate；ResultIngress proof/shared guard与runstore private projector/self-only CAS直接建立在当前Store/Lease上；ResultIngress authority改用ADR0066 held descriptor backend，禁止pathname reopen；generic`Append READY→RUNNING`拒绝，明确不引入`506a647`通用substrate；
4. S2′必须立即相邻：实现existing-only Run open/acquire（未知Run不得`MkdirAll`）与RB1-authoritative existing-worktree Bind/Receipt/Release；固定`.marshal/runtime-v1/existing-worktree-bindings/`仅为descriptor-bound recovery projection，缺/落后从RB1重建，损坏/超前fail closed；目标只bind/release，不创建、替换、clean、reset或删除；
