...
export function Dag({ run, onAtt }: { run: RunDetail; onAtt: (i: number) => void }) {
  const { nodes, edges } = useMemo(() => {
    ...
    const nodes: Node[] = stages.map((s, i) => ({
      ...
      style: { border: `1px solid ${s.color}`, background: s.color + "22", color: "inherit", fontSize: 12, borderRadius: 6, padding: "6px 10px" },
    }));
    const edges: Edge[] = stages.slice(1).map((_, i) => ({ id: "e" + i, source: String(i), target: String(i + 1), type: "smoothstep", animated: true, style: { stroke: "#444c56" } }));
    return { nodes, edges };
  }, [run]);
  return (
