...
const COLW = 230, ROWH = 150, MAXC = 5, NW = 150;
...
export function Dag({ run, onAtt }: { run: RunDetail; onAtt: (i: number) => void }) {
  const { nodes, edges } = useMemo(() => {
    ...
    const pos = (i: number) => { const row = Math.floor(i / MAXC), c = i % MAXC; const col = row % 2 === 0 ? c : MAXC - 1 - c; return { x: col * COLW, y: row * ROWH }; };
    const nodes: Node[] = seq.map((d, i) => ({ id: d.id, position: pos(i), data: { label: d.label, att: d.att }, sourcePosition: Position.Right, targetPosition: Position.Left,
      style: { width: NW, textAlign: "center", border: `1px solid ${d.color}`, background: d.color + (d.dim ? "0d" : "22"), color: "inherit", fontSize: 12, borderRadius: 6, padding: "6px 0", opacity: d.dim ? 0.45 : 1 } }));
    const edges: Edge[] = [];
    const link = (s: string, t: string, color = "#444c56", o: { label?: string; animated?: boolean; opacity?: number } = {}) =>
      edges.push({ id: `e${s}-${t}`, source: s, target: t, type: "smoothstep", animated: !!o.animated, markerEnd: { type: MarkerType.ArrowClosed, color }, style: { stroke: color, opacity: o.opacity ?? 1 }, label: o.label, labelStyle: { fill: color, fontSize: 10 } });
