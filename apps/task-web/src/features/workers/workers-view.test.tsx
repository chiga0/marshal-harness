...
import {callsOf, makeDetail, makeFakeTransport, makePlan, makeWorker, TASK_ID} from '../tasks/detail/testing/fixtures';
...
function renderView(workers = [makeWorker()], overrides: Partial<Transport> = {}) {
  ...
  const detail = makeDetail({plan: makePlan({revision: '7'})});
  const utils = render(
    <QueryClientProvider client={client}>
      <MemoryRouter initialEntries={[`/tasks/${TASK_ID}/team`]}>
        <Routes>
          <Route path="/tasks/:taskId/team/*" element={<WorkersView detail={detail} workers={workers} transport={transport} onChanged={() => {}} />} />
        </Routes>
      </MemoryRouter>
    </QueryClientProvider>,
  );
...
describe('团队视图（P08 / E23）', () => {
  it('列表展示节点/角色/状态/阶段/最近观察与真实进展摘要，无百分比与费用假数据', () => {
    renderView();
  ...
  it('单 Worker 取消走 cancelWorker（taskId+workerId+任务 revision+幂等键），与取消任务严格分开', async () => {
    ...
    const [taskId, workerId, body] = callsOf(calls, 'cancelWorker')[0]!.args as [string, string, {revision: string; idempotencyKey: string}];
    expect(taskId).toBe(TASK_ID);
    expect(workerId).toBe('worker-0001');
