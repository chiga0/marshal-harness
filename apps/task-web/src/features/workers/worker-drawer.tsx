// Worker 明细抽屉（side sheet，URL 可定位）：全部可观察字段如实展示；usage/audit 缺失显示暂无数据；
// 内含单 Worker 取消 flow（permission-free：无额外权限模型，仅按任务 revision CAS + 二次确认）。

import {useEffect, useRef} from 'react';
import {createPortal} from 'react-dom';
import {Badge} from '@/components/ui/badge';
import {Button} from '@/components/ui/button';
import type {Transport, WorkerRecord} from '@/lib/transport/types';
...
export function WorkerDrawer({taskId, revision, worker, transport, onClose, onChanged}: WorkerDrawerProps) {
      ...
      >
        ...
        <section aria-label="身份" className="space-y-1 text-sm leading-[22px]">
          ...
          <dl className="grid grid-cols-1 gap-y-1">
            ...
            <Row label="尝试次数" value={String(worker.attempts)} />
        ...
        <section aria-label="用量" className="space-y-1 text-sm leading-[22px]">
          ...
          {worker.usage ? (
            <dl className="grid grid-cols-1 gap-y-1">
              ...
              <Row label="费用" value={worker.usage.cost !== null ? `${worker.usage.cost} ${worker.usage.currency ?? ''}`.trim() : '暂无数据'} />
              <Row label="覆盖/来源" value={`${worker.usage.coverage} / ${worker.usage.source}`} />
            </dl>
          ) : (
            <p className="text-text-secondary" data-testid="usage-unavailable">暂无数据（服务端未提供该 Worker 的用量）。</p>
          )}
        </section>
        ...
        <section aria-label="操作" className="mt-auto space-y-2 border-t border-border pt-3">
          {terminal ? (
            ...
            <CancelWorkerFlow taskId={taskId} revision={revision} worker={worker} transport={transport} onChanged={onChanged} />
          )}
        </section>
      </div>
    </div>,
