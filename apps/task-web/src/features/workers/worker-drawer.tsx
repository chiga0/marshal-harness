...
export function WorkerDrawer({
  ...
  const panelRef = useModalLayer<HTMLDivElement>({
    open: true,
    onEscape: onClose,
    initialSelector: '[data-drawer-initial]',
    lockBodyScroll: true,
  });
  ...
  return createPortal(
    ...
    >
      <div
        ...
        tabIndex={-1}
        className="flex h-full w-full max-w-2xl flex-col gap-4 overflow-y-auto border-l border-border bg-surface p-6 focus:outline-none"
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="secondary">
            阶段：{workerPhaseLabel(worker.phase)}
          </Badge>
          <Badge variant="secondary">
            角色：{workerRoleLabel(worker.role)}
          </Badge>
        </div>
        ...
        />
        <details className="workspace-disclosure">
          <summary>技术与审计详情</summary>{' '}
          <section
            aria-label="时间"
            className="space-y-1 text-sm leading-[22px]"
          >
            ...
            <dl className="grid grid-cols-1 gap-y-1">
              ...
              <Row
                ...
                value={
                  worker.finishedAt
                    ? formatDateTime(worker.finishedAt)
                    : '未结束'
                }
              />
              <Row
                label="最近观察"
                value={formatDateTime(worker.lastObservedAt)}
              />
            </dl>
            ...
            {worker.progress ? (
              <dl className="grid grid-cols-1 gap-y-1">
                ...
                <Row
                  label="来源"
                  value={
                    <code className="text-xs">{worker.progress.source}</code>
                  }
                />
                <Row
                  label="最近工具"
                  value={worker.progress.tool ?? '暂无数据'}
                />
              </dl>
            ) : (
          ...
          >
            ...
            {worker.audit ? (
              <dl className="grid grid-cols-1 gap-y-1">
                <Row
                  label="耗时"
                  value={`${formatDuration(worker.audit.elapsedMs)}（来源：${worker.audit.elapsedSource}）`}
                />
                <Row
                  label="等待"
                  value={`${formatDuration(worker.audit.waitingMs)}（来源：${worker.audit.waitingSource}）`}
                />
                {worker.audit.repairId ? (
                  <Row
                    label="局部修复"
                    value={
                      <code className="text-xs">{worker.audit.repairId}</code>
                    }
                  />
                ) : null}
              </dl>
            ...
            {worker.usage.source === 'unavailable' ? (
              ...
              <dl className="grid grid-cols-1 gap-y-1">
                <Row
                  label="Token"
                  value={
                    worker.usage.tokens !== null
                      ? String(worker.usage.tokens)
                      : '暂无数据'
                  }
                />
                <Row
                  label="费用"
                  value={
                    worker.usage.cost !== null
                      ? `${worker.usage.cost}${worker.usage.currency !== null ? ` ${worker.usage.currency}` : ''}`
                      : '暂无数据'
                  }
                />
                <Row
                  label="来源"
                  value={<code className="text-xs">{worker.usage.source}</code>}
                />
                <Row label="覆盖" value={String(worker.usage.coverage)} />
              </dl>
            )}
          </section>
          <code className="break-all text-xs">{worker.id}</code>{' '}
          <section
            aria-label="身份"
            className="space-y-1 text-sm leading-[22px]"
          >
            ...
            <dl className="grid grid-cols-1 gap-y-1">
              <Row
                label="节点"
                value={<code className="text-xs">{worker.nodeId}</code>}
              />
              <Row
                label="Provider"
                value={<code className="text-xs">{worker.providerId}</code>}
              />
              <Row label="尝试" value={String(worker.attempt)} />
              <Row
                label="所属任务"
                value={
                  <code className="break-all text-xs">{worker.taskId}</code>
                }
              />
            </dl>
          </section>
        ...
        >
          {terminal ? (
            <p className="text-sm text-text-secondary">
              该 Worker 已到达终态（{worker.status}），不能再取消。
            </p>
          ) : (
            <CancelWorkerFlow
