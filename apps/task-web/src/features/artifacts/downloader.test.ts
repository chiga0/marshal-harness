...
describe('下载保护（E16/E17 本机落盘与防假冒）', () => {
  ...
  it('摘要复验通过才保存（按 artifactId 拉取内容）', async () => {
    const clicked = stubSaving();
    const content = 'hello delivery';
    const getArtifactContent = vi.fn(async () => new Blob([content]));
    const transport = {getArtifactContent} as unknown as Transport;
    ...
    expect(getArtifactContent).toHaveBeenCalledWith('artifact-x');
  ...
  it('摘要不一致拒绝保存，不落成可信成果', async () => {
    const clicked = stubSaving();
    const transport = {getArtifactContent: vi.fn(async () => new Blob(['tampered']))} as unknown as Transport;
    await expect(downloadArtifact(transport, {
      artifactId: 'artifact-x', fileName: 'a.txt', expectedBytes: null, expectedDigest: `sha256:${'0'.repeat(64)}`,
  ...
  it('大小不一致拒绝保存', async () => {
    const clicked = stubSaving();
    const content = 'abcd';
    const transport = {getArtifactContent: vi.fn(async () => new Blob([content]))} as unknown as Transport;
    await expect(downloadArtifact(transport, {
      artifactId: 'artifact-x', fileName: 'a.txt', expectedBytes: 999, expectedDigest: await digestOf(content),
  ...
  it('摘要格式不可复验时拒绝保存（不冒充成果）', async () => {
    const clicked = stubSaving();
    const transport = {getArtifactContent: vi.fn(async () => new Blob(['x']))} as unknown as Transport;
    await expect(downloadArtifact(transport, {
      artifactId: 'artifact-x', fileName: 'a.txt', expectedBytes: null, expectedDigest: 'not-a-digest',
