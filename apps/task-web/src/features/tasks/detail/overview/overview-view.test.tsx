...
import {makeFakeTransport, makeLeader, makeLeaderRequest, makePlan, makePreapprovalQuestion, makeQuestions, makeTask, makeWorker} from '../shared/test-fakes';
...
describe('概览（P04/P05）：等待、进展、验收、计划', () => {
  it('聚齐三类待处理：预批准问题走 task.answer、Leader 请求走 leader.reply、计划走 approvePlan，入口互不串用', () => {
    ...
    wrap(<OverviewView task={task} plan={makePlan()} questions={questions} workers={[makeWorker()]} leader={leader} transport={transport} onChanged={() => {}} />);
  ...
  it('零问题零等待任务不制造问卷（E06）', () => {
    ...
    wrap(<OverviewView task={task} plan={null} questions={makeQuestions()} workers={[]} leader={makeLeader()} transport={transport} onChanged={() => {}} />);
  ...
  it('等待批准的计划只显示一次批准入口（approve 在 allowedActions 中）', () => {
    ...
    wrap(<OverviewView task={task} plan={makePlan()} questions={makeQuestions()} workers={[]} leader={makeLeader()} transport={transport} onChanged={() => {}} />);
  ...
  it('已答复的 Leader 请求不计入待处理，也不提供答复操作', () => {
    ...
    wrap(<OverviewView task={task} plan={null} questions={makeQuestions()} workers={[]} leader={leader} transport={transport} onChanged={() => {}} />);
    expect(screen.getByTestId('waiting-empty')).toBeInTheDocument();
    expect(screen.queryByTestId('leader-request-card')).toBeNull();
  });
  ...
  it('Leader 投影不可用时如实说明（不猜有/无待处理请求）', () => {
    const {transport} = makeFakeTransport();
    wrap(<OverviewView task={makeTask()} plan={null} questions={makeQuestions()} workers={[]} leader={null} transport={transport} onChanged={() => {}} />);
  ...
  it('问题投影未加载时如实说明', () => {
    const {transport} = makeFakeTransport();
    wrap(<OverviewView task={makeTask()} plan={null} questions={null} workers={[]} leader={makeLeader()} transport={transport} onChanged={() => {}} />);
