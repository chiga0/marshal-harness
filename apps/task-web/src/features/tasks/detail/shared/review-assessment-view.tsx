...
const policyTitles={scope:'范围与交付边界',facts:'事实与创作边界',effects:'操作效果与反馈',recovery:'数据变化与恢复'};
...
export function ReviewAssessmentView({assessment}:{assessment:Assessment}) {
 return <section aria-label="逐项文本评审" className="space-y-3" data-testid="review-assessment">
  ...
  <div className="divide-y divide-border">{assessment.criteria.map(criterion=>{
   ...
   return <details key={criterion.id} open={['fail','unknown'].includes(check.assessment)} className="workspace-disclosure group py-3" data-testid="review-assessment-item" data-criterion-id={criterion.id}>
    <summary className="!flex items-start gap-2 !list-none [&::-webkit-details-marker]:hidden">
     <svg aria-hidden="true" viewBox="0 0 16 16" className="mt-1 h-4 w-4 shrink-0 transition-transform group-open:rotate-90" fill="none" stroke="currentColor"><path d="m6 3 5 5-5 5"/></svg>
     <span className="min-w-0 flex-1 break-words">{criterion.index+1}. {criterion.policyId?policyTitles[criterion.policyId]:criterion.requirement}</span>
     <span className="shrink-0 rounded-full bg-surface-muted px-2 py-0.5 text-xs font-medium text-text-secondary">{labels[check.assessment]}</span>
    </summary>
    <div className="mt-3 space-y-3 text-sm leading-6">
     <div><p className="text-xs font-medium text-text-secondary">原验收要求</p><p data-testid="review-criterion-requirement" className="whitespace-pre-wrap break-words">{criterion.requirement}</p></div>
     <p data-testid="review-criterion-reason" className="whitespace-pre-wrap break-words">{check.reason}</p>
