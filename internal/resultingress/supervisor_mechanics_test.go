...
func testCommandEvidence(t *testing.T, session string, command processsupervisor.CommandName, currentHead string, outcome SupervisorProcessOutcome) SupervisorCommandEvidence {
	...
	if command == processsupervisor.CommandClose {
		evidence.TranscriptDisposition = processsupervisor.TranscriptNotRequired
		evidence.TranscriptResolutionFactDigest = attemptTestDigest("resolution-" + string(command))
	}
...
func TestSupervisorCloseCheckpointRequiresExactTerminalReport(t *testing.T) {
	...
	transition := AttemptTransition{SupervisorOutcomeFactDigest: checkpointDigest, SupervisorClosed: ProcessSupervisorClosed{CloseIntentDigest: closed.RequestDigest, CloseReceiptDigest: closed.ReceiptDigest, CloseObservationDigest: closed.ObservationDigest, FinalCommandHead: closed.CommandHead, TranscriptDisposition: closed.TranscriptDisposition, TranscriptResolutionFactDigest: closed.TranscriptResolutionFactDigest}}
