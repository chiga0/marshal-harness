...
const (
	...
	AttemptTransitionTranscriptNotAdmitted    AttemptTransitionKind = "process-supervisor-transcript-not-admitted"
	AttemptTransitionTranscriptNotRequired    AttemptTransitionKind = "process-supervisor-transcript-not-required"
...
type AttemptTransition struct {
	...
	TranscriptResolution            TranscriptResolutionBinding `json:"transcriptResolution,omitempty,omitzero"`
}

const transcriptResolutionSchema = "process-supervisor-transcript-resolution/v1"

// TranscriptResolutionBinding is the complete secret-free authority snapshot
// sealed by a non-admission resolution fact. Its digest is the surrounding
// RB1 envelope digest, never a caller-provided field in this payload.
type TranscriptResolutionBinding struct {
	SchemaVersion                  string                                  `json:"schemaVersion"`
	ProtocolRevision               string                                  `json:"protocolRevision"`
	Disposition                    processsupervisor.TranscriptDisposition `json:"disposition"`
	Owner                          CurrentOwnerBinding                     `json:"owner"`
	SessionID                      string                                  `json:"sessionId"`
	SupervisorBootstrapFactDigest  string                                  `json:"supervisorBootstrapFactDigest"`
	...
	Child                          processsupervisor.ProcessIdentity       `json:"child"`
	TerminalizationID              string                                  `json:"terminalizationId"`
	TerminalGeneration             uint64                                  `json:"terminalGeneration"`
	BarrierFactDigest              string                                  `json:"barrierFactDigest"`
	EligibilityTerminal            EligibilityTerminal                     `json:"eligibilityTerminal"`
	...
	CollectOutcomeFactDigest       string                                  `json:"collectOutcomeFactDigest,omitempty"`
	QuarantineFactDigest           string                                  `json:"quarantineFactDigest,omitempty"`
	QuarantineReason               RejectionReason                         `json:"quarantineReason,omitempty"`
	QuarantineDRCDigest            string                                  `json:"quarantineDrcDigest,omitempty"`
	QuarantineEnvelopeDigest       string                                  `json:"quarantineEnvelopeDigest,omitempty"`
}
...
type AttemptAuthorityState struct {
	...
	CommittedResultAuthorityDigest   string                                  `json:"committedResultAuthorityDigest,omitempty"`
	...
	TranscriptDisposition            processsupervisor.TranscriptDisposition `json:"transcriptDisposition,omitempty"`
	TranscriptResolutionFactDigest   string                                  `json:"transcriptResolutionFactDigest,omitempty"`
	TranscriptResolution             TranscriptResolutionBinding             `json:"transcriptResolution,omitempty,omitzero"`
	SupervisorClosed                 ProcessSupervisorClosed                 `json:"supervisorClosed,omitempty,omitzero"`
...
func (s *ingressDurableStore) CompareAndAppend(expectedRevision uint64, expectedHead string, transition AttemptTransition) (AttemptAppendResult, error) {
	...
	if transition.Kind == AttemptTransitionTranscriptNotAdmitted || transition.Kind == AttemptTransitionTranscriptNotRequired {
		return AttemptAppendResult{}, fmt.Errorf("%w: %s requires current control owner authority", ErrControlOwnerNotCurrent, transition.Kind)
	}
...
func transitionDigest(state AttemptAuthorityState, kind AttemptTransitionKind) string {
	...
	case AttemptTransitionTranscriptNotAdmitted, AttemptTransitionTranscriptNotRequired:
		return state.TranscriptResolutionFactDigest
...
func prepareAttemptFact(prior AttemptAuthorityState, exists bool, fact *attemptAuthorityFact, historicalReplay bool) error {
	...
	case AttemptTransitionTranscriptNotAdmitted, AttemptTransitionTranscriptNotRequired:
		if prior.TranscriptResolutionFactDigest != "" || prior.BarrierDigest == "" || prior.BarrierAdmissionFactDigest != "" || prior.CommittedResultFactDigest != "" || prior.ProcessTerminalDigest == "" || prior.AllocationTerminalDigest == "" || prior.SupervisorClosedDigest != "" || prior.CleanupCompletedDigest != "" || prior.CleanupReleasedDigest != "" || prior.SupervisorBootstrapDigest == "" || prior.SupervisorStartedDigest == "" || prior.EligibilityTerminal.Kind == EligibilityTerminalCompleted || prior.SupervisorPendingIntentDigest != "" || prior.PendingEffectIntentFactDigest != "" || prior.SupervisorInterventionDigest != "" || prior.EffectInterventionDigest != "" {
			return ErrAttemptAuthorityOrder
		}
		if !transcriptResolutionMatchesState(prior, t.TranscriptResolution) {
			return ErrAttemptAuthorityConflict
		}
	case AttemptTransitionProcessSupervisorClosed:
		closed := t.SupervisorClosed
		legacyTranscript := historicalReplay && prior.TranscriptResolutionFactDigest == "" && closed.TranscriptDisposition == "" && closed.TranscriptResolutionFactDigest == ""
		if prior.ProcessTerminalDigest == "" || prior.AllocationTerminalDigest == "" || prior.SupervisorStartedDigest == "" || prior.SupervisorClosedDigest != "" || !legacyTranscript && (prior.TranscriptResolutionFactDigest == "" || closed.TranscriptDisposition != prior.TranscriptDisposition || closed.TranscriptResolutionFactDigest != prior.TranscriptResolutionFactDigest) || closed.SupervisorStartedFactDigest != prior.SupervisorStartedDigest || closed.ProcessTerminalFactDigest != prior.ProcessTerminalDigest || closed.AllocationTerminatedFactDigest != prior.AllocationTerminalDigest || closed.CleanupBindingDigest != prior.CleanupBindingDigest || closed.TerminalizationID != prior.TerminalizationID || closed.SessionID != prior.SupervisorStarted.Handshake.SessionID || closed.SupervisorProcess != prior.SupervisorStarted.Handshake.SupervisorProcess || closed.Owner != prior.Owner {
			return ErrAttemptAuthorityOrder
		}
		newSupervisorBinding := t.SupervisorOutcomeFactDigest != ""
		if !historicalReplay && (prior.SupervisorBootstrapDigest == "" || !newSupervisorBinding) {
...
func validateTransitionShape(t AttemptTransition) error {
	...
	if t.Kind != AttemptTransitionTranscriptNotAdmitted && t.Kind != AttemptTransitionTranscriptNotRequired && t.TranscriptResolution != (TranscriptResolutionBinding{}) {
		return fmt.Errorf("%w: transcript resolution on unrelated transition", ErrAttemptAuthorityConflict)
	}
	...
	case AttemptTransitionTranscriptNotAdmitted, AttemptTransitionTranscriptNotRequired:
		want := processsupervisor.TranscriptCollectedNotAdmitted
		if t.Kind == AttemptTransitionTranscriptNotRequired {
			want = processsupervisor.TranscriptNotRequired
		}
		if t.TranscriptResolution.Disposition != want || t.TranscriptResolution.SchemaVersion != transcriptResolutionSchema || t.TranscriptResolution.ProtocolRevision != processsupervisor.ProtocolRevision || transitionHasAnyPayloadExceptTranscriptResolution(t) {
			return fmt.Errorf("%w: invalid transcript resolution transition", ErrAttemptAuthorityConflict)
		}
	case AttemptTransitionProcessSupervisorClosed:
...
func transitionHasAnyPayload(t AttemptTransition) bool {
	return transitionHasAnyPayloadExceptTranscriptResolution(t) || t.TranscriptResolution != (TranscriptResolutionBinding{})
}

func transitionHasAnyPayloadExceptTranscriptResolution(t AttemptTransition) bool {
	copy := t
	copy.TranscriptResolution = TranscriptResolutionBinding{}
	return copy.LaunchAuthorizationID != "" || copy.CommandID != "" || copy.ObservedAt != "" || copy.Process != (ProcessObservation{}) || copy.TerminalizationID != "" || copy.EligibilityTerminal != (EligibilityTerminal{}) || copy.ProcessTerminalKind != "" || copy.ObservationDigest != "" || copy.ReceiptDigest != "" || copy.AdmissionFactDigest != "" || copy.AdmissionSequence != 0 || !zeroLaunchClosure(copy.LaunchClosure) || copy.LaunchMaterialsDigest != "" || copy.AgentLaunchSpecDigest != "" || copy.Owner != (CurrentOwnerBinding{}) || copy.SupervisorBootstrap != (SupervisorBootstrapPrepared{}) || copy.SupervisorStarted != (ProcessSupervisorStarted{}) || copy.SupervisorBindOutcomeFactDigest != "" || copy.SupervisorOutcomeFactDigest != "" || !zeroSupervisorCommandEvidence(copy.SupervisorBindEvidence) || len(copy.SupervisorPrecedingEvidence) != 0 || !zeroSupervisorCommandEvidence(copy.SupervisorEvidence) || copy.SupervisorClosed != (ProcessSupervisorClosed{}) || copy.SupervisorIntervention != (SupervisorIntervention{}) || copy.SupervisorClosedFactDigest != ""
}
...
func validateSupervisorCommandIntentAgainstState(state AttemptAuthorityState, intent SupervisorCommandIntent) error {
	...
	case processsupervisor.CommandClose:
		legacyTranscript := state.TranscriptResolutionFactDigest == "" && rebuild.TranscriptDisposition == "" && rebuild.TranscriptResolutionFactDigest == ""
		if state.ProcessTerminalDigest == "" || state.AllocationTerminalDigest == "" || state.SupervisorClosedDigest != "" || !legacyTranscript && (state.TranscriptResolutionFactDigest == "" || rebuild.TranscriptDisposition != state.TranscriptDisposition || rebuild.TranscriptResolutionFactDigest != state.TranscriptResolutionFactDigest) || rebuild.ProcessTerminalFactDigest != state.ProcessTerminalDigest || rebuild.AllocationTerminatedFactDigest != state.AllocationTerminalDigest || rebuild.CleanupBindingDigest != state.CleanupBindingDigest {
			return ErrAttemptAuthorityOrder
		}
	default:
		return ErrAttemptAuthorityOrder
...
func validateSupervisorCommandOutcomeAgainstIntent(state AttemptAuthorityState, evidence SupervisorCommandEvidence) error {
	...
	case processsupervisor.CommandClose:
		if evidence.Outcome.State != SupervisorSessionClosed || !sameSupervisorChildEvidence(state.ProcessStartedEvidence, evidence) || evidence.TranscriptDisposition != intent.Rebuild.TranscriptDisposition || evidence.TranscriptResolutionFactDigest != intent.Rebuild.TranscriptResolutionFactDigest {
			return ErrAttemptAuthorityConflict
		}
	default:
		return ErrAttemptAuthorityConflict
...
func validateBusinessOutcomeReferenceAny(state AttemptAuthorityState, digest string, command processsupervisor.CommandName, outcome SupervisorProcessState) error {
	evidence, found := supervisorCheckpointEvidence(state, digest)
	if !found || evidence.Disposition != "ok" || command != "" && evidence.Command != command || outcome != "" && evidence.Outcome.State != outcome {
		return ErrAttemptAuthorityConflict
	}
	return nil
}

func transcriptResolutionMatchesState(state AttemptAuthorityState, resolution TranscriptResolutionBinding) bool {
	if resolution.SchemaVersion != transcriptResolutionSchema || resolution.ProtocolRevision != processsupervisor.ProtocolRevision || resolution.Disposition.Validate() != nil || resolution.Owner != state.Owner || resolution.SessionID != state.SupervisorStarted.Handshake.SessionID || resolution.SupervisorBootstrapFactDigest != state.SupervisorBootstrapDigest || resolution.SupervisorStartedFactDigest != state.SupervisorStartedDigest || resolution.ProcessStartedFactDigest != state.ProcessStartedDigest || resolution.Child != state.ProcessStartedEvidence.Outcome.Process || resolution.TerminalizationID != state.TerminalizationID || resolution.TerminalGeneration != uint64(state.TerminalGeneration) || resolution.BarrierFactDigest != state.BarrierDigest || resolution.EligibilityTerminal != state.EligibilityTerminal || resolution.ProcessTerminalFactDigest != state.ProcessTerminalDigest || resolution.AllocationTerminatedFactDigest != state.AllocationTerminalDigest || resolution.CleanupBindingDigest != state.CleanupBindingDigest || resolution.LastObservationDigest != supervisorLastObservation(state) {
		return false
	}
	switch resolution.Disposition {
	case processsupervisor.TranscriptCollectedNotAdmitted:
		if validateBusinessOutcomeReferenceAny(state, resolution.CollectOutcomeFactDigest, processsupervisor.CommandCollect, SupervisorTranscriptCollected) != nil || requireDigest("quarantineFactDigest", resolution.QuarantineFactDigest) != nil || requireDigest("quarantineDrcDigest", resolution.QuarantineDRCDigest) != nil || requireDigest("quarantineEnvelopeDigest", resolution.QuarantineEnvelopeDigest) != nil || resolution.QuarantineReason == "" {
			return false
		}
	case processsupervisor.TranscriptNotRequired:
		if resolution.CollectOutcomeFactDigest != "" || resolution.QuarantineFactDigest != "" || resolution.QuarantineReason != "" || resolution.QuarantineDRCDigest != "" || resolution.QuarantineEnvelopeDigest != "" {
			return false
		}
		for _, checkpoint := range state.SupervisorCommandCheckpoints {
			if checkpoint.Evidence.Command == processsupervisor.CommandCollect && checkpoint.Evidence.Disposition == "ok" {
				return false
			}
		}
	default:
...
func closedCheckpointMatches(state AttemptAuthorityState, transition AttemptTransition) bool {
	...
	return found && evidence.Command == processsupervisor.CommandClose && evidence.RequestDigest == closed.CloseIntentDigest && evidence.ReceiptDigest == closed.CloseReceiptDigest && evidence.ObservationDigest == closed.CloseObservationDigest && evidence.CommandHead == closed.FinalCommandHead && evidence.TranscriptDisposition == closed.TranscriptDisposition && evidence.TranscriptResolutionFactDigest == closed.TranscriptResolutionFactDigest && terminalReportsEquivalent(state.ProcessTerminalEvidence, evidence)
}
...
func exactTransitionReplay(state AttemptAuthorityState, exists bool, t AttemptTransition) (AttemptAuthorityState, bool) {
	...
	case AttemptTransitionTranscriptNotAdmitted, AttemptTransitionTranscriptNotRequired:
		return state, state.TranscriptResolutionFactDigest != "" && state.TranscriptResolution == t.TranscriptResolution
	case AttemptTransitionProcessSupervisorClosed:
...
func applyAttemptAuthorityFactValue(fact attemptAuthorityFact, in *Ingress, historicalReplay bool) error {
	...
	case attemptTransitionResultAdmitted:
		...
		state.CommittedResultAuthorityDigest = fact.Digest
		state.TranscriptDisposition = processsupervisor.TranscriptCollectedAdmitted
		state.TranscriptResolutionFactDigest = fact.Digest
	...
	case AttemptTransitionTranscriptNotAdmitted, AttemptTransitionTranscriptNotRequired:
		state.TranscriptDisposition = t.TranscriptResolution.Disposition
		state.TranscriptResolution = t.TranscriptResolution
		state.TranscriptResolutionFactDigest = fact.Digest
	case AttemptTransitionProcessTerminal:
