...
func (closed ProcessSupervisorClosed) Validate() error {
	...
	legacyTranscript := closed.TranscriptDisposition == "" && closed.TranscriptResolutionFactDigest == ""
	if !legacyTranscript && (closed.TranscriptDisposition.Validate() != nil || requireDigest("transcriptResolutionFactDigest", closed.TranscriptResolutionFactDigest) != nil) {
		return fmt.Errorf("%w: invalid transcript disposition", ErrAttemptAuthorityConflict)
	}
	...
	if !zeroSupervisorCommandEvidence(closed.Mechanics) {
		if closed.Mechanics.Validate() != nil || requireDigest("closeObservationDigest", closed.CloseObservationDigest) != nil || closed.Mechanics.Command != processsupervisor.CommandClose || closed.Mechanics.SessionID != closed.SessionID || closed.Mechanics.RequestDigest != closed.CloseIntentDigest || closed.Mechanics.ReceiptDigest != closed.CloseReceiptDigest || closed.Mechanics.CommandHead != closed.FinalCommandHead || closed.Mechanics.ObservationDigest != closed.CloseObservationDigest || closed.Mechanics.Outcome.State != SupervisorSessionClosed || closed.Mechanics.TranscriptDisposition != closed.TranscriptDisposition || closed.Mechanics.TranscriptResolutionFactDigest != closed.TranscriptResolutionFactDigest {
...
func NewProcessSupervisorClosedFromRecovery(authority ProcessSupervisorCloseAuthority, recovery processsupervisor.CommittedCloseRecoveryEvidence) (ProcessSupervisorClosed, error) {
	...
	closed := ProcessSupervisorClosed{
		...
		TranscriptDisposition: outcome.TranscriptDisposition, TranscriptResolutionFactDigest: outcome.TranscriptResolutionFactDigest,
...
func validateSupervisorTransitionAgainstProjection(in *Ingress, prior AttemptAuthorityState, exists bool, transition AttemptTransition, historicalReplay bool) error {
	...
	case AttemptTransitionTranscriptNotAdmitted, AttemptTransitionTranscriptNotRequired:
		if !exists || prior.Owner != transition.TranscriptResolution.Owner || transition.Identity.AuthorityNamespaceID != transition.TranscriptResolution.Owner.Scope.AuthorityNamespaceID {
			return ErrAttemptAuthorityConflict
		}
		if _, err := currentOwner(transition.TranscriptResolution.Owner); err != nil {
			return err
		}
		if !transcriptResolutionMatchesState(prior, transition.TranscriptResolution) {
			return ErrAttemptAuthorityConflict
		}
		if transition.Kind == AttemptTransitionTranscriptNotAdmitted {
			var exact bool
			for _, quarantine := range in.quarantine {
				if quarantine.FactDigest == transition.TranscriptResolution.QuarantineFactDigest && quarantine.AttemptKey != "" && quarantine.AttemptKey == transcriptAttemptKey(prior.Identity) && quarantine.EnvelopeKind == KindWorkerResult && quarantine.SupervisorOutcomeFactDigest == transition.TranscriptResolution.CollectOutcomeFactDigest && quarantine.Reason == transition.TranscriptResolution.QuarantineReason && quarantine.DRCDigest == transition.TranscriptResolution.QuarantineDRCDigest && quarantine.EnvelopeDigest == transition.TranscriptResolution.QuarantineEnvelopeDigest {
					exact = true
					break
				}
			}
...
func transcriptAttemptKey(identity AttemptIdentity) string {
	key, _ := identity.Key()
	return key
}

// TranscriptResolutionRequest selects one closed ADR 0061 non-admission
// branch. QuarantineFactDigest is only a reference selector; the append path
// reloads and binds the complete durable quarantine fact.
type TranscriptResolutionRequest struct {
	Disposition          processsupervisor.TranscriptDisposition
	QuarantineFactDigest string
}

// AppendSupervisorTranscriptResolution creates the sole non-admission
// resolution fact under the current owner lock and current Run authority. The
// returned TransitionDigest is the only value legal in ClosePayload.
func (s *ingressDurableStore) AppendSupervisorTranscriptResolution(ctx context.Context, ownerVerifier CurrentOwnerLockVerifier, runVerifier CurrentRunAuthorityVerifier, expectedRevision uint64, expectedHead string, request AttemptAuthorizationRequest, owner CurrentOwnerBinding, resolution TranscriptResolutionRequest) (AttemptAppendResult, error) {
	if request.Identity.AuthorityNamespaceID != owner.Scope.AuthorityNamespaceID || request.CurrentRunAuthority != runAuthorityBindingFor(request.Identity) || resolution.Disposition != processsupervisor.TranscriptCollectedNotAdmitted && resolution.Disposition != processsupervisor.TranscriptNotRequired {
	...
	err := s.WithCurrentOwner(ctx, ownerVerifier, owner, func(ControlOwnerState) error {
		return withCurrentRunAuthority(ctx, runVerifier, request.CurrentRunAuthority, func() error {
			...
			if !found || state.Identity != request.Identity || state.Owner != owner {
				return ErrAttemptAuthorityConflict
			}
			if state.TranscriptResolutionFactDigest != "" {
				if state.TranscriptDisposition != resolution.Disposition || resolution.Disposition == processsupervisor.TranscriptCollectedNotAdmitted && state.TranscriptResolution.QuarantineFactDigest != resolution.QuarantineFactDigest || resolution.Disposition == processsupervisor.TranscriptNotRequired && resolution.QuarantineFactDigest != "" {
					return ErrAttemptAuthorityConflict
				}
				kind := AttemptTransitionTranscriptNotRequired
				if resolution.Disposition == processsupervisor.TranscriptCollectedNotAdmitted {
					kind = AttemptTransitionTranscriptNotAdmitted
				}
				transition := AttemptTransition{Kind: kind, Identity: request.Identity, TranscriptResolution: state.TranscriptResolution}
				var appendErr error
				result, appendErr = s.compareAndAppendWithOwner(expectedRevision, expectedHead, transition, false, true)
				return appendErr
			}
			if state.Revision != expectedRevision || state.HeadDigest != expectedHead {
				return ErrAttemptAuthorityConflict
			}
			binding := TranscriptResolutionBinding{
				SchemaVersion: transcriptResolutionSchema, ProtocolRevision: processsupervisor.ProtocolRevision, Disposition: resolution.Disposition, Owner: owner,
				SessionID: state.SupervisorStarted.Handshake.SessionID, SupervisorBootstrapFactDigest: state.SupervisorBootstrapDigest, SupervisorStartedFactDigest: state.SupervisorStartedDigest, ProcessStartedFactDigest: state.ProcessStartedDigest, Child: state.ProcessStartedEvidence.Outcome.Process,
				TerminalizationID: state.TerminalizationID, TerminalGeneration: uint64(state.TerminalGeneration), BarrierFactDigest: state.BarrierDigest, EligibilityTerminal: state.EligibilityTerminal, ProcessTerminalFactDigest: state.ProcessTerminalDigest, AllocationTerminatedFactDigest: state.AllocationTerminalDigest, CleanupBindingDigest: state.CleanupBindingDigest, LastObservationDigest: supervisorLastObservation(state),
			}
			kind := AttemptTransitionTranscriptNotRequired
			if resolution.Disposition == processsupervisor.TranscriptCollectedNotAdmitted {
				kind = AttemptTransitionTranscriptNotAdmitted
				var matched *QuarantineRecord
				for index := range projection.quarantine {
					candidate := &projection.quarantine[index]
					if candidate.FactDigest == resolution.QuarantineFactDigest {
						matched = candidate
						break
					}
				}
				if matched == nil || matched.AttemptKey != transcriptAttemptKey(state.Identity) || matched.EnvelopeKind != KindWorkerResult {
					return ErrAttemptAuthorityConflict
				}
				binding.CollectOutcomeFactDigest, binding.QuarantineFactDigest = matched.SupervisorOutcomeFactDigest, matched.FactDigest
				binding.QuarantineReason, binding.QuarantineDRCDigest, binding.QuarantineEnvelopeDigest = matched.Reason, matched.DRCDigest, matched.EnvelopeDigest
			} else if resolution.QuarantineFactDigest != "" {
				return ErrAttemptAuthorityConflict
			}
			transition := AttemptTransition{Kind: kind, Identity: request.Identity, TranscriptResolution: binding}
			var appendErr error
			result, appendErr = s.compareAndAppendWithOwner(expectedRevision, expectedHead, transition, false, true)
			return appendErr
		})
...
func (s *ingressDurableStore) AppendSupervisorCommandIntent(ctx context.Context, ownerVerifier CurrentOwnerLockVerifier, runVerifier CurrentRunAuthorityVerifier, expectedRevision uint64, expectedHead string, request AttemptAuthorizationRequest, owner CurrentOwnerBinding, intent SupervisorCommandIntent) (AttemptAppendResult, error) {
	...
	if processsupervisor.ValidatePreparedCommandProjection(intent.Command, intent.Rebuild) != nil {
		return AttemptAppendResult{}, ErrAttemptAuthorityConflict
	}
	var result AttemptAppendResult
	err := s.WithCurrentOwner(ctx, ownerVerifier, owner, func(ControlOwnerState) error {
		return withCurrentRunAuthority(ctx, runVerifier, request.CurrentRunAuthority, func() error {
...
func (s *ingressDurableStore) AppendSupervisorCommandOutcome(ctx context.Context, ownerVerifier CurrentOwnerLockVerifier, runVerifier CurrentRunAuthorityVerifier, expectedRevision uint64, expectedHead string, request AttemptAuthorizationRequest, owner CurrentOwnerBinding, outcome SupervisorCommandEvidence) (AttemptAppendResult, error) {
	...
	if outcome.Command == processsupervisor.CommandClose && (outcome.TranscriptDisposition.Validate() != nil || requireDigest("transcriptResolutionFactDigest", outcome.TranscriptResolutionFactDigest) != nil) {
		return AttemptAppendResult{}, ErrAttemptAuthorityConflict
	}
	var result AttemptAppendResult
	err := s.WithCurrentOwner(ctx, ownerVerifier, owner, func(ControlOwnerState) error {
...
func (s *ingressDurableStore) AppendSupervisorClosed(ctx context.Context, ownerVerifier CurrentOwnerLockVerifier, runVerifier CurrentRunAuthorityVerifier, expectedRevision uint64, expectedHead string, request CleanupAuthorizationRequest, closed ProcessSupervisorClosed, outcomeFactDigest string) (AttemptAppendResult, error) {
	...
	if closed.TranscriptDisposition.Validate() != nil || requireDigest("transcriptResolutionFactDigest", closed.TranscriptResolutionFactDigest) != nil {
		return AttemptAppendResult{}, ErrAttemptAuthorityConflict
	}
