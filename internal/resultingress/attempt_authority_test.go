...
func appendTestSupervisorClosed(t *testing.T, store *DurableStore, state AttemptAuthorityState, request CleanupAuthorizationRequest) AttemptAuthorityState {
	...
	if state.TranscriptResolutionFactDigest == "" {
		ownerState, found, err := store.OpenOwner(state.Owner.Scope)
		if err != nil || !found {
			t.Fatalf("current owner found=%v err=%v", found, err)
		}
		resolved, err := store.AppendSupervisorTranscriptResolution(context.Background(), attemptOwnerVerifier{want: ownerState.Acquisition}, attemptRunVerifier{want: request.CurrentRunAuthority}, state.Revision, state.HeadDigest, AttemptAuthorizationRequest{Identity: state.Identity, CurrentRunAuthority: request.CurrentRunAuthority}, state.Owner, TranscriptResolutionRequest{Disposition: processsupervisor.TranscriptNotRequired})
		if err != nil {
			t.Fatal(err)
		}
		state = appendTestSupervisorReconnect(t, store, resolved.State)
	}
	...
	closeIntent := testSupervisorIntent(state, processsupervisor.CommandClose, SupervisorCommandRebuildProjection{
		...
		TranscriptDisposition: state.TranscriptDisposition, TranscriptResolutionFactDigest: state.TranscriptResolutionFactDigest,
	})
	...
	closed := ProcessSupervisorClosed{
		...
		TranscriptDisposition:              state.TranscriptDisposition,
		TranscriptResolutionFactDigest:     state.TranscriptResolutionFactDigest,
...
func testSupervisorEvidence(t *testing.T, intent SupervisorCommandIntent, outcome SupervisorProcessOutcome, disposition string) SupervisorCommandEvidence {
	...
	evidence := SupervisorCommandEvidence{
		...
		TranscriptDisposition:          intent.Rebuild.TranscriptDisposition,
		TranscriptResolutionFactDigest: intent.Rebuild.TranscriptResolutionFactDigest,
	}
...
func appendTestBarrier(t *testing.T, store *DurableStore, state AttemptAuthorityState, terminalizationID string, reason TerminalReason) AttemptAppendResult {
	t.Helper()
	run := attemptTestRunAuthority(state.Identity)
	terminal := EligibilityTerminal{Kind: EligibilityTerminalCompleted, CompletionReason: reason}
	if reason != TerminalAttemptCompleted {
		terminal = EligibilityTerminal{Kind: EligibilityTerminalCancelled, CancelReason: EligibilityCancelDeadlineExceeded}
	}
