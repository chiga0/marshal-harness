...
type QuarantineRecord struct {
	...
	SupervisorOutcomeFactDigest string
}
...
func (i *Ingress) admitLocked(_ context.Context, drc DRC, envelope ResultEnvelope, collect SupervisorCommandEvidence, outcomeFactDigest string) (AdmissionFact, error) {
	...
	if governed {
		...
		if authorityState.BarrierDigest != "" {
			if envelope.Kind == KindWorkerResult && authorityState.SupervisorBootstrapDigest != "" && validateBusinessOutcomeReferenceAny(authorityState, outcomeFactDigest, processsupervisor.CommandCollect, SupervisorTranscriptCollected) == nil && zeroSupervisorCommandEvidence(collect) {
				i.recordSupervisorQuarantine(ReasonStaleLease, drcDigest, envelope, now, authorityKey, outcomeFactDigest)
			} else {
				i.recordQuarantine(ReasonStaleLease, drcDigest, envelope.ResultDigest, now)
			}
	...
	if governed {
		...
		if envelope.Kind == KindWorkerResult {
			...
			authorityState.CommittedResultAuthorityDigest = authorityHead
			authorityState.TranscriptDisposition = processsupervisor.TranscriptCollectedAdmitted
			authorityState.TranscriptResolutionFactDigest = authorityHead
...
func (i *Ingress) recordQuarantine(reason RejectionReason, drcDigest, envelopeDigest string, at time.Time) {
	i.recordQuarantineRecord(QuarantineRecord{
		Reason:         reason,
		DRCDigest:      drcDigest,
...
func (i *Ingress) recordSupervisorQuarantine(reason RejectionReason, drcDigest string, envelope ResultEnvelope, at time.Time, attemptKey, outcomeFactDigest string) {
	i.recordQuarantineRecord(QuarantineRecord{Reason: reason, DRCDigest: drcDigest, EnvelopeDigest: envelope.ResultDigest, ObservedAt: at, AttemptKey: attemptKey, EnvelopeKind: envelope.Kind, SupervisorOutcomeFactDigest: outcomeFactDigest})
}

func (i *Ingress) recordQuarantineRecord(record QuarantineRecord) {
	if i.store != nil {
		// 写出器由调用方（Admit）持锁；落账失败会让准确整体 admission reject，
		// 见调用路径的处理。best-effort 记录，绝不静默丢弃失败。
		digest, _ := i.store.recordQuarantinedLocked(record)
		record.FactDigest = digest
	}
	i.quarantine = append(i.quarantine, record)
}
