...
type resultQuarantinedFact struct {
	...
	AttemptKey                  string          `json:"attemptKey,omitempty"`
	EnvelopeKind                EnvelopeKind    `json:"envelopeKind,omitempty"`
	SupervisorOutcomeFactDigest string          `json:"supervisorOutcomeFactDigest,omitempty"`
	Digest                      string          `json:"digest"`
}
...
func (s *ingressDurableStore) RecordQuarantined(reason RejectionReason, drcDigest, envelopeDigest string, observedAt time.Time) error {
	return s.withExclusive(func() error {
		...
		_, err := s.recordQuarantinedLocked(QuarantineRecord{Reason: reason, DRCDigest: drcDigest, EnvelopeDigest: envelopeDigest, ObservedAt: observedAt})
		return err
	})
}

func (s *ingressDurableStore) recordQuarantinedLocked(record QuarantineRecord) (string, error) {
	fact := &resultQuarantinedFact{
		ProtocolRevision: resultIngressProtocolRevision,
		FactType:         resultFactTypeQuarantined,
		Sequence:         s.nextSequence,
		Reason:           record.Reason,
		DRCDigest:        record.DRCDigest,
		EnvelopeDigest:   record.EnvelopeDigest,
		ObservedAt:       record.ObservedAt.UTC().Format(time.RFC3339),
		AttemptKey:       record.AttemptKey, EnvelopeKind: record.EnvelopeKind, SupervisorOutcomeFactDigest: record.SupervisorOutcomeFactDigest,
	}
	if err := s.appendLine(fact,
		func() string { return fact.Digest },
		func(d string) { fact.Digest = d }); err != nil {
...
func (s *ingressDurableStore) applyLine(line []byte, in *Ingress) error {
	...
	case resultFactTypeAdmitted:
		...
		if fact.AttemptKey != "" {
			...
			if fact.EnvelopeKind == KindWorkerResult {
				...
				state.CommittedResultAuthorityDigest = storeddigest
				state.TranscriptDisposition = processsupervisor.TranscriptCollectedAdmitted
				state.TranscriptResolutionFactDigest = storeddigest
	...
	case resultFactTypeQuarantined:
		...
		fact.Digest = storeddigest
		hasSupervisorBinding := fact.AttemptKey != "" || fact.EnvelopeKind != "" || fact.SupervisorOutcomeFactDigest != ""
		if hasSupervisorBinding {
			if requireDigest("attemptKey", fact.AttemptKey) != nil || fact.EnvelopeKind != KindWorkerResult || requireDigest("supervisorOutcomeFactDigest", fact.SupervisorOutcomeFactDigest) != nil || requireDigest("drcDigest", fact.DRCDigest) != nil || requireDigest("envelopeDigest", fact.EnvelopeDigest) != nil {
				return ErrAttemptAuthorityConflict
			}
		} else if fact.AttemptKey != "" || fact.EnvelopeKind != "" || fact.SupervisorOutcomeFactDigest != "" {
			return ErrAttemptAuthorityConflict
		}
		...
		in.quarantine = append(in.quarantine, QuarantineRecord{
			...
			EnvelopeDigest: fact.EnvelopeDigest,
			ObservedAt:     at.UTC(),
			FactDigest:     storeddigest, AttemptKey: fact.AttemptKey, EnvelopeKind: fact.EnvelopeKind, SupervisorOutcomeFactDigest: fact.SupervisorOutcomeFactDigest,
		})
	default:
