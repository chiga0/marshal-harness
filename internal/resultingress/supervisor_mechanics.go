...
func (e SupervisorCommandEvidence) Validate() error {
	...
	if e.Command == processsupervisor.CommandClose {
		legacy := e.TranscriptDisposition == "" && e.TranscriptResolutionFactDigest == ""
		if !legacy && (e.TranscriptDisposition.Validate() != nil || requireDigest("transcriptResolutionFactDigest", e.TranscriptResolutionFactDigest) != nil) {
			return fmt.Errorf("%w: invalid close transcript resolution", ErrAttemptAuthorityConflict)
		}
	} else if e.TranscriptDisposition != "" || e.TranscriptResolutionFactDigest != "" {
		return fmt.Errorf("%w: transcript resolution on unrelated command", ErrAttemptAuthorityConflict)
	}
...
func (e SupervisorCommandEvidence) boundMechanicsDigests() (string, string, error) {
	if e.Disposition == "rejected" {
		...
		if e.Command == processsupervisor.CommandClose {
			result.TranscriptDisposition, result.TranscriptResolutionFactDigest = e.TranscriptDisposition, e.TranscriptResolutionFactDigest
		}
...
func NewSupervisorCommandEvidence(outcome processsupervisor.VerifiedCommandOutcome) (SupervisorCommandEvidence, error) {
	...
	evidence := SupervisorCommandEvidence{
		...
		TranscriptDisposition:          outcome.TranscriptDisposition,
		TranscriptResolutionFactDigest: outcome.TranscriptResolutionFactDigest,
...
func validateSupervisorRebuildProjection(command processsupervisor.CommandName, projection SupervisorCommandRebuildProjection) error {
	if processsupervisor.ValidateStoredPreparedCommandProjection(command, projection) != nil {
		return ErrAttemptAuthorityConflict
	}
	return nil
}
