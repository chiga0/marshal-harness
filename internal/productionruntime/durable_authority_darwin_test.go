...
// TestRehydratePreparedRunStartMatchesAdapterSuppliedOnPathB runs the same
// durable-identity contract over the sealed CLI's path-B composition
// (existing-worktree bind): the Runtime's PrepareRunStart output and the
// authority's RehydratePreparedRunStart replay must be identical. Any
// divergence here reproduces the sealed `StartPreparedRun`
// `application: authority-conflict` blocked dogfood evidence (#226).
func TestRehydratePreparedRunStartMatchesAdapterSuppliedOnPathB(t *testing.T) {
	...
	composed, err := ComposeRuntime(context.Background(), inputs, profile)
	if err != nil {
		t.Fatalf("compose sealed path B: %v", err)
	}
	defer func() { _ = composed.Runtime.Close() }()
	projection, err := inputs.Runs.ReadRunStartAuthorityUnderLease(context.Background(), inputs.RunLease)
	if err != nil {
	...
	ctrl := composed.Runtime.controller
	if ctrl == nil {
		t.Fatal("runtime controller missing after sealed path-B compose")
	}
	var durable application.PreparedRunStart
	if err := ctrl.withOwner(context.Background(), true, func(verifier resultingress.CurrentOwnerLockVerifier, _ OwnerProjection) error {
		var herr error
	...
	if durable != prepared {
		t.Fatalf("REPRODUCED path-B durable-projection mismatch:\ndurable=%+v\nsupplied=%+v", durable, prepared)
	}
}
