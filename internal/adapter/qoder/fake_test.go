...
func TestRunPassesFrozenArgvToWorker(t *testing.T) {
	...
	want := []string{"--print", "--output-format", "jsonl", "--permission-mode", "accept_edits", "--no-session-persistence", "--config-dir", filepath.Join(resolved, "config", "qoder"), "--setting-sources", "", "--model", "provider/model", "完成 fixture"}
