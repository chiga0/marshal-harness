...
func (authority *candidateReceiptAuthorityFixture) IssueExecutionReceipt(ctx context.Context, request CandidateReceiptRequest) ([]byte, error) {
	...
	receipt := CandidateExecutionReceipt{
		...
		SessionID:      request.SessionID, ModelID: request.ObservedModel, ProtocolVersion: request.ProtocolVersion, PermissionMode: request.PermissionMode, EventContract: conformanceEventContract, WorkerResultTransportDigest: expectedWorkerResultTransportDigest(), TranscriptDigest: request.TranscriptDigest, MarkerDigest: request.MarkerDigest, StartedAt: candidateExactTimestamp(startedAt), CompletedAt: candidateExactTimestamp(completedAt), ReceiptAuthorityKeyID: authority.identity.KeyID, ReceiptAuthorityKeyEpoch: authority.identity.KeyEpoch, SignatureAlgorithm: candidateSignatureAlgorithm, SignatureEncoding: candidateSignatureEncoding,
	}
