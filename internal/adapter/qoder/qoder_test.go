...
func TestProbeIsFailClosedUntilConformance(t *testing.T) {
	for _, version := range []string{supportedBinary, "1.1.24", "9.9.9"} {
		t.Run(version, func(t *testing.T) {
			...
			if version != supportedBinary {
				found := false
				for _, item := range probeErrors {
					message := item.(string)
					if strings.Contains(message, version) && strings.Contains(message, supportedBinary) {
						found = true
					}
				}
				if !found {
...
func TestBuildArgsFreezesRealNonInteractiveArgv(t *testing.T) {
	...
	want := []string{"--print", "--output-format", "jsonl", "--permission-mode", "accept_edits", "--no-session-persistence", "--config-dir", "/managed/config", "--setting-sources", "", "--model", "provider/model", "完成任务"}
...
func TestBuildArgsRejectsFabricatedRunSandboxArgv(t *testing.T) {
	...
	for _, want := range []string{"--print", "--output-format", "jsonl", "--permission-mode", "accept_edits", "--no-session-persistence", "--config-dir", "--setting-sources"} {
...
func TestManagedConfigDirRejectsSymlinkAndEscape(t *testing.T) {
	...
	t.Run("world-writable-existing", func(t *testing.T) {
		...
		// managedConfigDir normalizes loose permissions to 0o700.
		configDir, err := managedConfigDir(controlRoot)
		if err != nil {
			t.Fatal(err)
		}
		...
		if err != nil || info.Mode().Perm() != 0o700 {
			t.Fatalf("loose config dir was not normalized: %v", err)
		}
	})
}
