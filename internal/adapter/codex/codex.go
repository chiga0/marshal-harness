...
// supportedBinaries 是本 Adapter 支持的 Codex CLI 版本精确闭集；
// 闭集外的任何版本一律 fail closed，不接受范围或前后兼容猜测。
var supportedBinaries = []string{"0.145.0"}

// isSupportedBinary 报告探测到的版本是否属于支持闭集。
...
// `codex-cli <semver>`。其他任何格式都不构成版本证据。
...
func (a *Adapter) Probe(ctx context.Context) (domain.Record, error) {
	...
	if !isSupportedBinary(identity.version) {
		status = "unsupported"
		probeErrors = append(probeErrors, fmt.Sprintf("仅支持 Codex CLI %s，实际为 %s", strings.Join(supportedBinaries, "、"), identity.version))
	}
