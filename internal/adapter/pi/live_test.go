...
// TestLivePi is opt-in because it consumes the locally configured model
// provider. It never commits, pushes, publishes, or touches the source repo.
// Pi only supports ephemeral sessions; persist/resume fail closed in Run.
func TestLivePi(t *testing.T) {
	executable := os.Getenv("MARSHAL_LIVE_PI_PATH")
	if executable == "" {
		t.Skip("set MARSHAL_LIVE_PI_PATH to run the live adapter E2E")
	}
...
{"apiVersion":"marshal.dev/v1alpha1","kind":"WorkerResult","taskId":"TASK-LIVE","runId":"run-live","attemptId":"attempt-live","adapter":{"id":"pi","executable":"declared","version":"declared"},"status":"completed","summary":"created hello.txt","declaredChangedFiles":["hello.txt"],"declaredArtifacts":[],"declaredCommands":[],"declaredRisks":[],"startedAt":"2026-08-04T00:00:00Z","completedAt":"2026-08-04T00:00:01Z"}`
	...
	request := map[string]any{
		...
		"adapterId": "pi", "executionProfile": "workspace-write", "sessionPolicy": "ephemeral", "attemptTimeoutSeconds": 180, "maxOutputBytes": 5 << 20, "reviewFindings": []any{},
	}
	...
	if err != nil {
		transcript, _ := os.ReadFile(filepath.Join(controlRoot, "output", "pi-transcript.jsonl"))
		metadata, _ := os.ReadFile(filepath.Join(controlRoot, "output", "pi-transcript-meta.json"))
