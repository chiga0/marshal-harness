...
func buildProductionPrompt(in ProductionLaunchInput) string {
	...
	b.WriteString("- Replace summary truthfully and set outputTruncated truthfully.\n")
	b.WriteString("- declaredChangedFiles is a unique array of relative-path strings for files actually changed.\n")
	b.WriteString("- declaredArtifacts is [] unless the objective explicitly requests a named artifact; each artifact must be an object with id, kind, and exactly one relative path or URI. An ordinary changed file is not automatically an artifact.\n")
	b.WriteString("- declaredCommands is [] or an array of objects with commandId, status (passed, failed, not-run, or unknown), and optional summary.\n")
	b.WriteString("- declaredRisks is [] or an array of non-empty strings.\n")
