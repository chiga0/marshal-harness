...
// TestLiveOpenCode is opt-in because it consumes the locally configured model
// provider. It never commits, pushes, publishes, or touches the source repo.
func TestLiveOpenCode(t *testing.T) {
	executable := os.Getenv("MARSHAL_LIVE_OPENCODE_PATH")
	if executable == "" {
		t.Skip("set MARSHAL_LIVE_OPENCODE_PATH to run the live adapter E2E")
	}
...
{"apiVersion":"marshal.dev/v1alpha1","kind":"WorkerResult","taskId":"TASK-LIVE","runId":"run-live","attemptId":"attempt-live","adapter":{"id":"opencode","executable":"declared","version":"declared"},"status":"completed","summary":"created hello.txt","declaredChangedFiles":["hello.txt"],"declaredArtifacts":[],"declaredCommands":[],"declaredRisks":[],"startedAt":"2026-08-04T00:00:00Z","completedAt":"2026-08-04T00:00:01Z"}`
	...
	if err != nil {
		transcript, _ := os.ReadFile(filepath.Join(controlRoot, "output", "opencode-transcript.jsonl"))
		metadata, _ := os.ReadFile(filepath.Join(controlRoot, "output", "opencode-transcript-meta.json"))
...
// the real host binary through MARSHAL_LIVE_OPENCODE_PATH first and PATH
...
func liveProbeGate(t *testing.T) {
	...
	if executable == "" {
		...
		if err != nil {
			t.Skip("set MARSHAL_LIVE_OPENCODE_PATH or install opencode on PATH to run the live probe")
		}
		executable = resolved
	}
...
func TestLiveProbeGateExemption(t *testing.T) {
	...
	runGate := func(t *testing.T, envValue, binary string) (skipped, failed bool) {
		t.Helper()
		t.Setenv("MARSHAL_LIVE_OPENCODE_PATH", binary)
	...
	t.Run("exemption-never-masks-non-version-failures", func(t *testing.T) {
		...
		// OpenCode's readBinaryVersion only rejects an empty version output,
		// so an empty --version print is this adapter's unparseable version.
		if _, err := probeCapabilitySnapshot(fakeExecutable(t, "", "exit 0")); err == nil || !strings.Contains(err.Error(), "empty version") {
	...
	t.Run("exemption-never-widens-closed-supported-set", func(t *testing.T) {
		...
		for _, version := range []string{"1.18.12", "1.19.0", "9.9.9"} {
			if isSupportedBinary(version) {
