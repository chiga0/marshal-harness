...
// TranscriptDisposition is the closed Close-time transcript resolution union
// frozen by ADR 0061. It is business-authority evidence supplied by Core;
// process-supervisor only enforces the corresponding local mechanics state.
type TranscriptDisposition string

const (
	TranscriptCollectedAdmitted    TranscriptDisposition = "collected-admitted"
	TranscriptCollectedNotAdmitted TranscriptDisposition = "collected-not-admitted"
	TranscriptNotRequired          TranscriptDisposition = "not-required"
)

func (disposition TranscriptDisposition) Validate() error {
	switch disposition {
	case TranscriptCollectedAdmitted, TranscriptCollectedNotAdmitted, TranscriptNotRequired:
		return nil
	default:
		return ErrInvalid
	}
...
type ClosePayload struct {
	...
	CleanupBindingDigest           string                `json:"cleanupBindingDigest"`
	TranscriptDisposition          TranscriptDisposition `json:"transcriptDisposition"`
	TranscriptResolutionFactDigest string                `json:"transcriptResolutionFactDigest"`
}

func validateClosePayload(payload ClosePayload) error {
	if !validDigest(payload.ProcessTerminalFactDigest) || !validDigest(payload.AllocationTerminatedDigest) || !validDigest(payload.CleanupBindingDigest) || payload.TranscriptDisposition.Validate() != nil || !validDigest(payload.TranscriptResolutionFactDigest) {
		return ErrInvalid
	}
	return nil
}
