...
type PreparedCommandProjection struct {
	...
	TranscriptDisposition          TranscriptDisposition `json:"transcriptDisposition,omitempty"`
...
func projectPreparedPayload(command CommandName, payload any) (PreparedCommandProjection, error) {
	...
	case ClosePayload:
		...
		projection.TranscriptDisposition, projection.TranscriptResolutionFactDigest = value.TranscriptDisposition, value.TranscriptResolutionFactDigest
	default:
		return projection, ErrInvalid
	}
...
func validatePreparedProjection(command CommandName, projection PreparedCommandProjection) error {
	...
	case CommandClose:
		if !validDigest(projection.ProcessTerminalFactDigest) || !validDigest(projection.AllocationTerminatedFactDigest) || !validDigest(projection.CleanupBindingDigest) || projection.TranscriptDisposition.Validate() != nil || !validDigest(projection.TranscriptResolutionFactDigest) {
			return ErrInvalid
		}
		...
		allowed.TranscriptDisposition, allowed.TranscriptResolutionFactDigest = projection.TranscriptDisposition, projection.TranscriptResolutionFactDigest
	default:
		return ErrInvalid
	}
...
// ValidateStoredPreparedCommandProjection admits the exact pre-ADR0061 Close
// shape only for append-only historical replay. New PreparedCommand evidence
// and wire requests continue to require the closed union.
func ValidateStoredPreparedCommandProjection(command CommandName, projection PreparedCommandProjection) error {
	if command == CommandClose && projection.TranscriptDisposition == "" && projection.TranscriptResolutionFactDigest == "" {
		...
		allowed := PreparedCommandProjection{
			ProcessTerminalFactDigest:      projection.ProcessTerminalFactDigest,
			AllocationTerminatedFactDigest: projection.AllocationTerminatedFactDigest,
			CleanupBindingDigest:           projection.CleanupBindingDigest,
		}
		original, err := canonicalValue(projection)
		want, wantErr := canonicalValue(allowed)
		if err != nil || wantErr != nil || !bytes.Equal(original, want) {
