...
func committedCloseFixture(t *testing.T) (*Journal, string, PreparedCommand, VerifiedCommandOutcome) {
	...
	payload := ClosePayload{ProcessTerminalFactDigest: digest("1"), AllocationTerminatedDigest: digest("2"), CleanupBindingDigest: digest("3"), TranscriptDisposition: TranscriptCollectedAdmitted, TranscriptResolutionFactDigest: digest("4")}
	...
	result := MechanicsResult{Disposition: "ok", ReasonCode: "mechanics-closed", ObservationDigest: mustDigestValue(report), TranscriptDisposition: payload.TranscriptDisposition, TranscriptResolutionFactDigest: payload.TranscriptResolutionFactDigest, Payload: mustCanonical(report)}
...
func TestRecoverCommittedCloseFailsClosedOnTornTrailingOrDifferentIntent(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "different-command", mutate: func(t *testing.T, _ *Journal, _ string, prepared PreparedCommand) PreparedCommand {
			payload := ClosePayload{ProcessTerminalFactDigest: digest("1"), AllocationTerminatedDigest: digest("2"), CleanupBindingDigest: digest("4"), TranscriptDisposition: TranscriptCollectedAdmitted, TranscriptResolutionFactDigest: digest("5")}
...
func TestRecoverCommittedCloseRejectsIntentWithoutReceipt(t *testing.T) {
	...
	pre := productionTestAnchor()
	pre.JournalSequence, pre.JournalHead = snapshot.Sequence, snapshot.Head
	payload := ClosePayload{ProcessTerminalFactDigest: digest("1"), AllocationTerminatedDigest: digest("2"), CleanupBindingDigest: digest("3"), TranscriptDisposition: TranscriptNotRequired, TranscriptResolutionFactDigest: digest("4")}
