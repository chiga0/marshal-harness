...
type VerifiedCommandOutcome struct {
	...
	TranscriptResolutionFactDigest string
...
func verifiedCommandOutcome(request Request, response Response, recovery CommandRecoveryEvidence) (VerifiedCommandOutcome, error) {
	...
	if request.Command == CommandClose {
		_, payload, projectErr := projectRequest(request)
		closePayload, ok := payload.(ClosePayload)
		if projectErr != nil || !ok || result.TranscriptDisposition != closePayload.TranscriptDisposition || result.TranscriptResolutionFactDigest != closePayload.TranscriptResolutionFactDigest {
			return VerifiedCommandOutcome{}, ErrConflict
		}
	} else if result.TranscriptDisposition != "" || result.TranscriptResolutionFactDigest != "" {
		return VerifiedCommandOutcome{}, ErrConflict
	}
	...
	if request.Command == CommandClose {
		...
		outcome.TranscriptDisposition = closePayload.TranscriptDisposition
		outcome.TranscriptResolutionFactDigest = closePayload.TranscriptResolutionFactDigest
	}
	return outcome, nil
}
