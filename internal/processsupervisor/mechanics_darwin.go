...
func (mechanics *darwinMechanics) Close(ctx context.Context, payload ClosePayload) (MechanicsResult, error) {
	mechanics.mu.Lock()
	defer mechanics.mu.Unlock()
	if ctx.Err() != nil || validateClosePayload(payload) != nil || !mechanics.terminal || mechanics.closed || payload.TranscriptDisposition == TranscriptNotRequired && mechanics.collected || payload.TranscriptDisposition != TranscriptNotRequired && !mechanics.collected {
		return MechanicsResult{}, ErrConflict
	}
	if err := mechanics.proveTerminalLocked(); err != nil {
		return MechanicsResult{}, err
