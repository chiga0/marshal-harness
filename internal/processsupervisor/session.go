...
func (session *Session) handleLocked(raw []byte) Response {
	...
	if request.Command != CommandClose && (result.TranscriptDisposition != "" || result.TranscriptResolutionFactDigest != "") {
	...
	if request.Command == CommandClose {
		closePayload := payload.(ClosePayload)
		result.TranscriptDisposition = closePayload.TranscriptDisposition
		result.TranscriptResolutionFactDigest = closePayload.TranscriptResolutionFactDigest
	}
...
func (session *Session) admitRequest(request Request) (requestProjection, any, error) {
	...
	case CommandClose:
		value := payload.(ClosePayload)
		if session.cleanupBinding == "" || value.CleanupBindingDigest != session.cleanupBinding || value.TranscriptDisposition == TranscriptNotRequired && session.collected || value.TranscriptDisposition != TranscriptNotRequired && !session.collected {
			return requestProjection{}, nil, ErrConflict
		}
	}
...
func decodePayload(command CommandName, raw json.RawMessage, projection *requestProjection) (any, error) {
	...
	case CommandClose:
		...
		if strictCanonicalDecode(raw, &value) != nil || validateClosePayload(value) != nil {
			return nil, ErrInvalid
		}
		projection.CleanupBindingDigest = value.CleanupBindingDigest
		...
		projection.TranscriptDisposition = value.TranscriptDisposition
		projection.TranscriptResolutionFactDigest = value.TranscriptResolutionFactDigest
		return value, nil
	default:
		return nil, ErrInvalid
	}
...
func validateMechanicsResult(result MechanicsResult) error {
	...
	if (result.TranscriptDisposition == "") != (result.TranscriptResolutionFactDigest == "") || result.TranscriptDisposition != "" && (result.TranscriptDisposition.Validate() != nil || !validDigest(result.TranscriptResolutionFactDigest)) {
		return ErrInvalid
	}
