...
func validateProjection(request requestProjection) error {
	...
	case CommandClose:
		legacy := options.TranscriptDisposition == "" && options.TranscriptResolutionFactDigest == ""
		if !validDigest(options.ProcessTerminalFactDigest) || !validDigest(options.AllocationTerminatedDigest) || !validDigest(options.CleanupBindingDigest) || !legacy && (options.TranscriptDisposition.Validate() != nil || !validDigest(options.TranscriptResolutionFactDigest)) {
			return ErrIntervention
		}
		options.ProcessTerminalFactDigest, options.AllocationTerminatedDigest, options.CleanupBindingDigest = "", "", ""
		options.TranscriptDisposition, options.TranscriptResolutionFactDigest = "", ""
	}
...
func validateStoredResponse(response Response, request requestProjection) error {
	...
	if request.Command == CommandClose {
		if payload.TranscriptDisposition != request.TranscriptDisposition || payload.TranscriptResolutionFactDigest != request.TranscriptResolutionFactDigest {
			return ErrIntervention
		}
	} else if payload.TranscriptDisposition != "" || payload.TranscriptResolutionFactDigest != "" {
		return ErrIntervention
	}
