...
func observeCurrentPath(path string, afterRead func()) (CurrentPathObjectV2, error) {
	fd, err := unix.Open(path, unix.O_RDONLY|unix.O_CLOEXEC|unix.O_NOFOLLOW|unix.O_NONBLOCK, 0)
	if err != nil {
		return CurrentPathObjectV2{}, reject(ReasonObjectMismatch)
	}
...
func observePathIdentity(path string) (CurrentPathObjectV2, error) {
	var stat unix.Stat_t
	if err := unix.Lstat(path, &stat); err != nil || stat.Mode&unix.S_IFMT != unix.S_IFREG {
		return CurrentPathObjectV2{}, reject(ReasonObjectMismatch)
	}
	return CurrentPathObjectV2{Device: decimalIdentity(uint64(stat.Dev)), Inode: decimalIdentity(stat.Ino), Size: stat.Size}, nil
}

func decimalIdentity(value uint64) string { return fmt.Sprintf("%d", value) }

func executablePathNamesObject(path string, object CurrentPathObjectV2) bool {
