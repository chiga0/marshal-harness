...
`node --test --test-concurrency=1 packages/task-client/*.test.mjs`：原测试包含正式 HTTP handler 的真实 loopback 服务、全部 24 operations、独立消费端下载摘要、显式批准响应丢失/回执、冲突、恶意响应、Unicode 转义 token 反射、超时/abort 和资源回收；新增测试覆盖运行答复的两族互斥、恶意 transport 题目串绑、丢响应后的显式原 key/body 重放及不自动继续。Application 是清楚标记的 DI fixture，而非 SQLite 或真实 Agent；这些结果只证明客户端/HTTP 合同，不证明真实规划、执行、同执行 ACK、取消、重启恢复或业务验收。
