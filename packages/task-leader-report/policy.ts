...
export const GUIDANCE = '本业务只支持上述日期窗口区域销售报告，不支持任意任务。读取原 context.text 中 profile/startDate/endDate；任一日期为 null 时，先用一个 options:[] 的 business ask 请求用户回复 JSON 字符串 {"startDate":"YYYY-MM-DD","endDate":"YYYY-MM-DD"}，不得猜日期或把示例作为答案。收到原 leaderReplies 后保留原已给日期，按原回答规划上述两个互补作者与独立 verifier；请自己组织有界提案，不需要逐字复述中文。完整日期无需多问。本 profile 的 proposal.nodes 必须恰为三个节点：east、west 的 role 均为 author，providerId 为 null 或当前业务 Provider ID；verify 的 role 为 verifier 且 providerId 必须为 null。proposal.edges 必须恰为 [{"from":"east","to":"verify"},{"from":"west","to":"verify"}]。独立 Review 是 Core 受管阶段，不属于业务 DAG；不得另加 review/reviewer 节点或边。优先省略整个 proposal.budget，沿用 snapshot.task.limits；不要自行压缩 maxAttempts，预算涵盖已消耗及后续 Leader、作者、Review、验证、发布与后验执行，用户明确要求合法缩减时仍须满足原容量门禁。已正确的结果直接进入独立 Review/验证/交付，不制造修正轮次。报告授权仅由 Core 的 publication request 获取，不发业务问题替代发布授权。';
