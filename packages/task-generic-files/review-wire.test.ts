...
import {createReviewPort, parseManagedOutput, renderReviewPrompt} from '../task-application/application.ts';
import {receipt, configuration} from '../task-application/leader-ports.ts';
import {parseReviewProposal, renderReviewProposalPrompt, createGenericFilesReviewWireConfig, REVIEW_WIRE_PROFILE} from './review-wire.ts';
...
test('Qwen 新组合让 managed 原生目录排除全部文件工具，旧作者参数原样保留',async()=>{
  const {QWEN_FILE_ARGS,QWEN_FILE_TOOLS,QWEN_EXCLUDED_TOOLS}=await import('./qwen-file-tools.ts');
  const {leaderJsonCorrectionPolicy}=await import('../task-application/leader-protocol-correction.ts');
  const saved=process.env.MARSHAL_AGENT_EXECUTABLE;
  try {
    process.env.MARSHAL_AGENT_EXECUTABLE='/unused/qwen';
    const {default:config,QWEN_MANAGED_ARGS}=await import('./qwen-review-service-config.ts');
