...
import {createGenericFilesReviewWireConfig,AUTHOR_GUIDANCE} from './review-wire.ts';
import {createReviewPort,createLeaderPort,configuration} from '../task-application/leader-ports.ts';
import {startTaskService} from '../task-service/composition.ts';import {Store,LEADER_FORMAT,encode,digest} from '../task-store/store.ts';
test('仅作者指导文本或注册源码指纹变化，同new profile开根在claim前拒绝',async t=>{
 ...
 const files=['../task-business/index.ts','review-wire.ts','qwen-review-service-config.ts','qwen-file-tools.ts','short-wire.ts','../task-application/leader-ports.ts','../task-application/review-assessment-contract.ts','../task-application/review-assessment.ts','../task-application/leader.ts','../task-service/composition.ts'];
