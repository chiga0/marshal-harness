import {registerReviewAssessments} from '../task-application/review-assessment.ts';
import {withReviewCriteria, reviewCriteria, reviewSources, parseAssessmentProposal, REVIEW_ASSESSMENT_PROFILE, REVIEW_ASSESSMENT_PROPOSAL_PROFILE, REVIEW_POLICIES} from '../task-application/review-assessment-contract.ts';
import {registerFileAuthorInstructions} from '../task-business/index.ts';
import fs from 'node:fs';
import {fileURLToPath} from 'node:url';
import {createLeaderPort, createReviewPort, parseManagedOutput, renderLeaderPrompt} from '../task-application/application.ts';
import {encode, digest} from '../task-store/store.ts';
import {createGenericFilesConfig} from './index.ts';
import {parseLeaderProposal, LEADER_WIRE_PROFILE} from './short-wire.ts';
import {check, RULE, GUIDANCE, MAX_FILE} from './policy.ts';
...
export function createGenericFilesReviewWireConfig(options) {
  ...
  const code = digest(encode(['../task-business/index.ts', 'review-wire.ts', 'qwen-review-service-config.ts', 'qwen-file-tools.ts', 'short-wire.ts', '../task-application/leader-ports.ts', '../task-application/review-assessment-contract.ts', '../task-application/review-assessment.ts', '../task-application/leader.ts', '../task-service/composition.ts']
