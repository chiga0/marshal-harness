...
const COUNTEREXAMPLE_REVIEW = '先检验反例，再作结论：对每个涉及操作效果或数据变化的验收要求，列出执行前状态、具体动作、执行后状态、失败时状态及支撑它们的候选原文或实现。在审查中逐步推演这条链，不能以“没有矛盾”“作者说可用”“符合惯例”代替证据。纯静态页面里<form>未写action仍可默认向当前页面提交；启用的submit按钮不是不可用示意，字段可填写也不能证明已收集、保存或完成登记。未获用户明确授权的交互演示，没有实际完成路径时应在操作处明确标注并禁用相关输入和提交控件，或改为仅展示说明/锚点；用户明确要求可交互原型时可保留入口前标明模拟的控件，但仍需核对模拟效果与反馈，不能冒充真实提交。底注、无外链、无脚本均不能证明无默认提交。对于更新、覆盖、删除、同步、迁移或回退，分别模拟新增数据与修改既有数据的情况：删除新增条目不等于撤销覆盖；恢复既有值必须有可取得的前值、版本、快照或其他明确恢复来源，并说明如何区分本次变化与原有数据。摘要、计数、当前值、仅含标识的日志不能还原丢失前值。允许诚实限定回退范围，不要求所有方案具有任意恢复能力；但不能一边缺少恢复来源，一边承诺不丢旧数据。发现一个有效反例即针对原节点提出rework，写明原句、状态变化、断点和最小修正；未知的关键效果不能判为已证明。没有发现反例时才accept，并在summary中给出关键证据和检查边界，不能把文本Review描述成实际执行过浏览器、数据库或外部系统。';

export function renderReviewProposalPrompt(input) {
  return '独立只读 Review：按原需求和验收检查全部冻结选果，不修改文件，不把作者声称pass当作证据，不启动额外进程。' +
    FACT_GROUNDING + INTERACTION_GROUNDING + COUNTEREXAMPLE_REVIEW + '方案还须内部一致且依赖可行：逐项检查读取、派生、重建和恢复需要的数据来源、权限及前置条件是否已在方案中定义且可取得，不能由摘要或标识推导不存在的原始内容。风险要求必须对应可执行的验收方法与回退条件；字段或关键词齐全不代表方案成立。发现缺失依赖或相互矛盾的承诺应rework，指出具体链路断点与修正要求；若无法满足授权范围则明确限制，不虚构可恢复性。' + '逐项阅读snapshot.task.input、snapshot.plan中已批准scope/acceptance、snapshot.interactions中的用户回答以及materials完整原文。核对候选每一项可核实的业务陈述，而非只数标题、条目或检查禁词。对无依据的设施/服务/流程/承诺提出rework，finding写明候选原句、缺少的来源和删除/改为明确建议/澄清的修正；不能因为与已知事实不矛盾就accept。对用户明确要求的虚构创作或方案建议按其范围审查，不把所有创造性内容误判为事实错误。' +
