...
async function finish(f, task, ticket, type, beforeConclude = false) {
  ...
  if (type !== 'review') await f.review(await f.take('review'));
  let leader = await f.take('leader');
  await f.decision(leader, [{type: 'work', kind: 'verify', nodeIds: ['verify'], selectionDigest: hash(leader.input.leader.snapshot.selection)}]);
  await f.verify(await f.take('execute', 'verify'));
  const record = await current(f, task), artifact = await f.read(tx => record.task.artifactIds.map(id => f.app.artifacts.metadata(tx, id)).find(a => a.kind === 'delivery'));
  await f.decision(await f.take('leader'), [{type: 'deliver', artifactId: artifact.id, acceptanceDigest: record.acceptance.digest, reviewDigest: record.leader.review.digest}]);
  if (beforeConclude) return await f.take('leader');
  await f.decision(await f.take('leader'), [{type: 'conclude', outcome: 'succeeded', summary: '原需求、原两分支和独立验证已完整完成', basisDigests: [record.acceptance.digest, record.leader.review.digest]}]);
}
for (const type of ['leader', 'review']) test(type + ' interruption: one budgeted current-generation successor completes the original team', {timeout: 20000}, async t => {
  const {f, task, ticket} = await setup(t, type), observation = await bound(f, ticket, true), before = await current(f, task);
  await f.reopen(); await f.app.execution.reconcileCleanup(ticket.workerId, observation); await f.app.leader.recover(task.id);
  assert.equal((await f.get(task.id)).status, 'running'); assert.equal((await current(f, task)).attempts, before.attempts);
  const successor = await f.take(type); assert.notEqual(successor.workerId, ticket.workerId); assert.notEqual(successor.generation, ticket.generation);
  assert.equal(successor.deadline, ticket.deadline); assert.equal(successor.input.leaderReplies[0].answer, 'north');
  assert.deepEqual((await current(f, task)).selectedResults, before.selectedResults);
  await assert.rejects(() => f.app.execution.finish(ticket, {cleanup: observation.payload.cleanup}), {code: 'recovery_required'});
  await finish(f, task, successor, type); const completed = await f.get(task.id); assert.equal(completed.status, 'completed');
  const head = await f.read(tx => tx.head(task.id)); await f.reopen(); await f.app.leader.recover(task.id);
  assert.deepEqual(await f.get(task.id), completed); assert.deepEqual(await f.read(tx => tx.head(task.id)), head);
});
...
async function finishPostverify(f, task, ticket) {
  ...
  await f.app.execution.started(ticket, await handle.started); await f.app.execution.finish(ticket, await handle.completion);
  const record = await current(f, task);
  await f.decision(await f.take('leader'), [{type: 'conclude', outcome: 'succeeded', summary: '原交付与发布后验完整完成',
...
for (const paused of [false, true]) test('postverify interruption preserves publication and pause fence=' + paused, {timeout: 20000}, async t => {
  ...
  const observation = await bound(f, ticket, true), before = await current(f, task);
  if (paused) await f.call({operation: 'task.pause', taskId: task.id, key: 'pause', body: {expectedRevision: (await f.get(task.id)).revision}});
  await f.reopen(); await f.app.execution.reconcileCleanup(ticket.workerId, observation); await f.app.leader.recover(task.id);
  const record = await current(f, task); assert.equal(record.task.status, paused ? 'paused' : 'running');
  ...
  if (paused) {
    assert.equal(await f.take('postverify'), null);
    await f.call({operation: 'task.resume', taskId: task.id, key: 'resume', body: {expectedRevision: record.task.revision}});
  }
  const successor = await f.take('postverify'); assert.notEqual(successor.generation, ticket.generation);
  assert.equal(successor.input.postverify.publicationReceiptDigest, ticket.input.postverify.publicationReceiptDigest);
  await finishPostverify(f, task, successor); const done = await current(f, task), head = await f.read(tx => tx.head(task.id));
  await f.reopen(); await f.app.leader.recover(task.id); assert.deepEqual(await current(f, task), done); assert.deepEqual(await f.read(tx => tx.head(task.id)), head);
});
test('final conclude interruption retains accepted decision, Review and verification instead of rerunning the team', {timeout: 20000}, async t => {
  ...
  const before = await current(f, task), observation = await bound(f, conclude, false); await f.reopen();
  await f.app.execution.reconcileCleanup(conclude.workerId, observation); await f.app.leader.recover(task.id);
  const successor = await f.take('leader'); assert.equal(successor.input.leader.snapshot.evidence.some(x => x.kind === 'verification'), true);
  ...
  const done = await current(f, task); assert.equal(done.task.status, 'completed'); assert.equal(done.attempts, before.attempts + 1);
...
for (const status of ['absent', 'matched', 'conflict', 'unknown']) test('unreserved publication action cold lookup=' + status + ' preserves original command and authority', async t => {
  ...
  const before = await current(f, task), old = await f.read(tx => tx.taskCommands(task.id).find(c => JSON.parse(c.payload).action === 'publication'));
  assert.equal(old.attemptId, ''); await f.reopen(); await f.app.leader.recover(task.id);
  const record = await current(f, task); assert.equal(record.attempts, before.attempts); assert.equal(port.starts, 0);
  const original = await f.read(tx => tx.command(old.id)); assert.equal(original.status, status === 'unknown' ? 'unknown' : 'observed');
  assert.equal(original.generation, old.generation);
  if (status === 'absent') {
    const next = await f.take('publication'); assert.equal(next.input.publication.binding.actionId, before.leader.publication.actionId);
    ...
    await f.app.execution.started(next, await handle.started); await f.app.execution.finish(next, await handle.completion);
    await finishPostverify(f, task, await f.take('postverify')); assert.equal(port.starts, 1);
  } else if (status === 'matched') {
    assert.equal(record.leader.publication.status, 'matched'); await finishPostverify(f, task, await f.take('postverify')); assert.equal(port.starts, 0);
  } else assert.equal(record.task.status, status === 'unknown' ? 'intervention' : 'failed');
  const head = await f.read(tx => tx.head(task.id)); await f.app.leader.recover(task.id); assert.deepEqual(await f.read(tx => tx.head(task.id)), head);
});
test('second cold open before postverify reservation closes exhausted handoff instead of dangling old verify', async t => {
  ...
  const before = await current(f, task); await f.reopen(); await f.app.leader.recover(task.id);
  assert.equal((await current(f, task)).attempts, before.attempts); await f.reopen(); await f.app.leader.recover(task.id);
  assert.equal((await f.get(task.id)).status, 'failed');
  assert.equal((await f.read(tx => tx.taskCommands(task.id).filter(c => c.status !== 'observed' && JSON.parse(c.payload).action === 'postverify'))).length, 0);
...
for (const fence of ['cancel', 'deadline']) test('publication lookup preserves matched effect after ' + fence + ' without successors', {timeout: 20000}, async t => {
  ...
  const before = await current(f, task); await f.reopen(); if (fence === 'deadline') f.app.now = () => ticket.deadline + 1;
  await f.app.execution.reconcileCleanup(ticket.workerId, observation);
  if (cancel) {
    const commandId = (await current(f, task)).cancelIntent.commandId, command = await f.read(tx => tx.command(commandId));
    await f.app.execution.settleControl(command.id, command.revision); // Original public control legitimately becomes unknown while effect unresolved.
  }
  await f.app.leader.recover(task.id); const closed = await current(f, task);
  ...
  const head = await f.read(tx => tx.head(task.id)); await f.reopen(); await f.app.leader.recover(task.id); assert.deepEqual(await f.read(tx => tx.head(task.id)), head);
});
test('wrong lookup binding and rollback preserve original obligation; retry only repeats read-only lookup', {timeout: 20000}, async t => {
  ...
  const observation = await bound(f, ticket, true); await f.reopen(); await f.app.execution.reconcileCleanup(ticket.workerId, observation);
  const head = await f.read(tx => tx.head(task.id));
  await assert.rejects(() => f.app.leader.recover(task.id), {code: 'invalid_leader_receipt'}); assert.deepEqual(await f.read(tx => tx.head(task.id)), head);
  ...
  try {await assert.rejects(() => f.app.leader.recover(task.id));} finally {f.app.transaction = transaction;}
  assert.equal(failed, true); assert.deepEqual(await f.read(tx => tx.head(task.id)), head);
  assert.equal((await current(f, task)).leader.publication.status, 'unknown'); await f.app.leader.recover(task.id);
  assert.equal((await current(f, task)).leader.publication.status, 'matched'); assert.equal(port.starts, 0);
});
test('unreserved publication does not infer permission after deadline inside lookup', async t => {
  ...
  const before = await current(f, task); deadline = Date.parse(before.task.deadlineAt); await f.reopen(); await f.app.leader.recover(task.id);
  assert.equal((await f.get(task.id)).status, 'failed'); assert.equal((await current(f, task)).attempts, before.attempts); assert.equal(port.starts, 0);
});
test('matched publication requires only the original two remaining postverify/conclude attempts, not another create', {timeout: 20000}, async t => {
  ...
  const observation = await bound(f, ticket, true); assert.equal((await current(f, task)).attempts, 10);
  await f.reopen(); await f.app.execution.reconcileCleanup(ticket.workerId, observation); await f.app.leader.recover(task.id);
  assert.equal((await f.get(task.id)).status, 'running'); await finishPostverify(f, task, await f.take('postverify'));
  assert.equal((await current(f, task)).attempts, 12); assert.equal(port.starts, 0);
});
...
test('second interruption cannot reset the same obligation retry count or budget', {timeout: 20000}, async t => {
  ...
  await f.reopen(); await f.app.execution.reconcileCleanup(ticket.workerId, observation); await f.app.leader.recover(task.id);
  const successor = await f.take('leader');
  ...
  const again = await bound(f, successor, false); await f.reopen();
  const attempts = (await current(f, task)).attempts; await f.app.execution.reconcileCleanup(successor.workerId, again); await f.app.leader.recover(task.id);
  assert.equal((await f.get(task.id)).status, 'failed'); assert.equal((await current(f, task)).attempts, attempts);
  assert.equal((await f.read(tx => tx.taskCommands(task.id).filter(c => c.status === 'pending' && JSON.parse(c.payload).action === 'leader'))).length, 0);
});
...
test('actual local publication created before SQL receipt: exact lookup preserves inode/bytes and never creates again', {timeout: 20000}, async t => {
  ...
  const attempts = (await current(f, task)).attempts; await f.reopen(); await f.app.execution.reconcileCleanup(ticket.workerId, observation); await f.app.leader.recover(task.id);
  const record = await current(f, task); assert.equal(record.leader.publication.status, 'matched'); assert.ok(record.leader.publication.receiptArtifactId);
  ...
  const ref = await f.read(tx => f.app.artifacts.metadata(tx, record.leader.publication.receiptArtifactId)), evidence = JSON.parse(f.app.artifacts.bytes(ref));
  assert.equal(evidence.status, 'matched'); assert.equal(evidence.createdByThisExecution, false);
  const head = await f.read(tx => tx.head(task.id)); await f.app.leader.recover(task.id); assert.deepEqual(await f.read(tx => tx.head(task.id)), head); assert.equal(starts, 1);
});
...
for (const [budget, answered, allowed] of [[10, false, false], [12, true, false], [13, true, true]])
  test('unplanned publication recovery includes original complete cost: budget=' + budget + ', answered=' + answered, {timeout: 20000}, async t => {
    const port = controlledEffects(), f = await fixture(t, effectOptions(port));
    ...
    let ticket = await f.take('leader');
    if (answered) {
      await f.decision(ticket, [{type: 'ask', kind: 'business', prompt: '请确认地区', options: [], subject: (await current(f, task)).inputDigest, nodeIds: []}]);
      ...
      ticket = await f.take('leader');
    }
    const before = await current(f, task), observation = await bound(f, ticket, false);
    assert.equal(before.plan, null); assert.equal(before.attempts, answered ? 2 : 1);
    await f.reopen(); await f.app.execution.reconcileCleanup(ticket.workerId, observation); await f.app.leader.recover(task.id);
    const resumed = await current(f, task); assert.equal(resumed.attempts, before.attempts); assert.deepEqual(resumed.limits, before.limits);
    assert.equal(resumed.task.deadlineAt, before.task.deadlineAt); assert.equal(await f.read(tx => f.app.execution.capacity(tx).value.active.length), 0);
    if (!allowed) {
      assert.equal(resumed.task.status, 'failed'); assert.equal(resumed.plan, null);
      assert.equal((await f.read(tx => tx.taskCommands(task.id).filter(c => c.status === 'pending' && JSON.parse(c.payload).action === 'leader'))).length, 0);
    } else {
      assert.equal(resumed.task.status, 'running'); const successor = await f.take('leader');
      ...
      const dispatch = await f.read(tx => tx.taskCommands(task.id).find(c => c.status === 'pending' && JSON.parse(c.payload).action === 'dispatch'));
      await f.app.execution.expandDispatch(dispatch.id, dispatch.revision); await f.author(await f.take('execute', 'east')); await f.author(await f.take('execute', 'west'));
      let leader = await f.take('leader');
      ...
      await f.review(await f.take('review')); leader = await f.take('leader');
      await f.decision(leader, [{type: 'work', kind: 'verify', nodeIds: ['verify'], selectionDigest: hash(leader.input.leader.snapshot.selection)}]);
      await f.verify(await f.take('execute', 'verify')); const record = await current(f, task);
      const artifact = await f.read(tx => record.task.artifactIds.map(id => f.app.artifacts.metadata(tx, id)).find(value => value.kind === 'delivery'));
      await f.decision(await f.take('leader'), [{type: 'deliver', artifactId: artifact.id, acceptanceDigest: record.acceptance.digest, reviewDigest: record.leader.review.digest}]);
      ...
      const publication = await f.take('publication'), handle = f.app.leader.effects.publication.start({ticket: publication, prepared: {cwd: f.parent, prompt: '受控原授权发布'}});
      await f.app.execution.started(publication, await handle.started); await f.app.execution.finish(publication, await handle.completion);
      await finishPostverify(f, task, await f.take('postverify')); assert.equal((await current(f, task)).attempts, 13); assert.equal(port.starts, 1);
    }
    const done = await current(f, task), head = await f.read(tx => tx.head(task.id));
    await f.reopen(); await f.app.leader.recover(task.id); assert.deepEqual(await current(f, task), done); assert.deepEqual(await f.read(tx => tx.head(task.id)), head);
  });
