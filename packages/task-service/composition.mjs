...
import {Store, CUSTODY_FORMAT, INTERACTION_FORMAT} from '../task-store/store.mjs';
...
const format = (custody, questions) => Buffer.from(JSON.stringify({profile: PROFILE, layout: questions ? 3 : custody ? 2 : 1}) + '\n');
...
class ServiceRoot {
  ...
  constructor(root, mode, custody, questions) {
    this.root = root; this.parent = path.dirname(root);
    this.format = format(custody, questions);
    try {
...
export async function startTaskService({root, mode, providers, prepare, collect, release, businessFactory, verification, clarification, custody, runtimeQuestions, dispose = () => {},
  ...
  try {
    files = new ServiceRoot(root, mode, !!custody, !!runtimeQuestions);
    const storeOptions = runtimeQuestions ? {format: INTERACTION_FORMAT} : custody ? {format: CUSTODY_FORMAT} : {};
    ...
    application = new TaskApplication({...applicationOptions, execution, store, owner, depot, verification, clarification, runtimeQuestions});
    if (custody) {
