...
import {observationConfiguration} from '../task-application/observation.ts';
...
export async function startTaskService({
  root, mode, providers, prepare, collect, release, businessFactory, verification, clarification, custody, runtimeQuestions, repair, unpermitted, workerCancellation, auditDisclosure, observability, leader, review, publication, dispose = () => {},
  ...
  requireValue(workerCancellation === undefined || object(workerCancellation) && Object.keys(workerCancellation).join(',') === 'profile' &&
    workerCancellation.profile === 'task-worker-cancellation/v1' && custody !== undefined);
