...
const readTask = async (f, task) => (await f.read(tx => f.app.get(tx, task.id)));
...
export async function setup(t, type, options = {}) {
  const f = await fixture(t, ['publication', 'postverify'].includes(type) ? {publication: publication(), publicationExpected: () => ({original: 'expected'}), ...options} : options);
  ...
  await f.decision((await f.take('leader')), [{type: 'ask', kind: 'business', prompt: '请确认地区', options: [], subject: (await readTask(f, task)).inputDigest, nodeIds: []}]);
  ...
  await f.decision((await f.take('leader')), [{type: 'plan', proposal}]);
  ...
  const dispatch = (await f.read(tx => tx.commands().find(c => JSON.parse(c.payload).action === 'dispatch'))); await f.app.execution.expandDispatch(dispatch.id, dispatch.revision);
  (await f.author((await f.take('execute', 'east')))); (await f.author((await f.take('execute', 'west'))));
  let ticket = (await f.take('leader')); if (type === 'leader') return {f, task, ticket};
  ...
  ticket = (await f.take('review')); if (type === 'review') return {f, task, ticket};
  await f.review(ticket); await f.decision((await f.take('leader')), [{type: 'work', kind: 'verify', nodeIds: ['verify'], selectionDigest}]);
  await f.verify((await f.take('execute', 'verify')));
  const record = (await readTask(f, task)), artifact = (await f.read(tx => record.task.artifactIds.map(id => f.app.artifacts.metadata(tx, id)).find(a => a.kind === 'delivery')));
  ...
  ticket = (await f.take('publication')); if (type === 'publication') return {f, task, ticket};
  ...
  return {f, task, ...(options.unreserved ? {} : {ticket: (await f.take('postverify'))})};
}
export async function bound(f, ticket, start) {
  ...
  try {
    ...
    if (start) {
      ...
      (await f.app.execution.started(ticket, runtime.started)); assert.equal((await runtime.completion).cleanup.cleaned, true);
...
const snapshot = async (f, task) => (await f.read(tx => ({head: tx.head(task.id), task: f.app.get(tx, task.id), capacity: f.app.execution.capacity(tx).value,
...
for (const type of ['leader', 'review', 'publication', 'postverify']) test('attached ' + type + ': original signed cleanup, no DAG fiction or retry', {timeout: 20000}, async t => {
  const {f, task, ticket} = await setup(t, type), observation = await bound(f, ticket, true), before = (await readTask(f, task));
  (await f.reopen()); f.app.now = () => ticket.deadline + 1;
  const result = (await await f.app.execution.reconcileCleanup(ticket.workerId, observation)), after = (await readTask(f, task));
  ...
  assert.deepEqual(after.leader.history, before.leader.history); assert.equal((await f.read(tx => f.app.execution.capacity(tx).value.active.length)), 0);
  assert.equal((await f.read(tx => tx.command(ticket.commandId))).status, type === 'publication' ? 'unknown' : 'observed');
  ...
  if (type === 'leader') {assert.equal(after.leader.activeCallId, null); assert.equal(after.leader.activeWorkerId, null); assert.equal(after.leader.obligationId, null);
    assert.equal((await f.read(tx => JSON.parse(tx.projection('interaction', ticket.input.leader.obligationId).bytes))).status, 'closed');}
  ...
  const settled = (await snapshot(f, task)); assert.deepEqual((await await f.app.execution.reconcileCleanup(ticket.workerId, observation)), result); assert.deepEqual((await snapshot(f, task)), settled);
  (await f.reopen()); assert.deepEqual((await await f.app.execution.reconcileCleanup(ticket.workerId, observation)), result); assert.deepEqual((await snapshot(f, task)), settled);
});
test('attached publication original signed none-start is not an unknown created effect', {timeout: 20000}, async t => {
  const {f, task, ticket} = await setup(t, 'publication'), observation = await bound(f, ticket, false); (await f.reopen()); f.app.now = () => ticket.deadline + 1;
  assert.equal((await await f.app.execution.reconcileCleanup(ticket.workerId, observation)).status, 'failed');
  const record = (await readTask(f, task)); assert.equal(record.task.status, 'failed'); assert.equal(record.leader.publication.status, 'failed');
  assert.equal(record.leader.publication.receiptArtifactId, null); assert.equal((await f.read(tx => tx.command(ticket.commandId))).status, 'observed');
});
test('attached Leader mismatched obligation/call/input/command stays unresolved; original signed proof remains usable', {timeout: 20000}, async t => {
  const {f, task, ticket} = await setup(t, 'leader'), observation = await bound(f, ticket, false); (await f.reopen()); f.app.now = () => ticket.deadline + 1;
  const before = (await snapshot(f, task)), originalTransaction = f.app.transaction.bind(f.app);
  ...
  for (const fault of ['obligation', 'input', 'activeCall', 'command']) {
    ...
    try {await assert.rejects(f.app.execution.reconcileCleanup(ticket.workerId, observation), {code: 'recovery_required'});}
    finally {f.app.transaction = originalTransaction;}
    assert.deepEqual((await snapshot(f, task)), before, fault + ' must roll back');
  }
  assert.equal((await await f.app.execution.reconcileCleanup(ticket.workerId, observation)).status, 'failed');
});
test('unbound attached reservations never inherit staging-only eligibility from v7 format', async t => {
  const {f, task, ticket} = await setup(t, 'leader'); assert.equal(ticket.startProtocol, undefined); (await f.reopen());
  const before = (await snapshot(f, task)); await assert.rejects(f.app.execution.settleUnpermitted(ticket.workerId), {code: 'recovery_required'});
  assert.deepEqual((await snapshot(f, task)), before);
});
