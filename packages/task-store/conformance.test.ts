...
export function runStoreConformance(label, factory) {
  ...
  test(`${label}: tenantScope 边界预留——根租户范围名为稳定 storeId`, async t => {
    const root = setup(t);
    const scopeCheck = factory.create(root);
    t.after(() => scopeCheck.close());
    const scope = scopeCheck.info().tenantScope, storeId = scopeCheck.info().storeId;
    assert.equal(typeof scope, 'string');
    assert.equal(scope, storeId, 'tenantScope 以 storeId 为值');
    scopeCheck.close();
    const reopened = factory.open(root);
    t.after(() => reopened.close());
    assert.equal(reopened.info().tenantScope, scope, '冷开 tenantScope 不变');
    assert.equal(reopened.info().tenantScope, reopened.info().storeId);
    const other = factory.create(setup(t));
    t.after(() => other.close());
    assert.notEqual(other.info().tenantScope, scope, '不同 root 边界名必须不同');
  });
}
