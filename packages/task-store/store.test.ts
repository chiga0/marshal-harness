...
async function fixture(t, options = {}) {
  ...
  t.after(() => store.close());
  const owner = await store.claimOwner(0n, 'instance-1', NOW + 60000);
...
async function assertEmpty(store, owner) {
...
test('core: one transaction commits every facet and reopens original bytes', async t => {
  const { root, store, owner } = await fixture(t);
  const event = await store.write(owner, allFacets);
  ...
  await assert.rejects(() => next.read(owner, () => {}), code('owner'));
  await assert.rejects(() => next.renewOwner(owner, NOW + 90000), code('owner'));
  const nextOwner = await next.claimOwner(owner.generation, 'instance-2', NOW + 60000);
  assert.equal(nextOwner.generation, 2n);
  await next.read(nextOwner, tx => {
...
test('core: every failure point rolls back all facets, including across reopen', async t => {
  for (let n = 1; n <= 5; n++) await t.test(`after write ${n}`, async t => {
    const { root, store, owner } = await fixture(t);
    await assert.rejects(() => store.write(owner, tx => allFacets(tx, n)), code('unavailable'));
    await assertEmpty(store, owner); store.close();
    const next = Store.openExisting(root, { clock: () => NOW }); t.after(() => next.close());
    await assertEmpty(next, await next.claimOwner(1, 'reopened', NOW + 60000));
  });
});

test('core: exact receipt replay precedes CAS and does not debit or enqueue again', async t => {
  const { store, owner } = await fixture(t);
  const original = await store.write(owner, allFacets);
  const replay = await store.write(owner, tx => {
  ...
  await assert.rejects(() => store.write(owner, tx => tx.receipt(receiptKey.scope, receiptKey.operation, receiptKey.keyDigest, digest(encode('other')))), code('conflict'));
  await assert.rejects(() => store.write(owner, tx => tx.putReceipt(receiptKey, requestDigest, ref('task-1', original), encode({ forged: true }))), code('conflict'));
});

test('core: caught method errors poison the transaction and preserve no prefix', async t => {
  const { store, owner } = await fixture(t);
  await assert.rejects(() => store.write(owner, tx => {
    allFacets(tx);
    try { tx.putProjection('task', 'task-1', 0, ref('task-1', makeEvent('task-1', 1, { type: 'approved', fixtureOnly: true })), encode({ bad: true })); } catch {}
  }), code('conflict'));
  await assertEmpty(store, owner);
});

test('core: nested, async, read-only and escaped transactions fail closed', async t => {
  const { store, owner } = await fixture(t);
  let called = false;
  await assert.rejects(() => store.write(owner, async () => { called = true; }), code('async-transaction'));
  assert.equal(called, false);
  let retained;
  await assert.rejects(() => store.write(owner, tx => { retained = tx; allFacets(tx); return Promise.resolve(); }), code('async-transaction'));
  await Promise.resolve();
  assert.throws(() => retained.head('task-1'), code('closed'));
  await assertEmpty(store, owner);
  // Async seam: a concurrent lifecycle call inside an active transaction fails
  // closed as its own rejection, and Store still poisons the active write — both
  // sides report nested-transaction. close() is synchronous and keeps the old
  // shape: a nested close also poisons the active write.
  for (const nested of [() => store.read(owner, () => {}), () => store.claimOwner(1, 'nested', NOW + 70000)]) {
    let rejection;
    await assert.rejects(() => store.write(owner, tx => { allFacets(tx); nested().catch(error => { rejection = error; }); }), code('nested-transaction'));
    await new Promise(resolve => setImmediate(resolve));
    assert.equal(rejection?.code, 'nested-transaction');
    await assertEmpty(store, owner);
  }
  await assert.rejects(() => store.write(owner, tx => { allFacets(tx); try { store.close(); } catch {} }), code('nested-transaction'));
  await assertEmpty(store, owner);
  await assert.rejects(() => store.read(owner, tx => { try { tx.append('task-1', empty(), [makeEvent('task-1', 1, {})]); } catch {} }), code('read-only'));
  await store.write(owner, tx => { retained = tx; });
  assert.throws(() => retained.append('task-1', empty(), [makeEvent('task-1', 1, {})]), code('closed'));
});

test('core: owner generation, renewal and commit-time expiry fence writes', async t => {
  let now = NOW;
  const { store, owner } = await fixture(t, { clock: () => now });
  const renewed = await store.renewOwner(owner, NOW + 120000);
  await assert.rejects(() => store.read(owner, () => {}), code('owner'));
  await assert.rejects(() => store.claimOwner(0, 'stale', NOW + 60000), code('owner'));
  await assert.rejects(() => store.write(renewed, tx => { allFacets(tx); now = NOW + 120001; }), code('owner'));
  now = NOW;
  await assertEmpty(store, renewed);
  const next = await store.claimOwner(1, 'new-instance', NOW + 60000);
  assert.equal(next.generation, 2n);
  await assert.rejects(() => store.write(renewed, () => {}), code('owner'));
});

test('core: unknown outbox is not reset or dispatched on replay/reopen', async t => {
  const { root, store, owner } = await fixture(t);
  await store.write(owner, allFacets);
  let original;
  ...
  store.close();
  const next = Store.openExisting(root, { clock: () => NOW }); t.after(() => next.close());
  const nextOwner = await next.claimOwner(1, 'next', NOW + 60000);
  await next.write(nextOwner, tx => {
    const { generation, revision, status, observation, ...enqueue } = original;
    assert.equal(tx.enqueue(enqueue).status, 'unknown');
  });
  await next.read(nextOwner, tx => { assert.equal(tx.command('start-1').revision, 2n); assert.equal(tx.commands().length, 1); });
  await assert.rejects(() => next.write(nextOwner, tx => tx.observeCommand('start-1', 2, 'pending', original.source)), code('invalid'));
  await assert.rejects(() => next.write(nextOwner, tx => tx.enqueue({ id: 'new-command', taskId: 'task-1', kind: 'start', inputDigest: original.inputDigest, payload: original.payload, source: original.source })), code('owner'));
});

test('core: real multi-event batch roundtrip, sequence/digest and cross-stream rejection', async t => {
  const { store, owner } = await fixture(t);
  const events = [1, 2, 3].map(n => makeEvent('task-1', n, { n }));
  await store.write(owner, tx => assert.deepEqual(tx.append('task-1', empty(), events), { sequence: 3n, digest: events[2].digest }));
  await store.read(owner, tx => assert.deepEqual(tx.events('task-1'), events));
  await assert.rejects(() => store.write(owner, tx => tx.append('task-2', empty(), [events[0]])), code('invalid'));
  await assert.rejects(() => store.write(owner, tx => tx.append('task-1', empty(), [events[0]])), code('conflict'));
  await assert.rejects(() => store.write(owner, tx => tx.append('task-2', empty(), [{ ...makeEvent('task-2', 1, {}), digest: events[0].digest }])), code('invalid'));
  await assert.rejects(() => store.write(owner, tx => tx.putProjection('task', 'task-2', 0, { stream: 'task-2', sequence: 1n, digest: events[0].digest }, encode({}))), code('conflict'));
});
...
test('recovery: SQLite lifetime lock rejects second process; close permits a new owner', async t => {
  ...
  const next = Store.openExisting(root, { clock: () => NOW }); t.after(() => next.close());
  await assert.rejects(() => next.read(owner, () => assert.fail('prior owner reused')), code('owner'));
  assert.equal((await next.claimOwner(1, 'new-owner', NOW + 60000)).generation, 2n);
});
...
test('recovery: abrupt process exit before/after commit preserves atomic facts, never sends outbox', async t => {
  for (const mode of ['before-commit', 'after-commit']) await t.test(mode, async t => {
    const { root, store } = await fixture(t); const storeId = store.info().storeId; store.close();
    ...
    const owner = await next.claimOwner(2, 'recovered', Date.now() + 60000);
    if (mode === 'before-commit') await assertEmpty(next, owner);
    else await next.read(owner, tx => {
...
test('recovery: create/open never initialize missing, partial, old or malformed roots', async t => {
  for (const mutation of ['missing-file', 'missing-format', 'corrupt', 'wrong-format', 'future-version', 'missing-trigger', 'missing-table']) await t.test(mutation, async t => {
    const { root, store } = await fixture(t); store.close();
...
test('recovery: child-parent sync failure must be resolved on open before claiming', async t => {
  ...
  const owner = await store.claimOwner(0, 'after-sync', NOW + 60000); await store.write(owner, allFacets);
  const storeId = owner.storeId; store.close();
  const next = Store.openExisting(root, { clock: () => NOW }); t.after(() => next.close());
  assert.equal((await next.claimOwner(1, 'after-restart', NOW + 60000)).storeId, storeId);
});

test('recovery: unsafe files, symlinks and directory replacement reject without adoption', async t => {
  for (const mutation of ['permissions', 'hardlink', 'symlink', 'root-replaced']) await t.test(mutation, async t => {
    const { root, store, owner } = await fixture(t), database = path.join(root, 'authority.sqlite');
    ...
    await assert.rejects(() => store.read(owner, () => assert.fail('unsafe root admitted')), code('unavailable'));
  });
...
test('bounds: exact record-count and page limits; ignored overflow rolls back real batch', async t => {
  const { store, owner } = await fixture(t);
  ...
  await assertEmpty(store, owner);
  await store.write(owner, tx => tx.append('task-1', empty(), events));
  for (let after = 0; after < events.length; after += LIMITS.page) {
    await store.read(owner, tx => assert.deepEqual(tx.events('task-1', after, LIMITS.page), events.slice(after, after + LIMITS.page)));
  }
  await assert.rejects(() => store.read(owner, tx => tx.events('task-1', 0, LIMITS.page + 1)), code('invalid'));
});

test('bounds: default full pages of projections and pending/observed commands survive cold reopen', async t => {
  for (const kind of ['projection', 'pending', 'observed']) await t.test(kind, async t => {
    const { root, store, owner } = await fixture(t);
    ...
    store.close();
    const next = Store.openExisting(root, { clock: () => NOW }); t.after(() => next.close());
    const nextOwner = await next.claimOwner(owner.generation, 'pagination-reader', NOW + 60000);
    ...
    for (;;) {
      ...
      const page = await next.read(nextOwner, tx => readPage(tx, after));
    ...
    await assert.rejects(() => next.write(nextOwner, tx => {
...
test('bounds: task command pages exclude unrelated history and retain original reference/access limits across reopen', async t => {
  const {root, store, owner} = await fixture(t), target = 'task-target', expected = [];
  for (let n = 0; n < 228; n++) {
  ...
  const nextOwner = await next.claimOwner(owner.generation, 'task-page-reader', NOW + 60000), result = [], sizes = [];
  let after = '';
  for (;;) {
    const rows = await next.read(nextOwner, tx => tx.taskCommands(target, after)); sizes.push(rows.length);
  ...
  await next.read(nextOwner, tx => assert.deepEqual(tx.taskCommands('missing-task'), []));
  for (const args of [['', '', 1], [target, 'bad cursor', 1], [target, '', LIMITS.page + 1]])
    await assert.rejects(() => next.read(nextOwner, tx => tx.taskCommands(...args)), code('invalid'));
  await assert.rejects(() => next.write(nextOwner, tx => {
  ...
  await next.read(nextOwner, tx => assert.deepEqual(tx.head('task-page-prefix'), empty()));
});

test('bounds: aggregate bytes are enforced independently of count; oversize poisons prefix', async t => {
  const { store, owner } = await fixture(t);
  ...
  await store.write(owner, tx => tx.append('task-1', empty(), [event]));
  const count = LIMITS.transactionBytes / size;
  await store.read(owner, tx => { for (let n = 0; n < count; n++) tx.head('task-1'); });
  await assert.rejects(() => store.read(owner, tx => { for (let n = 0; n <= count; n++) tx.head('task-1'); }), code('limit'));
  ...
  await store.read(owner, tx => assert.deepEqual(tx.events('task-2'), []));
});

test('bounds: deterministic transaction deadline, malformed bytes and owned copy boundaries', async t => {
  let monotonic = 0;
  const { store, owner } = await fixture(t, { monotonic: () => monotonic });
  await assert.rejects(() => store.write(owner, tx => { allFacets(tx); monotonic = LIMITS.transactionMs + 1; }), code('deadline'));
  monotonic = 0; await assertEmpty(store, owner);
  for (const bytes of [Buffer.from('{"payload":{},"payload":{},"sequence":"1","stream":"task-1"}'), Buffer.from('{ "payload":{},"sequence":"1","stream":"task-1"}')]) {
    await assert.rejects(() => store.write(owner, tx => tx.append('task-1', empty(), [{ sequence: 1n, digest: digest(bytes), bytes }])), code('invalid'));
  }
  const event = await store.write(owner, allFacets); event.bytes.fill(0);
  await store.read(owner, tx => { const read = tx.events('task-1')[0]; assert.notEqual(read.bytes[0], 0); read.bytes.fill(0); });
  await store.read(owner, tx => assert.notEqual(tx.events('task-1')[0].bytes[0], 0));
});
