...
export class Store {
  ...
  info() {
    try { this.#enter(); const row = this.#metadata(); return Object.freeze({ format: row.format, storeId: row.store_id, generation: row.generation, tenantScope: row.store_id }); }
  ...
  // Read-only snapshot of every stream head, FIFO-ordered with other lifecycle
  // operations. Used by audit anchoring to pin each stream's current chain head;
