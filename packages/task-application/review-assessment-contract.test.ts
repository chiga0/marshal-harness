...
import {withReviewCriteria,reviewCriteria,reviewSources,parseAssessmentProposal,validateReviewAssessment,REVIEW_ASSESSMENT_PROPOSAL_PROFILE} from './review-assessment-contract.ts';
...
test('持久化读取校验不提供receipt，不假装重新获得原材料真值',async()=>{
 const {validateStoredReviewAssessment}=await import('./review-assessment-contract.ts');
