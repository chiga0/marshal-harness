...
async function fixture(t) {
  ...
  let app = new TaskApplication({store, clock: () => now, owner: await store.claimOwner(0, 'first', now + 60000), execution});
  ...
  return {get app() {return app;}, get store() {return store;}, root, execution,
    async reserve() {
      ...
      const task = await app.dispatch(request, context), command = (await app.execution.poll()).items[0];
      return {task, request, ticket: await app.execution.nextWork(command.id, command.revision)};
    },
    async reopen(expire = false, beforeClaim = () => {}) {
      ...
      await beforeClaim(store);
      app = new TaskApplication({store, clock: () => now, owner: await store.claimOwner(generation, 'next', now + 60000), execution});
      return oldApp;
    },
    async edit(ticket, change) { await app.transaction(true, tx => {
...
test('v5 original reservation and prepared metadata settle once after deadline; old owner/result stay fenced', async t => {
  const f = await fixture(t), {task, ticket, request} = await f.reserve();
  assert.deepEqual(ticket.startProtocol, protocol);
  (await f.app.execution.observeInput(ticket, 'prepared', '原输入，不是已提交模型的证明'));
  const before = (await f.snapshot(task.id)), old = (await f.reopen(true));
  await assert.rejects(old.execution.started(ticket, {executionId: 'late'}), {code: 'application_unavailable'});
  await assert.rejects(f.app.execution.started(ticket, {executionId: 'late'}), {code: 'recovery_required'});
  await assert.rejects(f.app.execution.finish(ticket, {}), {code: 'recovery_required'});
  const result = (await f.app.execution.settleUnpermitted(ticket.workerId)), after = (await f.snapshot(task.id));
  ...
  assert.deepEqual((await f.app.execution.settleUnpermitted(ticket.workerId)), result); assert.deepEqual((await f.snapshot(task.id)), after);
  await f.reopen(); assert.deepEqual((await f.app.execution.settleUnpermitted(ticket.workerId)), result); assert.deepEqual((await f.snapshot(task.id)), after);
});
test('cancel wins; original unknown Operation settles only with the obligation, receipt stays exact', async t => {
  const f = await fixture(t), {task, ticket} = await f.reserve(), current = await f.app.dispatch({operation: 'task.get', taskId: task.id}, context);
  ...
  const stop = (await f.app.execution.poll()).items.find(command => command.kind === 'stop');
  ...
  (await f.app.execution.settleUnpermitted(ticket.workerId));
...
test('settlement SQL callback failure rolls back capacity, event, outbox and operation; retry then exact replay', async t => {
  const f = await fixture(t), {task, ticket} = await f.reserve(); await f.reopen();
  const before = (await f.snapshot(task.id)), write = f.store.write.bind(f.store);
  f.store.write = (owner, fn) => write(owner, tx => {fn(tx); throw Error('test-before-commit');});
  await assert.rejects(f.app.execution.settleUnpermitted(ticket.workerId), {code: 'application_unavailable'});
  assert.deepEqual((await f.snapshot(task.id)), before); f.store.write = write;
  (await f.app.execution.settleUnpermitted(ticket.workerId)); const after = (await f.snapshot(task.id));
  (await f.app.execution.settleUnpermitted(ticket.workerId)); assert.deepEqual((await f.snapshot(task.id)), after);
});
test('any original permit fact, wrong tuple, handoff or conflicting metadata denies negative proof without writes', async t => {
  ...
  for (const [name, corrupt] of Object.entries(cases)) await t.test(name, async t => {
    const f = await fixture(t), {task, ticket} = await f.reserve(); (await f.edit(ticket, (record, task, tx) => corrupt(record, task, tx, f))); await f.reopen();
    const before = (await f.snapshot(task.id)); await assert.rejects(f.app.execution.settleUnpermitted(ticket.workerId)); assert.deepEqual((await f.snapshot(task.id)), before);
  });
  await t.test('actual metadata handoff producer conflicts', async t => {
    const f = await fixture(t), {task, ticket} = await f.reserve();
    (await f.app.execution.observeInput(ticket, 'prepared', '原输入')); (await f.app.execution.observeInput(ticket, 'handed-off')); await f.reopen();
    const before = (await f.snapshot(task.id)); await assert.rejects(f.app.execution.settleUnpermitted(ticket.workerId), {code: 'recovery_required'});
...
test('actual bind transaction prevents unpermitted settlement and preclaim requires all three same-source facts', async t => {
  const f = await fixture(t), {task, ticket} = await f.reserve(), profile = {id: 'test', scope: 'inherited-process-group', eligible: true};
  ...
  (await f.app.execution.bindCustody(ticket, descriptor, profile)); await f.reopen();
  const before = (await f.snapshot(task.id)); await assert.rejects(f.app.execution.settleUnpermitted(ticket.workerId), {code: 'recovery_required'});
  assert.deepEqual((await f.snapshot(task.id)), before);
  // Synthetic corruption negative, NOT a signed cleanup or real process test.
  (await f.edit(ticket, record => {delete record.custody;}));
  const generation = f.store.info().generation; f.store.close();
  const reader = Store.openExisting(f.root, {format: UNPERMITTED_FORMAT});
  try {await assert.rejects(TaskCleanup.inspectBeforeClaim(reader)); assert.equal(reader.info().generation, generation);} finally {reader.close();}
});
test('v5 cannot construct an App with disclosure callback or missing protocol; old formats reject protocol', async t => {
  const f = await fixture(t), disclosure = createAuditDisclosure({id: 'test', version: '1', redact() {throw Error('must never run');}});
...
test('mixed original bound/unbound siblings: cancel stays unknown until real signed last cleanup; all replays exact', {timeout: 15000}, async t => {
  ...
  const plan = await f.app.proposePlan(task.id, task.revision, {summary: '两个分支', nodes: ['a', 'b'].map(id =>
  ...
  const dispatch = (await f.app.execution.poll()).items.find(command => JSON.parse(command.payload).action === 'dispatch');
  f.app.execution.expandDispatch(dispatch.id, dispatch.revision);
  const commands = (await f.app.execution.poll()).items.filter(command => JSON.parse(command.payload).action === 'execute');
  const a = (await f.app.execution.nextWork(commands[0].id, commands[0].revision)), b = (await f.app.execution.nextWork(commands[1].id, commands[1].revision));
  ...
  const receipt = await f.app.dispatch(request, context); await f.reopen(); (await f.app.execution.reconcile(task.id));
  const stop = (await f.app.execution.poll()).items.find(command => command.kind === 'stop'); f.app.execution.settleControl(stop.id, stop.revision);
  const settled = (await f.app.execution.settleUnpermitted(a.workerId)), interim = (await f.snapshot(task.id));
  ...
  (await f.app.execution.reconcileCleanup(b.workerId, observation));
  assert.equal((await f.app.dispatch({operation: 'operation.get', operationId: receipt.id}, context)).status, 'succeeded');
  assert.equal((await f.snapshot(task.id)).task.task.status, 'cancelled'); assert.equal((await f.snapshot(task.id)).capacity.active.length, 0);
  assert.deepEqual(await f.app.dispatch(request, context), receipt); const final = (await f.snapshot(task.id));
  await f.reopen(); assert.deepEqual((await f.app.execution.settleUnpermitted(a.workerId)), settled);
  (await f.app.execution.reconcileCleanup(b.workerId, observation)); assert.deepEqual((await f.snapshot(task.id)), final);
});
test('more than 100 legitimate progress events cannot hide original bound permit or block signed recovery', {timeout: 15000}, async t => {
  const f = await fixture(t), {ticket, task} = await f.reserve(), directory = path.join(path.dirname(f.root), 'custody');
  ...
  (await f.app.execution.bindCustody(ticket, handle.descriptor, profile)); handle.permit();
  ...
  (await f.app.execution.started(ticket, runtime.started));
  for (let sequence = 1; sequence <= 105; sequence++) assert.equal((await f.app.execution.progress(ticket, sequence, {summary: '原协议进度', tool: null, source: 'execution'})), true);
  ...
  (await f.reopen(false, async store => assert.deepEqual((await TaskCleanup.inspectBeforeClaim(store)).items, [handle.descriptor])));
  await assert.rejects(f.app.execution.settleUnpermitted(ticket.workerId), {code: 'recovery_required'});
  (await f.app.execution.reconcileCleanup(ticket.workerId, observation));
  assert.equal((await f.snapshot(task.id)).capacity.active.length, 0); const final = (await f.snapshot(task.id));
  (await f.reopen(false, async store => assert.deepEqual((await TaskCleanup.inspectBeforeClaim(store)).items, [])));
  (await f.app.execution.reconcileCleanup(ticket.workerId, observation)); assert.deepEqual((await f.snapshot(task.id)), final);
});
