...
test('原Port解析诊断只改变显示，关闭/外来/迟到事件不伪造阶段',async t=>{
  for(const mode of ['json','shape','disabled','foreign','late','provider'])await t.test(mode,async t=>{
    const enabled=mode!=='disabled',f=fixture(t,{observability:enabled?{profile:'task-observation/v1',retainPrompts:false}:null});
    ...
    const get=()=>f.read(tx=>f.app.execution.worker(tx,ticket.workerId).record.worker);const worker=get();assert.equal(worker.status,'failed');
