...
async function fixture(t, {disclosure = null, depotEnabled = true} = {}) {
  ...
  let owner = await store.claimOwner(0, 'fixture', now + 3600000), depot = depotEnabled ? ArtifactDepot.create(objects) : null;
  ...
  const f = {parent, state, objects, get app() {return app;}, get depot() {return depot;}, get store() {return store;},
    ...
    async reserve(inputRefs = []) {
      ...
      const command = (await f.read(tx => tx.taskCommands(task.id)))[0];
      return {task, ticket: await app.execution.nextWork(command.id, command.revision)};
    },
    audit: taskId => f.call({operation: 'task.audit', taskId}),
    async reopen() {
      store.close(); depot?.close(); store = Store.openExisting(state, {clock: () => now});
      owner = await store.claimOwner(owner.generation, 'next', now + 3600000); depot = depotEnabled ? ArtifactDepot.openExisting(objects) : null;
...
test('default retains only observed metadata; exact replay changes neither Task revision nor budget, and old records are unavailable', async t => {
  const f = await fixture(t), {task, ticket} = await f.reserve(), secret = 'synthetic-private-body-only-in-prepare-4e80';
  ...
  assert.equal((await f.app.execution.observeInput(ticket, 'prepared', secret)), true);
  const preparedHead = (await f.read(tx => tx.head(task.id)));
  (await f.app.execution.observeInput(ticket, 'prepared', secret)); assert.deepEqual((await f.read(tx => tx.head(task.id))), preparedHead);
  ...
  f.advance(1); (await f.app.execution.observeInput(ticket, 'handed-off'));
  const handedHead = (await f.read(tx => tx.head(task.id))); (await f.app.execution.observeInput(ticket, 'handed-off')); assert.deepEqual((await f.read(tx => tx.head(task.id))), handedHead);
  ...
  await f.reopen(); assert.deepEqual(await f.audit(task.id), audit);
  await assert.rejects(f.app.execution.observeInput(ticket, 'handed-off'), {code: 'recovery_required'});
});
...
test('explicit disclosure stores only returned bytes, bounded Unicode preview and original input refs; cold download is exact', async t => {
  ...
  let calls = 0;
  const f = await fixture(t, {disclosure: disclosure((prompt, identity) => {calls++; assert.ok(identity.workerId); return prompt.replace(privatePart, '[已移除]');})});
  ...
  (await f.app.execution.observeInput(ticket, 'prepared', raw)); (await f.app.execution.observeInput(ticket, 'prepared', raw)); assert.equal(calls, 1);
...
test('policy decline/throw/Promise/oversize and missing Depot produce unavailable content, not a new execution fence', async t => {
  ...
  for (let index = 0; index < policies.length; index++) {
    const f = await fixture(t, {disclosure: disclosure(policies[index]), depotEnabled: index !== policies.length - 1}), {task, ticket} = await f.reserve();
    assert.equal((await f.app.execution.observeInput(ticket, 'prepared', 'original-private-input')), true);
    ...
    assert.equal(audit.prompts[0].observation.snapshot, null); assert.equal(await f.app.execution.mayStart(ticket), true); assert.equal(audit.attempts, 1);
...
test('wrong ticket/body, partial SQL failure and missing committed snapshot never fabricate a handoff or repair bytes', async t => {
  const f = await fixture(t, {disclosure: disclosure(prompt => prompt)}), {task, ticket} = await f.reserve();
  assert.equal((await f.app.execution.observeInput(ticket, 'handed-off')), false);
  await assert.rejects(f.app.execution.observeInput({...ticket, inputDigest: 'sha256:' + '0'.repeat(64)}, 'prepared', 'public'), {code: 'recovery_required'});
  const before = (await f.read(tx => tx.head(task.id))), write = t.mock.method(f.store, 'write', () => {throw Error('private-sql-failure');});
  await assert.rejects(f.app.execution.observeInput(ticket, 'prepared', 'public'), {code: 'application_unavailable'}); write.mock.restore();
  assert.deepEqual((await f.read(tx => tx.head(task.id))), before); assert.equal((await f.audit(task.id)).prompts[0].observation.stage, 'unavailable');
  (await f.app.execution.observeInput(ticket, 'prepared', 'public'));
  await assert.rejects(f.app.execution.observeInput(ticket, 'prepared', 'different'), {code: 'invalid_request'});
  ...
  assert.equal((await f.app.execution.observeInput(ticket, 'prepared', 'public')), true, 'historical observation replay does not re-put content');
...
test('API audit binding rejects wrong task/worker, fabricated observation stage and foreign snapshot', async t => {
  const f = await fixture(t, {disclosure: disclosure(prompt => prompt)}), {task, ticket} = await f.reserve();
  (await f.app.execution.observeInput(ticket, 'prepared', 'public')); const original = await f.audit(task.id);
