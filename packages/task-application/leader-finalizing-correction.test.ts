...
test('真实S01同预算：12 attempts/7 calls/一次业务rework后的finalizing仅再用一次调用完成',async t=>{
 ...
 await f.decision(await f.take('leader'),[{type:'plan',proposal:{...proposal,nodes:proposal.nodes.filter(n=>n.id!=='west'),edges:[{from:'east',to:'verify'}]}}]);
 ...
 let leader=await f.take('leader');await f.decision(leader,[{type:'work',kind:'review',nodeIds:['east'],selectionDigest:hash(leader.input.leader.snapshot.selection)}]);
 await f.review(await f.take('review'),{verdict:'rework',findings:[{id:'facts',nodeIds:['east'],requirement:'原事实',observation:'新增事实',requestedChange:'删除新增事实'}]});
 let view=await f.call({operation:'task.leader',taskId:task.id});await f.decision(await f.take('leader'),[{type:'repair',nodeIds:['east'],basis:{kind:'review',digest:view.review.digest},feedback:'修复原事实'}]);
 await f.author(await f.take('execute','east'));
