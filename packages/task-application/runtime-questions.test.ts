...
async function fixture(t, {consumer = true, validator = () => true, proposalValue = proposal, layoutValue = layout, nodeIds = ['code']} = {}) {
  ...
  let owner = await store.claimOwner(0, 'first', now + 3600000);
  ...
  const f = {get app() {return app;}, get execution() {return app.execution;}, runtimeQuestions, verification, root,
    ...
    async start() {
      ...
      const plan = await app.proposePlan(task.id, task.revision, proposalValue), current = await f.get(task.id);
      ...
      const dispatch = (await f.read(tx => tx.commands())).find(row => JSON.parse(row.payload).action === 'dispatch');
      await app.execution.expandDispatch(dispatch.id, dispatch.revision); return task.id;
    },
    async take(nodeId) {const row = (await f.read(tx => tx.commands())).find(row => JSON.parse(row.payload).nodeId === nodeId);
      return await app.execution.nextWork(row.id, row.revision);},
    async started(ticket) {await app.execution.started(ticket, cleanFact(ticket));},
...
test('native correlation IDs preserve opaque bytes, reject malformed bounds, and remain bound to the original question', async t => {
  const f = await fixture(t), taskId = await f.start(), code = (await f.take('code')); (await f.started(code));
  ...
  for (const [field, limit] of [['nativeRequestId', 256], ['toolCallId', 128]]) {
    for (const invalid of ['', ' ', null, 12, '\u0000', '\ud800', 'x'.repeat(limit + 1), '界'.repeat(Math.floor(limit / 3) + 1)]) {
      await assert.rejects(f.execution.registerQuestion(code, question({[field]: invalid})), error => error.code === 'invalid_request');
  ...
  const q = (await f.execution.registerQuestion(code, request)), head = (await f.read(tx => tx.head(taskId)));
  const fact = (await f.read(tx => JSON.parse(tx.projection('interaction', q.questionId).bytes.toString('utf8'))));
  assert.equal(fact.nativeRequestId, request.nativeRequestId); assert.equal(fact.toolCallId, request.toolCallId);
  assert.deepEqual((await f.execution.registerQuestion(code, request)), q);
  await assert.rejects(f.execution.registerQuestion(code, {...request, toolCallId: request.toolCallId.replace('|', ':')}), error => error.code === 'state_conflict');
  assert.deepEqual((await f.read(tx => tx.head(taskId))), head);
...
test('same Task/Worker answer is durable, exact-once dispatched/ACKed and reaches independent verification input', async t => {
  ...
  const q = (await f.execution.registerQuestion(code, question())), head = (await f.read(tx => tx.head(taskId)));
  assert.deepEqual((await f.execution.registerQuestion(code, question())), q); assert.deepEqual((await f.read(tx => tx.head(taskId))), head);
  ...
  const docs = (await f.take('docs')); assert.ok(docs); (await f.started(docs)); // Independent branch stays dispatchable.
  ...
  const dispatched = (await f.execution.dispatchAnswer(code, q.questionId));
  ...
  await assert.rejects(f.execution.dispatchAnswer(code, q.questionId), error => error.code === 'state_conflict');
  ...
  await assert.rejects(f.execution.acknowledgeAnswer(code, q.questionId, {...ack, deliveryNonce: 'b'.repeat(64)}), error => error.code === 'state_conflict');
  assert.equal((await f.execution.acknowledgeAnswer(code, q.questionId, ack)), true);
  const afterACK = (await f.read(tx => tx.head(taskId))); assert.equal((await f.execution.acknowledgeAnswer(code, q.questionId, ack)), true);
  assert.deepEqual((await f.read(tx => tx.head(taskId))), afterACK);
  ...
  assert.equal((await f.finish(code)).status, 'completed'); assert.equal((await f.finish(docs)).status, 'completed');
  const verify = (await f.take('verify')); assert.equal(verify.input.interactionRefs.length, 1);
  assert.equal(verify.input.interactionRefs[0].answer.answer, 'north'); assert.equal(hash(verify.input), verify.inputDigest);
  const handle = f.verification.start({ticket: verify, prepared: {cwd: '/controlled-fixture'}}); (await f.started(verify));
  assert.equal((await f.execution.finish(verify, await handle.completion)).status, 'completed');
  assert.equal((await f.get(taskId)).status, 'completed');
  await f.reopen(); assert.equal((await f.call(request)).replayed, true);
...
test('pause keeps accepted answer pending, stale CAS cannot cancel and resumed delivery uses original deadline', async t => {
  const f = await fixture(t), taskId = await f.start(), code = (await f.take('code')); (await f.started(code));
  const q = (await f.execution.registerQuestion(code, question())), request = await f.answer(taskId, q);
  ...
  assert.equal((await f.execution.dispatchAnswer(code, q.questionId)), null);
  ...
  assert.equal((await f.execution.dispatchAnswer(code, q.questionId)).deadlineAt, deadline);
});
...
test('cancel/expiry/SQL failure never fabricate ACK or new execution and clean failure releases delivery obligation', async t => {
  for (const mode of ['cancel-before-answer', 'cancel-before-dispatch', 'cancel-after-dispatch', 'expiry', 'ack-sql']) {
    ...
    if (['cancel-after-dispatch', 'ack-sql'].includes(mode)) delivery = (await f.execution.dispatchAnswer(code, q.questionId));
    if (mode === 'ack-sql') {
      f.failSQL(true); await assert.rejects(f.execution.acknowledgeAnswer(code, q.questionId, {questionDigest: q.questionDigest,
      ...
      (await f.execution.fail(code, 'worker_failed'));
    } else if (mode === 'expiry') {f.advance(10001); (await f.execution.reconcile(taskId));}
    ...
    await assert.rejects(f.execution.dispatchAnswer(code, q.questionId));
    ...
    (await f.finish(code, false)); (await f.execution.reconcile(taskId));
    assert.equal((await f.read(tx => f.execution.capacity(tx).value.active.length)), 0);
    ...
    assert.equal((await f.read(tx => tx.commands())).filter(row => JSON.parse(row.payload).action === 'answer').every(row => row.status === 'observed'), true);
  }
});

test('question proof, validator, same-key shape and cold generation all remain fail-closed', async t => {
  const f = await fixture(t), taskId = await f.start(), code = (await f.take('code')); (await f.started(code));
  f.failSQL(true); await assert.rejects(f.execution.registerQuestion(code, question()), error => error.code === 'application_unavailable');
  f.failSQL(false); assert.equal((await f.call({operation: 'task.questions', taskId})).items.length, 0);
  const q = (await f.execution.registerQuestion(code, question()));
  await assert.rejects(f.execution.registerQuestion(code, question({prompt: '替换原问题'})), error => error.code === 'state_conflict');
  ...
  await f.reopen(); assert.equal((await f.call(request)).replayed, true);
  await assert.rejects(f.execution.dispatchAnswer(code, q.questionId), error => error.code === 'recovery_required');
  (await f.execution.reconcile(taskId)); assert.equal((await f.call({operation: 'task.questions', taskId})).items[0].deliveryStatus, 'cancelled');
  assert.equal((await f.read(tx => f.execution.capacity(tx).value.active.length)), 1); // No invented old-process cleanup.
});
...
test('three-level and diamond dependencies preserve exactly original inherited answer in actual prepared prompt and final receipt', async t => {
  ...
  const f = await fixture(t, {proposalValue: plan, layoutValue: layouts, nodeIds: ['code', 'middle']}), taskId = await f.start(), code = (await f.take('code')); (await f.started(code));
  const q = (await f.execution.registerQuestion(code, question())); await f.call(await f.answer(taskId, q));
  const d = (await f.execution.dispatchAnswer(code, q.questionId)); (await f.execution.acknowledgeAnswer(code, q.questionId,
    {questionDigest: d.questionDigest, answerDigest: d.answerDigest, deliveryNonce: d.deliveryNonce}));
  assert.equal((await f.finish(code)).status, 'completed');
  const middle = (await f.take('middle')); (await f.started(middle)); assert.equal(middle.input.interactionRefs.length, 1);
  const middleQ = (await f.execution.registerQuestion(middle, question({nativeRequestId: 'middle-ui', questionNonce: 'b'.repeat(64)})));
  const middleAnswer = await f.answer(taskId, middleQ); middleAnswer.key = 'middle-answer'; await f.call(middleAnswer);
  const middleD = (await f.execution.dispatchAnswer(middle, middleQ.questionId));
  (await f.execution.acknowledgeAnswer(middle, middleQ.questionId, {questionDigest: middleD.questionDigest, answerDigest: middleD.answerDigest, deliveryNonce: middleD.deliveryNonce}));
  assert.equal((await f.finish(middle)).status, 'completed');
  const docs = (await f.take('docs')); (await f.started(docs));
  ...
  for (const change of [ticket => ticket.input.interactionRefs[0].ackDigest = 'sha256:' + 'f'.repeat(64),
    ...
    await assert.rejects(f.app.transaction(false, tx => f.app.runtimeQuestions.resultRefs(tx, f.app.get(tx, taskId), wrong)), error => error.code === 'recovery_required');
  }
  assert.deepEqual((await f.read(tx => tx.head(taskId))), before);
  assert.equal((await f.finish(docs)).status, 'completed');
  const verify = (await f.take('verify')); assert.deepEqual(verify.input.interactionRefs.map(ref => ref.questionDigest), [q.questionDigest, middleQ.questionDigest]); (await f.started(verify));
...
test('own acknowledged refs merge without leaking another branch pending question; unACKed inheritance refuses', async t => {
  const f = await fixture(t, {nodeIds: ['code', 'docs']}), taskId = await f.start(), code = (await f.take('code')), docs = (await f.take('docs'));
  (await f.started(code)); (await f.started(docs));
  const first = (await f.execution.registerQuestion(code, question())), other = (await f.execution.registerQuestion(docs, question({nativeRequestId: 'ui-2', questionNonce: 'b'.repeat(64)})));
  await f.call(await f.answer(taskId, first)); const d = (await f.execution.dispatchAnswer(code, first.questionId));
  (await f.execution.acknowledgeAnswer(code, first.questionId, {questionDigest: d.questionDigest, answerDigest: d.answerDigest, deliveryNonce: d.deliveryNonce}));
  const values = (await f.read(tx => f.app.runtimeQuestions.resultRefs(tx, f.app.get(tx, taskId), code)));
  assert.equal(values.length, 1); assert.equal(values[0].questionDigest, first.questionDigest);
  await assert.rejects(f.app.transaction(false, tx => f.app.runtimeQuestions.inherited(tx, f.app.get(tx, taskId), [{questionDigest: other.questionDigest}])),
...
test('question registered while paused resumes awaiting-answer; pause never dispatches an answer', async t => {
  ...
  const q = (await f.execution.registerQuestion(code, question())); current = await f.get(taskId);
  assert.equal(current.status, 'paused'); assert.equal((await f.execution.dispatchAnswer(code, q.questionId)), null);
  ...
  await f.call(await f.answer(taskId, q)); const d = (await f.execution.dispatchAnswer(code, q.questionId));
  (await f.execution.acknowledgeAnswer(code, q.questionId, {questionDigest: d.questionDigest, answerDigest: d.answerDigest, deliveryNonce: d.deliveryNonce}));
  assert.equal((await f.get(taskId)).status, 'running'); assert.equal((await f.finish(code)).status, 'completed');
});
