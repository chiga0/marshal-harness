import {boundedPublicText, normalizedDiagnostic} from '../agent-observation/normalization.ts';
...
export const OBSERVATION_POLICY = Object.freeze({id: 'builtin-text-redaction', version: '1',
  sourceDigest: 'sha256:' + createHash('sha256').update(fs.readFileSync(new URL('./observation.ts', import.meta.url))).update(fs.readFileSync(new URL('../agent-observation/normalization.ts', import.meta.url))).update(fs.readFileSync(new URL('./input-audit.ts', import.meta.url))).digest('hex')});
