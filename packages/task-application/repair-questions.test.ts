...
async function fixture(t) {
  ...
  let owner = await store.claimOwner(0, 'first', Date.now() + 3600000);
  ...
  const f = {
    ...
    read: fn => app.transaction(false, fn), commands: async () => (await f.read(tx => tx.commands())),
    head: async taskId => (await f.read(tx => tx.head(taskId))),
    questions: taskId => f.call({operation: 'task.questions', taskId}),
    async facts(taskId) {return (await f.read(tx => app.get(tx, taskId).runtimeQuestions.questions.map(q => ({state: q,
      records: [q.id, q.answerId, q.dispatchId, q.ackId].map(id => tx.projection('interaction', id))}))));},
    async refs(ticket) {return (await f.read(tx => app.execution.worker(tx, ticket.workerId).record.interactionRefs));},
    async take(nodeId) {
      const row = (await f.commands()).find(row => row.status === 'pending' && JSON.parse(row.payload).nodeId === nodeId);
    ...
    async setup() {
      ...
      const row = (await f.commands()).find(row => JSON.parse(row.payload).action === 'plan');
      const ticket = (await app.execution.nextWork(row.id, row.revision));
      ...
      const dispatch = (await f.commands()).find(row => JSON.parse(row.payload).action === 'dispatch');
      (await app.execution.expandDispatch(dispatch.id, dispatch.revision));
    ...
    async author(ticket, content, afterStarted = null) {
      ...
      (await app.execution.started(ticket, started)); if (afterStarted) await afterStarted();
      ...
      (await app.execution.finish(ticket, complete)); business.release(ticket); return {prepared, complete};
    },
    async verify(inspect = () => {}) {
      const ticket = (await f.take('verify')); inspect(ticket);
      ...
      (await app.execution.started(ticket, await handle.started));
      const result = await handle.completion;
      (await app.execution.finish(ticket, result)); business.release(ticket); (await app.execution.reconcile(ticket.taskId));
      for (const row of (await f.commands()).filter(row => row.status === 'pending')) (await app.execution.settleControl(row.id, row.revision));
...
test('retained original ACK survives cold owner change and code-only repair without redelivery or stale completion', {timeout: 30000}, async t => {
  const f = await fixture(t), {taskId, plan} = await f.setup(), originalCode = (await f.take('code')), docs = (await f.take('docs'));
  const oldResult = await f.author(originalCode, 'wrong');
  let question, answerRequest, ack;
  await f.author(docs, 'retained', async () => {
    question = (await f.app.execution.registerQuestion(docs, {sessionId: 'controlled-session', nativeRequestId: 'ui:original|docs',
    ...
    const delivery = (await f.app.execution.dispatchAnswer(docs, question.questionId)); assert.equal(delivery.answer, 'north');
    await assert.rejects(f.app.execution.dispatchAnswer(docs, question.questionId), {code: 'state_conflict'});
    ack = {questionDigest: delivery.questionDigest, answerDigest: delivery.answerDigest, deliveryNonce: delivery.deliveryNonce};
    const beforeBadACK = (await f.head(taskId));
    await assert.rejects(f.app.execution.acknowledgeAnswer(docs, question.questionId, {...ack, deliveryNonce: 'b'.repeat(64)}), {code: 'state_conflict'});
    assert.deepEqual((await f.head(taskId)), beforeBadACK);
    assert.equal((await f.app.execution.acknowledgeAnswer(docs, question.questionId, ack)), true);
    const acknowledged = (await f.head(taskId));
    assert.equal((await f.app.execution.acknowledgeAnswer(docs, question.questionId, ack)), true);
    assert.deepEqual((await f.head(taskId)), acknowledged);
  });
  const originalRefs = (await f.refs(docs)); assert.equal(originalRefs.length, 1);
  ...
  const originalQuestions = await f.questions(taskId), originalFacts = (await f.facts(taskId));
  ...
  assert.deepEqual(await f.questions(taskId), originalQuestions); assert.deepEqual((await f.facts(taskId)), originalFacts);
  ...
  const code = (await f.take('code')); assert.notEqual(code.generation, docs.generation);
  assert.equal(code.repairId, receipt.repairId); assert.deepEqual(code.input.interactionRefs, []);
  const beforeOldFinish = (await f.head(taskId));
  await assert.rejects(f.app.execution.finish(originalCode, oldResult.complete), {code: 'recovery_required'});
  await assert.rejects(f.app.execution.dispatchAnswer(docs, question.questionId), {code: 'recovery_required'});
  await assert.rejects(f.app.execution.acknowledgeAnswer(docs, question.questionId, ack), {code: 'recovery_required'});
  assert.deepEqual((await f.head(taskId)), beforeOldFinish); assert.deepEqual((await f.facts(taskId)), originalFacts);
  ...
  assert.deepEqual((await f.facts(taskId)), originalFacts); assert.deepEqual((await f.refs(docs)), originalRefs);
  const beforeReplay = (await f.head(taskId));
  assert.equal((await f.call(answerRequest)).replayed, true); assert.deepEqual((await f.head(taskId)), beforeReplay);
