...
import {withReviewCriteria, reviewCriteria, reviewSources} from './review-assessment-contract.mjs';
import {startReviewWithAssessments} from './review-assessment.mjs';

export async function assessmentFixture(t, options = {}) {
  const f = fixture(t, {reviewAssessments: true, maxCalls: 16, verificationCheck() {}, ...options});
  ...
  assert.equal((await f.decision(f.take('leader'),[{type:'plan',proposal:withReviewCriteria(proposal)}])).status,'completed');
...
export async function requestReview(f) {
  const leader=f.take('leader'),selection=leader.input.leader.snapshot.selection;
