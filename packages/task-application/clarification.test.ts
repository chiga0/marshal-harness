...
async function fixture(t, options = {}) {
  ...
  let offset = 0, fault = false, store = Store.create(root), owner = await store.claimOwner(0, 'questions', Date.now() + 3600000);
...
test('one frozen batch, append-only answers/previews, one final approval and no planning/attempt obligation before approval', async t => {
  const f = await fixture(t), task = await f.create(), original = await f.questions(task.id);
  ...
  assert.equal((await f.read(tx => tx.commands())).length, 0); assert.equal((await f.call({operation: 'task.audit', taskId: task.id})).attempts, 0);
  ...
  assert.equal((await f.read(tx => tx.commands())).length, 0); assert.equal(f.hooks.renders, 3);
  const previews = (await f.read(tx => tx.projections('interaction'))).map(row => ({row, value: JSON.parse(row.bytes)})).filter(item => item.value.plan);
  ...
  const before = (await f.read(tx => tx.head(task.id)));
  ...
  assert.equal(replay.currentTask.revision, 3); assert.deepEqual((await f.read(tx => tx.head(task.id))), before);
  const approval = await f.call({operation: 'task.approve', taskId: task.id, key: 'approve', body: approvedBody(second.task)});
  assert.equal(approval.status, 'accepted'); assert.equal((await f.read(tx => tx.commands())).length, 1);
  const record = JSON.parse((await f.read(tx => tx.projection('task', task.id))).bytes);
  ...
  await assert.rejects(f.app.proposePlan(task.id, record.task.revision, plan()), {code: 'state_conflict'});
...
test('answer route is part of only the new receipt digest; wrong subject/key/CAS, old preview and invalid shape do not mutate', async t => {
  const f = await fixture(t), task = await f.create(), q = await f.questions(task.id), lang = bySlot(q, 'language'), fmt = bySlot(q, 'format');
  ...
  assert.deepEqual((await f.read(tx => tx.head(task.id))), head);
});

test('renderer/validator failures and boundary changes roll back answer, preview, Operation and receipt', async t => {
  const f = await fixture(t), task = await f.create(), q = await f.questions(task.id), body = answerBody(q, 'zh'), questionId = bySlot(q, 'language');
  const before = (await f.read(tx => ({head: tx.head(task.id), interactions: tx.projections('interaction').length, operations: tx.projections('operation').length})));
  ...
  assert.deepEqual((await f.read(tx => ({head: tx.head(task.id), interactions: tx.projections('interaction').length, operations: tx.projections('operation').length}))), before);
...
test('cold read/replay uses original facts; missing or changed renderer/validator rejects new writes without hiding history', async t => {
  ...
  for (const port of [null, template({}, {renderer: {...identity('changed-renderer'), render: plan}})]) {
    (await f.reopen(port)); assert.deepEqual(await f.questions(task.id), current); assert.equal(f.hooks.renders, calls);
  ...
  await f.reopen(); await f.answer(task.id, bySlot(current, 'format'), answerBody(current, 'csv'), 'format');
  const ready = await f.get(task.id); (await f.reopen(null));
...
test('cancel during out-of-transaction rendering wins, answer first keeps original cancel CAS, exact replay never revives Task', async t => {
  const f = await fixture(t), task = await f.create(), q = await f.questions(task.id), entered = deferred(), release = deferred();
  ...
  assert.equal((await f.read(tx => tx.projections('interaction'))).length, 1); assert.equal((await f.get(task.id)).status, 'cancelling');
...
test('original deadline and declaration limits hold; complete input stays on original zero-question planner path', async t => {
  const f = await fixture(t), task = await f.create(), q = await f.questions(task.id);
  ...
  assert.equal((await f.read(tx => tx.commands())).filter(command => command.taskId === complete.id).length, 1);
...
test('initial preview is atomic and finite; unsupported declarations or initial rendering never fall back to a planner', async t => {
  ...
  assert.deepEqual((await f.read(tx => [tx.projections('task'), tx.projections('interaction'), tx.projections('operation'), tx.commands()])), [[], [], [], []]);
  ...
  for (const change of [{applies: () => Promise.reject(Error('invalid async selection'))},
    ...
    const other = await fixture(t, {port: template({}, change)});
    await assert.rejects(other.create(), {code: 'unsupported_task'});
    assert.equal((await other.read(tx => tx.projections('task'))).length, 0); assert.equal((await other.read(tx => tx.commands())).length, 0);
  }
});
...
test('renderer wait cannot cross owner generation or original deadline; validator drift blocks writes and future facts fail closed', async t => {
  const f = await fixture(t), task = await f.create(), questions = await f.questions(task.id), entered = deferred(), release = deferred();
  ...
  f.hooks.render = async () => {entered.resolve(); await release.promise;};
  const oldApp = f.app, pending = oldApp.dispatch(request, principal); await entered.promise; await f.reopen(); release.resolve();
  await assert.rejects(pending, {code: 'recovery_required'});
  assert.deepEqual(await f.questions(task.id), questions); assert.equal((await f.read(tx => tx.projections('interaction'))).length, 1);
  delete f.hooks.render;
  (await f.reopen(template({}, {slots: ['language', 'format'].map(id => ({id, prompt: id === 'language' ? '输出使用 zh 还是 en？' : '内容使用 csv 还是 json？',
  ...
  await f.reopen(); f.hooks.render = () => {f.advance(60001);};
  await assert.rejects(f.call(request), {code: 'question_expired'}); assert.deepEqual((await f.read(tx => tx.projections('interaction'))).length, 1);
...
test('legacy approval predicate and legacy Task schema cannot accept clarification final preview; zero-question approval is unchanged', async t => {
  ...
  const record = JSON.parse((await f.read(tx => tx.projection('task', task.id))).bytes), body = approvedBody(record.task);
  ...
  const legacy = await f.create({...input(), intent: '原零问题'}, 'legacy'), p = await f.app.proposePlan(legacy.id, 1, plan());
...
test('real loopback HTTP + SQLite exposes frozen questions, exact subject receipt, final approval and cold query', {timeout: 15000}, async t => {
  const f = await fixture(t), token = 'public-clarification-http-fixture-token-000000'; let handler;
  ...
  await f.reopen(); const replay = await request(answerRoute, body, 'answer'); assert.equal(replay.status, 202); assert.equal(replay.body.replayed, true);
  ...
  assert.equal((await f.read(tx => tx.commands())).length, 1); assert.equal((await request(route + '/audit')).body.attempts, 0);
});
