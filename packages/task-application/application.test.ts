...
async function fixture(t, options = {}) {
  ...
  let owner = await store.claimOwner(0, 'server-1', NOW + 3600000);
  ...
  return {get app() { return app; }, get store() { return store; }, get owner() { return owner; },
    ...
    async counts() { return await store.read(owner, tx => ({commands: tx.commands(), tasks: tx.projections('task')})); },
    async reopen() {
      store.close(); store = Store.openExisting(root, {clock: () => now});
      owner = await store.claimOwner(owner.generation, 'server-next', now + 3600000);
...
async function planned(f) {
  ...
  const plan = await f.app.proposePlan(created.id, created.revision, proposal());
...
test('real SQLite: create, plan, exact confirmation, original receipt and outbox survive reopen', async t => {
  ...
  assert.ok((await f.counts()).commands.every(command => command.generation === 1n && command.status === 'pending'));
});

test('exact-key conflict, stale revision and forged plan each reject without partial events or dispatch', async t => {
  const f = await fixture(t), {task, approval} = await planned(f);
...
test('cancel is a durable fence and stop obligation, never manufactured terminal cleanup', async t => {
  ...
  assert.equal((await f.counts()).commands.filter(c => c.kind === 'stop').length, 1);
  await f.reopen(); assert.deepEqual(await f.call(command), op);
...
test('expired confirmation, invalid plan and graph without a plan have no authority', async t => {
  const f = await fixture(t), created = await f.call({operation: 'task.create', key: 'c', body: body()});
  await assert.rejects(f.call({operation: 'task.graph', taskId: created.id}), failure('plan_conflict', 409));
  await assert.rejects(f.app.proposePlan(created.id, 1, {...proposal(), edges: [{from: 'check', to: 'check'}]}), failure('invalid_plan_graph', 400));
  assert.equal((await f.call({operation: 'task.get', taskId: created.id})).revision, 1);
  const plan = await f.app.proposePlan(created.id, 1, proposal()); f.advance(60000);
...
test('read projection/event pagination and unknown usage reflect durable facts', async t => {
  const f = await fixture(t), {task, plan, approval} = await planned(f); await f.call(approval);
...
test('caller authentication is checked before any durable create or read', async t => {
  ...
  assert.deepEqual((await f.counts()), {commands: [], tasks: []});
});

test('maximum accepted plan scope is HTTP-consumable; excess scope preserves original Task and events', async t => {
  const f = await fixture(t), created = await f.call({operation: 'task.create', key: 'create', body: body()});
  const proposed = proposal(); proposed.nodes[0].scope = Array.from({length: 32}, (_, n) => 'scope-' + n);
  const plan = await f.app.proposePlan(created.id, 1, proposed);
  ...
  await assert.rejects(f.app.proposePlan(created.id, 2, proposed), failure('invalid_plan_node', 400));
