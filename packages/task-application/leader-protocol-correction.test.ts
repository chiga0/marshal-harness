...
import {fixture,proposal} from './leader.fixture.ts';
import {startLeaderWithJsonCorrection,leaderJsonFailure,prepareLeaderWithJsonCorrection} from './leader-protocol-correction.ts';
import {validate} from '../task-api/contract.ts';
const malformed='{"profile":"task-managed-leader/v1","actions":[}';
const create=async(f)=>await f.call({operation:'task.create',key:'create',body:{intent:'交付已知结果',limits:{timeoutMs:60000,maxAttempts:20,maxWorkers:3}}});
...
test('syntax rejection retains failed outcome and creates one budgeted successor; duplicate finish and cold reopen do not duplicate',async t=>{
 const f=await fixture(t,{protocolCorrection:true}),task=await create(f),first=await f.take('leader');
 ...
 const count=async()=>await f.read(tx=>tx.commands().filter(c=>c.status==='pending'&&JSON.parse(c.payload).action==='leader').length);
 assert.equal(await count(),1);(await f.app.execution.finish(first,f.results.get(first.workerId)));assert.equal(await count(),1);
 await f.reopen();await f.app.leader.recoverUnreserved?.(task.id);
 const pending=await f.read(tx=>tx.commands().filter(c=>c.status==='pending'&&c.generation===f.app.owner.generation&&JSON.parse(c.payload).action==='leader'));
 // Original unreserved recovery translates a prior-generation pending command.
 if(!pending.length)await f.app.execution.reconcile(task.id);
 const second=await f.take('leader');assert.ok(second);assert.notEqual(second.workerId,first.workerId);assert.equal(second.deadline,first.deadline);
 ...
 assert.equal((await f.read(tx=>f.app.get(tx,task.id))).attempts,2);assert.equal((await f.read(tx=>f.app.get(tx,task.id))).leader.calls,2);
...
test('second syntax failure, closed profile, empty text and valid wrong shape never loop',async t=>{
 for(const mode of ['second','disabled','empty','shape'])await t.test(mode,async t=>{
  const f=await fixture(t,{protocolCorrection:mode!=='disabled'}),task=await create(f);let ticket=await f.take('leader');
  await f.rawDecision(ticket,mode==='empty'?'':mode==='shape'?'{}':malformed);
  if(mode==='second'){ticket=await f.take('leader');await f.rawDecision(ticket,malformed);}
  const record=await f.read(tx=>f.app.get(tx,task.id));assert.equal(record.task.status,'cancelling');
...
async function originalResult(f,ticket,extra={}) {
 ...
 (await f.app.execution.started(ticket,await handle.started));return handle.completion;
}
...
test('known tools, permission request, empty text, suffix, duplicate-key, malformed receipt, unknown cleanup and wrong phase never become repair authority',async t=>{
 for(const mode of ['tool','permission','suffix','duplicate','unknown','provider-failed','receipt'])await t.test(mode,async t=>{
  const f=await fixture(t,{protocolCorrection:true}),task=await create(f),ticket=await f.take('leader');
  ...
  (await f.app.execution.finish(ticket,result));assert.equal((await view(f,task.id)).protocolCorrection.used,0);
 });
});
...
test('budget insufficient, cancellation, currentness change and expired task refuse successor without changing authority gates',async t=>{
 for(const mode of ['budget','cancel','stale','expired'])await t.test(mode,async t=>{
  const f=await fixture(t,{protocolCorrection:true});
  ...
  const ticket=await f.take('leader'),result=await originalResult(f,ticket);
  ...
  if(mode==='stale'){
   ...
   await f.app.transaction(true,tx=>{const value=f.app.get(tx,task.id);value.input.intent='new semantic input';value.inputDigest='sha256:'+'f'.repeat(64);value.task.revision++;f.app.save(tx,value,'fixture.changed',{});});
  }
  (await f.app.execution.finish(ticket,result));assert.equal((await view(f,task.id)).protocolCorrection.used,0);
 });
});
...
test('mixed syntax errors never bypass duplicate, depth, numeric, encoding or envelope exclusion',async t=>{
 ...
 for(const [name,text]of Object.entries(cases))await t.test(name,async t=>{const f=await fixture(t,{protocolCorrection:true}),task=await create(f),ticket=await f.take('leader');const result=await originalResult(f,ticket,{text});assert.equal(leaderJsonFailure(f.leaderPort,ticket,result),null);await f.app.execution.finish(ticket,result);assert.equal((await view(f,task.id)).protocolCorrection.used,0);});
});
test('permission-only facts with no caller callback or no tool progress remain ineligible',async t=>{
 for(const extra of [{permission:true,noPermissionCallback:true},{permissionDiagnostic:true}])await t.test(JSON.stringify(extra),async t=>{const f=await fixture(t,{protocolCorrection:true}),task=await create(f),ticket=await f.take('leader');const result=await originalResult(f,ticket,extra);assert.equal(leaderJsonFailure(f.leaderPort,ticket,result),null);await f.app.execution.finish(ticket,result);assert.equal((await view(f,task.id)).protocolCorrection.used,0);});
});
test('correction preparation is captured byte-for-byte before Provider start',async t=>{
 const f=await fixture(t,{protocolCorrection:true,preparePrompt:'完整受控输入',observability:{profile:'task-observation/v1',retainPrompts:true}}),task=await create(f),first=await f.take('leader');await f.rawDecision(first,malformed);const ticket=await f.take('leader');
 const prepared=await prepareLeaderWithJsonCorrection(f.leaderPort,ticket,{cwd:f.parent},{});
 assert.match(prepared.prompt,/协议格式纠错/);
 await f.app.execution.observeInput(ticket,'prepared',prepared.prompt);await f.app.execution.observeInput(ticket,'handed-off');
...
test('real SIGKILL before/inside/after finish commit and after claim/start never remints private classification or duplicates a handle',async t=>{
 ...
 for(const phase of ['before-finish','inside-commit','after-commit','claimed','started'])await t.test(phase,async t=>{
  const child=fork(new URL('./leader-protocol-correction-crash.fixture.ts',import.meta.url),[phase],{stdio:['ignore','ignore','ignore','ipc']});let state={};child.on('message',value=>Object.assign(state,value));
  const [,signal]=await once(child,'exit');assert.equal(signal,'SIGKILL');assert.ok(state.parent);
  const f=await fixture(t,{protocolCorrection:true,existingParent:state.parent});
  let value=await view(f,state.taskId),record=await f.read(tx=>f.app.get(tx,state.taskId));
  ...
  if(phase==='after-commit'){
   ...
   const snapshot=await f.read(tx=>({head:tx.head(state.taskId),attempts:f.app.get(tx,state.taskId).attempts}));
   await f.app.execution.reconcile(state.taskId);await f.app.leader.recover(state.taskId);
   assert.deepEqual(await f.read(tx=>({head:tx.head(state.taskId),attempts:f.app.get(tx,state.taskId).attempts})),snapshot);
   assert.equal(await f.read(tx=>tx.commands().filter(c=>c.status==='pending'&&JSON.parse(c.payload).action==='leader').length),0);
...
test('pending correction cancellation does not reserve a new attempt and does not refund budget',async t=>{
 const f=await fixture(t,{protocolCorrection:true}),task=await create(f),first=await f.take('leader');await f.rawDecision(first,malformed);
 await f.call({operation:'task.cancel',taskId:task.id,key:'cancel',body:{expectedRevision:(await f.get(task.id)).revision}});
 assert.equal(await f.take('leader'),null);const record=await f.read(tx=>f.app.get(tx,task.id));assert.equal(record.attempts,1);assert.equal(record.leader.protocolCorrection.used,1);assert.equal(record.leader.protocolCorrection.successorWorkerId,null);
});
test('unknown or stopping execution cannot gain syntax successor authority from a late valid receipt',async t=>{
 for(const status of ['unknown','stopping'])await t.test(status,async t=>{
  const f=await fixture(t,{protocolCorrection:true}),task=await create(f),ticket=await f.take('leader'),result=await originalResult(f,ticket);
...
test('observed real failure closing-bracket pattern remains eligible without extracting actions',async t=>{
 ...
 assert.equal(leaderJsonFailure(f.leaderPort,ticket,result)?.stage,'wire-json');await f.app.execution.finish(ticket,result);assert.equal((await view(f,task.id)).protocolCorrection.used,1);
});
