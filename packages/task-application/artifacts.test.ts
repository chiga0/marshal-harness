...
async function fixture(t) {
  ...
  let owner = await store.claimOwner(0, 'server', Date.now() + 3600000);
  ...
  return {get app() { return app; }, get depot() { return depot; }, get store() { return store; }, objects,
    ...
    async reopen() {
      ...
      owner = await store.claimOwner(owner.generation, 'next-server', Date.now() + 3600000);
...
test('real SQLite/depot input upload, exact idempotency, binding and cold reopen', async t => {
  const f = await fixture(t), req = upload(), artifact = await f.call(req);
  ...
  assert.equal((await f.read(tx => tx.commands())).length, 1);
  ...
  assert.equal((await f.read(tx => tx.projections('artifact'))).length, 3);
});
...
test('unknown/duplicate refs and malformed uploads never create Task or publish objects', async t => {
  ...
  assert.equal((await f.read(tx => tx.projections('task'))).length, 0);
  assert.equal((await f.read(tx => tx.projections('artifact'))).length, 0);
  assert.equal((await f.read(tx => tx.commands())).length, 0);
});
...
test('failed SQL commit leaves only orphan bytes, never a manifest or receipt', async t => {
  const f = await fixture(t), req = upload();
...
test('real HTTP + TaskClient upload, input binding and verified binary download', {timeout: 10000}, async t => {
  const f = await fixture(t), token = 'public-artifacts-http-test-token-0000';
  let handler;
