...
import {assessmentFixture, assessmentProposal, produceReview, requestReview} from './review-assessment.fixture.mjs';
import {registerReviewAssessments, reviewAssessmentsEnabled, reviewAssessmentEvidence} from './review-assessment.mjs';
import {receipt, configuration, createReviewPort, parseManagedOutput} from './leader-ports.mjs';
import {fixture,hash} from './leader.fixture.mjs';
...
test('one original Provider completion and receipt bind a v2 Artifact; the original receipt survives the controller outcome container without minting new authority',async t=>{
  ...
  const before=f.read(tx=>({head:tx.head(f.taskId),attempts:f.app.get(tx,f.taskId).attempts}));
  f.app.execution.finish(ticket,native.result);assert.deepEqual(f.read(tx=>({head:tx.head(f.taskId),attempts:f.app.get(tx,f.taskId).attempts})),before);
  f.reopen();assert.deepEqual(await evidence(f,f.taskId),stored);assert.equal(native.calls,1);
});
...
test('a mapper returning a different valid original report cannot attach captured checks to it',async t=>{
  ...
  assert.equal(f.app.execution.finish(ticket,native.result).status,'failed');assert.equal((await view(f,f.taskId)).review,null);
});
...
test('Depot staging failure publishes no partial assessment and retrying finish uses the original Provider result once',async t=>{
  const f=await assessmentFixture(t),ticket=f.reviewTicket,native=await produceReview(f,ticket,assessmentProposal(ticket));
  const stage=f.app.artifacts.stageOutputs.bind(f.app.artifacts),before=f.read(tx=>tx.head(f.taskId));
  f.app.artifacts.stageOutputs=()=>{throw new Error('controlled_depot_failure');};
  assert.throws(()=>f.app.execution.finish(ticket,native.result),/controlled_depot_failure/);
  assert.deepEqual(f.read(tx=>tx.head(f.taskId)),before);assert.equal((await view(f,f.taskId)).review,null);
  f.app.artifacts.stageOutputs=stage;assert.equal(f.app.execution.finish(ticket,native.result).status,'completed');assert.equal(native.calls,1);
});
...
test('invalid checks, failed/cancelled Provider and unconfirmed cleanup cannot create a v2 acceptance',async t=>{
  for(const mode of ['missing-check','unknown-accept','foreign-quote','failed','cancelled','unknown-cleanup'])await t.test(mode,async t=>{
    ...
    assert.equal(reviewAssessmentEvidence(f.reviewPort,ticket,native.result),null);
    f.app.execution.finish(ticket,native.result);assert.equal((await view(f,f.taskId)).review,null);assert.equal(native.calls,1);
    if(['failed','cancelled'].includes(mode))assert.equal(f.read(tx=>f.app.get(tx,f.taskId)).failureCode,'leader_result_rejected');
  });
});
...
test('cancellation, expiration and changed semantic readset preserve original gates after a valid assessment',async t=>{
  for(const mode of ['cancel','deadline','stale'])await t.test(mode,async t=>{
    ...
    f.app.execution.finish(ticket,native.result);assert.equal((await view(f,f.taskId)).review,null);
  });
});
...
test('original rework evidence selects affected nodes, then a fresh selection receives a new complete assessment',async t=>{
  ...
  const leader=f.take('leader');await f.decision(leader,[{type:'repair',nodeIds:['east'],basis:{kind:'review',digest:review.digest},feedback:'按原拒收补齐规则'}]);
  const repair=f.take('execute','east');assert.equal(repair.input.repair.evidence.digest,(await f.call({operation:'artifact.get',artifactId:review.evidenceIds[0]})).digest);
  f.author(repair);const second=await requestReview(f);
  ...
  const record=f.read(tx=>f.app.get(tx,f.taskId));assert.equal(record.reworkCount,1);assert.equal(record.leader.repairRounds,1);
  ...
  f.reopen();assert.deepEqual(await evidence(f,f.taskId),accepted);
});
...
test('direct Application reopen rejects an immutable assessment identity toggle without rewriting the task',async t=>{
  for(const enabled of [false,true])await t.test(String(enabled),async t=>{
    ...
    original.closeStore();
    const opened=fixture(t,{existingParent:original.parent,reviewAssessments:!enabled,maxCalls:16,verificationCheck(){}});
...
test('bound v2 Review continues through the original verification, delivery and completion and survives cold reopen',async t=>{
  ...
  let ticket=f.take('leader');assert.ok(ticket.input.leader.materials.some(material=>material.content.includes('task-independent-review/v2')));
  ...
  const artifact=f.read(tx=>current.artifactIds.map(id=>f.app.artifacts.metadata(tx,id)).find(row=>row.kind==='delivery'));
  assert.equal((await f.decision(f.take('leader'),[{type:'deliver',artifactId:artifact.id,acceptanceDigest:audit.acceptance.digest,reviewDigest:review.digest}])).status,'completed');
  assert.equal((await f.decision(f.take('leader'),[{type:'conclude',outcome:'succeeded',summary:'原独立检查完成',basisDigests:[audit.acceptance.digest,review.digest]}])).status,'completed');
  assert.equal((await f.get(f.taskId)).status,'completed');const before=f.read(tx=>tx.head(f.taskId));
  f.reopen();assert.equal((await f.get(f.taskId)).status,'completed');assert.deepEqual(f.read(tx=>tx.head(f.taskId)),before);assert.deepEqual(await evidence(f,f.taskId),stored);
});
...
test('real SIGKILL before finish, after staging, inside transaction and after commit never remints evidence or repeats Review',async t=>{
  ...
  for(const phase of ['before-finish','after-staging','inside-transaction','after-commit'])await t.test(phase,async t=>{
    const child=fork(new URL('./review-assessment-crash.fixture.mjs',import.meta.url),[phase],{stdio:['ignore','ignore','ignore','ipc']});
    ...
    if(phase==='after-staging'){
      ...
      const bytes=f.app.artifacts.bytes(staged[0].ref);assert.equal(JSON.parse(bytes.toString()).profile,'task-independent-review/v2');
      assert.equal(f.read(tx=>tx.projections('artifact').some(row=>JSON.parse(row.bytes).artifact?.digest===staged[0].ref.digest)),false);
    }
    if(committed){
      ...
      f.app.leader.recover(state.taskId);
      const leader=f.take('leader');assert.equal(leader.executionType,'leader');
      assert.equal(f.read(tx=>tx.commands().filter(row=>row.status==='pending'&&JSON.parse(row.payload).action==='review').length),0);
    }else{
      ...
      const after=f.read(tx=>({head:tx.head(state.taskId),attempts:f.app.get(tx,state.taskId).attempts}));
      f.app.execution.reconcile(state.taskId);f.app.leader.recover(state.taskId);
      assert.deepEqual(f.read(tx=>({head:tx.head(state.taskId),attempts:f.app.get(tx,state.taskId).attempts})),after);
      assert.equal(f.read(tx=>tx.commands().filter(row=>row.status==='pending'&&JSON.parse(row.payload).action==='review').length),0);
    }
  });
});
