...
async function fixture(t, qualified = false, format = WORKER_CANCELLATION_FORMAT) {
  ...
  const f = {get app() {return app;}, get store() {return store;}, root,
    ...
    async reserve(key = 'new') {const task = await f.create(key), command = (await f.commands()).find(c => c.taskId === task.id); return {task, ticket: (await app.execution.nextWork(command.id, command.revision))};},
    async pair() {const task = await f.create(); const frozen = await app.proposePlan(task.id, task.revision, plan);
      ...
      return {task, a: (await app.execution.nextWork(commands.find(c => JSON.parse(c.payload).nodeId === 'a').id, 1)),
        b: (await app.execution.nextWork(commands.find(c => JSON.parse(c.payload).nodeId === 'b').id, 1))};},
...
test('legal large plans cancel with bounded reads and retain completed independent branches', async t => {
  for (const {chain, siblings} of [{chain: 13, siblings: 13}, {chain: 30, siblings: 16}])
    await t.test(`${chain + siblings + 2} nodes`, async t => {
      const f = await fixture(t), descendants = Array.from({length: chain}, (_, i) => 'c' + i),
      ...
      const plannerCommand = (await f.commands()).find(c => c.taskId === task.id),
        planner = (await f.app.execution.nextWork(plannerCommand.id, plannerCommand.revision)), sp = started(planner);
      (await f.app.execution.started(planner, sp)); (await f.app.execution.finish(planner, {...result(sp), plan: proposed}));
      ...
      assert.equal((await f.app.execution.expandDispatch(dispatch.id, dispatch.revision)), true);
      const reserve = async nodeId => {const c = (await f.commands()).find(c => JSON.parse(c.payload).nodeId === nodeId);
        const ticket = (await f.app.execution.nextWork(c.id, c.revision)); assert.ok(ticket); return ticket;};
      const a = await reserve('a'), sa = started(a); (await f.app.execution.started(a, sa));
      for (const nodeId of independent) {
        const ticket = await reserve(nodeId), fact = started(ticket); (await f.app.execution.started(ticket, fact));
        assert.equal((await f.app.execution.finish(ticket, result(fact))).status, 'completed');
      }
      const before = (await f.snapshot(task.id)), retained = before.workers.filter(w => independent.includes(w.worker.nodeId));
      ...
      assert.deepEqual((await f.app.execution.reconcile(task.id)).stopWorkerIds, [a.workerId]);
      const cancelling = (await f.snapshot(task.id));
      ...
      assert.equal((await f.app.execution.finish(a, result(sa))).status, 'cancelled');
      const final = (await f.snapshot(task.id));
      ...
      await f.reopen(); assert.deepEqual(await f.app.dispatch(request, context), receipt);
      assert.deepEqual((await f.snapshot(task.id)), final);
    });
});
test('target receipt/CAS, late result, retained sibling and dependency close use one original reducer', async t => {
  const f = await fixture(t), {task, a, b} = await f.pair(), sa = started(a), sb = started(b);
  (await f.app.execution.started(a, sa)); (await f.app.execution.started(b, sb));
  const before = (await f.snapshot(task.id)), {request, receipt} = await f.cancel(a);
  assert.equal(validate(receipt, 'Operation'), true); assert.equal(receipt.workerId, a.workerId);
  assert.equal((await f.app.execution.mayStart(a)), false); assert.deepEqual((await f.app.execution.reconcile(task.id)).stopWorkerIds, [a.workerId]);
  ...
  assert.equal((await f.snapshot(task.id)).capacity.active.length, 1); assert.equal((await f.snapshot(task.id)).workers.find(w => w.worker.id === a.workerId).resultRef, null);
  (await f.app.execution.finish(b, result(sb)));
  const final = (await f.snapshot(task.id)); assert.equal(final.task.task.status, 'failed'); assert.equal(final.task.task.code, 'worker_cancelled');
  ...
  await f.reopen(); assert.deepEqual(await f.app.dispatch(request, context), receipt); assert.deepEqual((await f.snapshot(task.id)), final);
});
test('cancelled planner cannot publish a plan', async t => {
  const f = await fixture(t), {task, ticket} = await f.reserve(), {receipt} = await f.cancel(ticket);
  assert.equal((await f.app.execution.started(ticket, started(ticket))).stop, true);
  const fact = (await f.snapshot(task.id)).workers[0];
  (await f.app.execution.finish(ticket, {...result({executionId: fact.executionId, startedAt: fact.worker.startedAt}), plan}));
...
test('global cancel can win remaining siblings, while deadline/new-key/cross-Task requests cannot retarget original receipt', async t => {
  const f = await fixture(t), {task, a, b} = await f.pair(), sa = started(a), sb = started(b);
  (await f.app.execution.started(a, sa)); (await f.app.execution.started(b, sb)); const target = await f.cancel(a);
  await assert.rejects(f.cancel(a, 'new-key'), {code: 'state_conflict'});
  (await f.app.execution.finish(a, result(sa))); // Free only A before reserving another Task.
  const other = await f.create('other-task'), plannerCommand = (await f.commands()).find(c => c.taskId === other.id);
  const otherTicket = (await f.app.execution.nextWork(plannerCommand.id, plannerCommand.revision));
  ...
  await assert.rejects(f.cancel(b), {code: 'state_conflict'});
  (await f.app.execution.finish(b, result(sb))); (await f.app.execution.reconcile(task.id));
  const command = (await f.commands()).find(c => JSON.parse(c.payload).operationId === global.id); (await f.app.execution.settleControl(command.id, command.revision));
  ...
  const g = await fixture(t), reserved = await g.reserve(); g.advance(45001);
...
test('target cleanup mismatch never releases unknown capacity or forges a successful Operation', async t => {
  const f = await fixture(t), {ticket, task} = await f.reserve(), fact = started(ticket); (await f.app.execution.started(ticket, fact));
  const {receipt} = await f.cancel(ticket), before = (await f.snapshot(task.id));
  await assert.rejects(f.app.execution.finish(ticket, result({...fact, executionId: 'foreign'})));
  assert.deepEqual((await f.snapshot(task.id)), before); assert.equal((await f.query('operation.get', {operationId: receipt.id})).status, 'accepted');
});
test('named never-permitted target settles independently from a bound sibling, with exact original signed cleanup and cold replay', {timeout: 15000}, async t => {
  const f = await fixture(t, true), {task, a, b} = await f.pair(), directory = path.join(path.dirname(f.root), 'custody'); fs.mkdirSync(directory, {mode: 0o700});
  ...
  const handle = await manager.prepare(f.app.execution.custodyBinding(b, profile)); (await f.app.execution.bindCustody(b, handle.descriptor, profile));
  await handle.stop(); const observation = manager.read(handle.descriptor); assert.equal(observation.payload.permitReceived, false);
  const {request, receipt} = await f.cancel(a); (await f.reopen(false)); (await f.app.execution.reconcile(task.id));
  const stop = (await f.commands()).find(c => JSON.parse(c.payload).operationId === receipt.id); (await f.app.execution.settleControl(stop.id, stop.revision));
  assert.equal((await f.query('operation.get', {operationId: receipt.id})).status, 'unknown');
  (await f.app.execution.settleUnpermitted(a.workerId)); const interim = (await f.snapshot(task.id));
  assert.equal(interim.capacity.active.length, 1); assert.equal(interim.task.task.status, 'intervention');
  assert.equal((await f.query('operation.get', {operationId: receipt.id})).status, 'succeeded');
  await assert.rejects(f.app.execution.settleUnpermitted(b.workerId), {code: 'recovery_required'});
  (await f.app.execution.reconcileCleanup(b.workerId, observation)); assert.equal((await f.snapshot(task.id)).capacity.active.length, 0);
  const final = (await f.snapshot(task.id)); (await f.reopen(false)); (await f.app.execution.settleUnpermitted(a.workerId)); (await f.app.execution.reconcileCleanup(b.workerId, observation));
  assert.deepEqual((await f.snapshot(task.id)), final); assert.deepEqual(await f.app.dispatch(request, context), receipt);
});
test('target stop proof corruption and original extra scope fail closed without releasing capacity', {timeout: 15000}, async t => {
  const f = await fixture(t), {ticket, task} = await f.reserve(), directory = path.join(path.dirname(f.root), 'custody'); fs.mkdirSync(directory, {mode: 0o700});
  const manager = createExecutionCustody({root: directory}); t.after(() => manager.close());
  const profile = {id: 'recorded-scope', scope: 'inherited-process-group', eligible: true};
  const handle = await manager.prepare(f.app.execution.custodyBinding(ticket, profile)); (await f.app.execution.bindCustody(ticket, handle.descriptor, profile));
  (await f.app.execution.recordExtraScope(ticket, 'untracked-effect')); const {receipt} = await f.cancel(ticket); await handle.stop();
  const observation = manager.read(handle.descriptor); await f.reopen(); (await f.app.execution.reconcile(task.id));
  const before = (await f.snapshot(task.id)); await assert.rejects(f.app.execution.reconcileCleanup(ticket.workerId, observation), {code: 'recovery_required'});
  assert.deepEqual((await f.snapshot(task.id)), before);
  const stop = (await f.commands()).find(c => JSON.parse(c.payload).operationId === receipt.id); (await f.app.execution.settleControl(stop.id, stop.revision));
  assert.equal((await f.query('operation.get', {operationId: receipt.id})).status, 'unknown');
  const g = await fixture(t, true), reserved = await g.reserve(); await g.cancel(reserved.ticket);
  // Explicit corrupt projection negative, not a receipt or execution producer.
  (await g.app.transaction(true, tx => {const {row, record} = g.app.execution.worker(tx, reserved.ticket.workerId), task = g.app.get(tx, reserved.task.id);
  ...
  await g.reopen(); const corrupt = (await g.snapshot(reserved.task.id)); await assert.rejects(g.app.execution.settleUnpermitted(reserved.ticket.workerId), {code: 'recovery_required'});
  assert.deepEqual((await g.snapshot(reserved.task.id)), corrupt);
});
test('target unknown retains capacity/operation; cross-worker route never replays another receipt', async t => {
  const f = await fixture(t), {task, a, b} = await f.pair(), sa = started(a), sb = started(b);
  (await f.app.execution.started(a, sa)); (await f.app.execution.started(b, sb)); const {request, receipt} = await f.cancel(a);
  await assert.rejects(f.app.dispatch({...request, workerId: b.workerId}, context), {code: 'revision_conflict'});
  (await f.app.execution.finish(a, {...result(sa), cleanup: {started: sa, cleaned: false}}));
  assert.equal((await f.query('operation.get', {operationId: receipt.id})).status, 'unknown');
  assert.equal((await f.snapshot(task.id)).capacity.active.length, 2); assert.equal((await f.query('task.get', {taskId: task.id})).status, 'intervention');
  await f.reopen(); assert.deepEqual(await f.app.dispatch(request, context), receipt); await assert.rejects(f.cancel(b), {code: 'state_conflict'});
});
test('v6 original protocol survives configuration removal; unqualified reservation cannot gain it on reopen', async t => {
  for (const qualified of [false, true]) await t.test(String(qualified), async t => {
    const f = await fixture(t, qualified), {task, ticket} = await f.reserve(), {request, receipt} = await f.cancel(ticket), before = (await f.snapshot(task.id));
    (await f.reopen(!qualified)); (await f.app.execution.reconcile(task.id));
    const stop = (await f.commands()).find(c => JSON.parse(c.payload).action === 'worker-cancel'); (await f.app.execution.settleControl(stop.id, stop.revision));
    assert.equal((await f.query('operation.get', {operationId: receipt.id})).status, 'unknown');
    if (qualified) {
      (await f.app.execution.settleUnpermitted(ticket.workerId)); const final = (await f.snapshot(task.id));
      assert.equal(final.workers[0].cleanup, null); assert.equal(final.workers[0].worker.status, 'cancelled');
      assert.equal(final.capacity.active.length, 0); assert.equal(final.task.attempts, before.task.attempts);
      assert.equal((await f.query('operation.get', {operationId: receipt.id})).status, 'succeeded');
      (await f.reopen(false)); (await f.app.execution.settleUnpermitted(ticket.workerId)); assert.deepEqual((await f.snapshot(task.id)), final);
    } else {await assert.rejects(f.app.execution.settleUnpermitted(ticket.workerId), {code: 'recovery_required'}); assert.equal((await f.snapshot(task.id)).capacity.active.length, 1);}
    assert.deepEqual(await f.app.dispatch(request, context), receipt);
...
test('new cancellation rolls back completely; old format remains 501 and old Operation bytes stay closed', async t => {
  const f = await fixture(t), {ticket, task} = await f.reserve(), current = await f.query('task.get', {taskId: task.id}), before = (await f.snapshot(task.id));
  ...
  f.store.write = write; assert.deepEqual((await f.snapshot(task.id)), before); await f.cancel(ticket);
  const old = await fixture(t, false, FORMAT), original = await old.reserve();
