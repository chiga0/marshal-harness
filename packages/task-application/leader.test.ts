...
export async function fixture(t, options = {}) {
  ...
  let store = Store.create(root, {format: LEADER_FORMAT}), owner = await store.claimOwner(0, 'test', Date.now() + 3600000), serial = 0;
...
test('v7 real SQLite: necessary reply → plan approval → two authors → independent Review → stage verification → deliver/conclude → cold exact bytes', async t => {
  const f = await fixture(t), task = await f.call({operation: 'task.create', key: 'create', body: {intent: '交付两区域结果，但区域待用户明确',
  ...
  ticket = (await f.take('leader')); assert.equal(ticket.deadline, originalDeadline); assert.equal(ticket.input.leader.snapshot.interactions.replies[0].answer, 'north');
  ...
  const dispatch = (await f.read(tx => tx.commands().find(command => JSON.parse(command.payload).action === 'dispatch')));
  (await f.app.execution.expandDispatch(dispatch.id, dispatch.revision));
  const east = (await f.take('execute', 'east')), west = (await f.take('execute', 'west')); assert.ok(east && west); assert.equal((await f.take('execute', 'verify')), null);
  (await f.author(east)); (await f.author(west)); assert.equal((await f.take('execute', 'verify')), null);
  ticket = (await f.take('leader')); const selected = ticket.input.leader.snapshot.selection;
  ...
  const delivery = (await f.read(tx => current.artifactIds.map(id => f.app.artifacts.metadata(tx, id)).find(item => item.kind === 'delivery')));
  assert.deepEqual(audit.firstReview, {passed: 0, total: 0, pending: 0}); assert.equal(view.review.verdict, 'accept');
  ticket = (await f.take('leader')); assert.equal((await f.decision(ticket, [{type: 'deliver', artifactId: delivery.id,
    acceptanceDigest: audit.acceptance.digest, reviewDigest: view.review.digest}])).status, 'completed');
  ticket = (await f.take('leader')); assert.equal((await f.decision(ticket, [{type: 'conclude', outcome: 'succeeded', summary: '两原分支独立检查后完整交付',
  ...
  const before = (await f.read(tx => tx.head(task.id))), bytes = await f.call({operation: 'artifact.content', artifactId: delivery.id});
  assert.equal((await f.read(tx => f.app.execution.capacity(tx).value.active.length)), 0);
  await f.reopen(); assert.deepEqual(await f.get(task.id), current); assert.deepEqual((await f.read(tx => tx.head(task.id))), before);
