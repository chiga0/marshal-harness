...
test('真实漏转义引号与多语言同形缺陷仅消费一次原纠错预算',async t=>{
  ...
  for(const text of [publicOutput,'{"summary":"title "Reading Club" today"}','{"summary":"titre "Café lecture" ici"}'])await t.test(text.slice(0,35),async t=>{
    const f=fixture(t,{protocolCorrection:true});const task=await f.call({operation:'task.create',key:'create',body:{intent:'原目标',limits:{timeoutMs:60000,maxAttempts:20,maxWorkers:3}}});
    const first=f.take('leader');await f.rawDecision(first,text);
    ...
    const next=f.take('leader');await f.rawDecision(next,text);assert.equal((await f.call({operation:'task.leader',taskId:task.id})).protocolCorrection.used,1);
...
test('漏引号混合硬错误与不明确词法仍拒绝纠错，不截断或补全',async t=>{
  ...
  for(const text of [
    ...
    const f=fixture(t,{protocolCorrection:true});const task=await f.call({operation:'task.create',key:'create',body:{intent:'原目标',limits:{timeoutMs:60000,maxAttempts:20,maxWorkers:3}}});
    await f.rawDecision(f.take('leader'),text);assert.equal((await f.call({operation:'task.leader',taskId:task.id})).protocolCorrection.used,0);
  });
});
