...
const sourceDigest = 'sha256:' + createHash('sha256').update(fs.readFileSync(new URL(import.meta.url))).update(fs.readFileSync(new URL('./leader.mjs',import.meta.url))).update(fs.readFileSync(new URL('../task-service/composition.mjs',import.meta.url))).digest('hex');
