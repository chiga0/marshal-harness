...
function fixture(t, maxWorkers = 2) {
  ...
  let store = Store.create(root, {clock: () => now}), owner = store.claimOwner(0, 'server', now + 3600000);
  const execution = {maxWorkers, providerIds: ['fixture'], defaultProvider: 'fixture'};
  let app = new TaskApplication({store, owner, clock: () => now, execution});
  ...
  return {get app() {return app;}, get execution() {return app.execution;},
    ...
    reopen() {store.close(); store = Store.openExisting(root, {clock: () => now}); owner = store.claimOwner(owner.generation, 'next', now + 3600000);
      app = new TaskApplication({store, owner, clock: () => now, execution});}};
}
