...
export class TaskExecutionCoordinator {
  ...
  async #run(entry) {
    ...
    try {
      const typed = ['leader', 'review', 'publication', 'postverify'].includes(entry.ticket.executionType);
      const prepared = await this.#bounded(entry, this.#options.prepareMs, context =>
        typed ? this.#managed.prepare(entry.ticket, context) : this.#prepare(entry.ticket, context));
      ...
      const provider = verifying ? this.#verification : typed ? this.#managed.provider(entry.ticket) : this.#providers.get(entry.ticket.providerId);
