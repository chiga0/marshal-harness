...
- `create` 只创建尚不存在的独立根，已存在的空目录也不接管。父目录必须已存在、归当前用户所有且路径 canonical；根目录为 `0700`，格式与对象文件为 `0600`。旧根、缺失/部分 bootstrap、不兼容 format 拒绝打开，无自动迁移。
...
- 写入 `.pending-<uuid>` 非执行数据，完成文件 `fsync` 后以硬链接原子无覆盖安装，再移除本次临时链接并 `fsync` 根目录。初始化还同步父目录。重复 `put` 必须重新校验原 bytes，不能覆盖损坏对象，并同步对象与目录后返回。
...
定向验证：`node --test packages/task-artifacts/depot.test.mjs`。测试使用真实临时数据文件与确定性 I/O 故障注入，无模型、原生编译、SQLite 替代状态或实机交付完成声明。
