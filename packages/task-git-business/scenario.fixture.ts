...
import {gitDescription, PATCH, CONTEXT} from './index.ts';
...
export function proposal() {
  return {summary: 'library 折扣计算与 client 发票组合，两个仓库并行修改后独立验收',
    nodes: [
      {id: 'library', role: 'author', goal: 'net(cents,discount) 校验非负 safe integer 且 discount<=cents，返回差值，非法 throw。', scope: ['net.ts'], providerId: null},
      {id: 'client', role: 'author', goal: 'invoice(rows,net) 返回 {lines,total}，逐行调用 net，累计金额必须为 safe integer，非法 throw。', scope: ['invoice.ts'], providerId: null},
