...
export function normalizedDiagnostic(value) {
  ...
  const codes = value.source === 'provider-permission' && value.stage === 'permission' ?
    ...
    ({preparing:['preparation_failed','deadline_exceeded'],starting:['provider_start_failed','deadline_exceeded'],provider:['provider_failed','deadline_exceeded'],protocol:['invalid_json','invalid_leader_result','invalid_leader_decision','invalid_review_report'],collecting:['collection_failed','deadline_exceeded',...FILE_COLLECTION_CAUSES,...BUSINESS_COLLECTION_CAUSES],cleanup:['cleanup_unconfirmed','deadline_exceeded']}[value.stage] ?? []) : [];
