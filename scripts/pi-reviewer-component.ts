...
export async function configuration({installed,manifest,executable,sdkEntry}) {
 ...
 const {createPiProvider}=await load('packages/agent-provider-pi/index.ts');
 const {createGenericFilesReviewWireConfig}=await load('packages/task-generic-files/review-wire.ts');
 const {receipt,safeManagedDiagnostic}=await load('packages/task-application/leader-ports.ts');
...
export async function prepare(options) {
 ...
 const {launchProtocol}=await load('packages/agent-runtime/index.ts');const {BRIDGE_PROFILE,BRIDGE_ENV}=await load('packages/agent-provider-pi/bridge-contract.ts');
 ...
 const bridge=path.join(installed,'packages/agent-provider-pi/native-bridge.ts'),events=[],sent=[];let state,ready,runtime,problem;
 try {
