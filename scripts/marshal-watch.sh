...
#   --json 输出按优先级排序的行动队列：generatedAt 与 items，
#   每个 item 含 runId、state、priority、action、ageSeconds、processOwnership。
...
for run_id in sorted(os.listdir(runs_dir)):
    ...
    try:
        ...
        continue
    owned = state == "RUNNING" and any(owner_present(line, run_id) for line in procs)
    item = None
    if state == "RUNNING":
        if owned:
...
if mode == "text":
    print("[%s] %s" % (datetime.now().strftime("%m-%d %H:%M:%S"), " ".join(text_tokens)))
else:
    print(json.dumps({"generatedAt": now_utc.strftime("%Y-%m-%dT%H:%M:%SZ"), "items": items},
                     ensure_ascii=False))
if summary_path:
    try:
