...
import {cases,validateDelivery,hasInvitationDate,SemanticReviewRequired} from './experience-cases.ts';
import {readBoundReview,pendingAnswerGap,finalReviewer} from './experience-review-evidence.ts';
import {captureFailureProjections} from './experience-failure-evidence.ts';
import {staticInvitationControls} from './experience-static-invitation.ts';
import {captureScreenshot} from './experience-screenshot.ts';
...
const {reviewCriteria,validateStoredReviewAssessment}=await import(pathToFileURL(path.join(installed,'packages/task-application/review-assessment-contract.ts')).href);
...
const evidence={caseId,candidate:admission.sourceHead,manifestDigest:opts['--manifest'],runtime,
  ...
  supportingScriptDigests:Object.fromEntries(['experience-cases.ts','experience-failure-evidence.ts','experience-review-evidence.ts','experience-static-invitation.ts','experience-screenshot.ts']
...
try {
  persist();
  const initialize=spawnSync(process.execPath,[path.join(installed,'packages/task-local/main.ts'),'init','--settings-dir',settings],
