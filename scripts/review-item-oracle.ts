...
import {validateReviewAssessment} from '../packages/task-application/review-assessment-contract.ts';
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href) {
  ...
  if(!caseId||!resultFile||!inputFile||extra.length)throw new Error('usage: node scripts/review-item-oracle.ts CASE RESULT_JSON FROZEN_INPUT_JSON');
