...
import {validateExample, digest} from './review-assessment-cases.mjs';
...
export async function evaluate({config,api,input,cwd,deadlineMs=180000,cleanupMs=10000,onPrepared=()=>{}}) {
  ...
  const activity=[]; let extraBehavior=false,managedDiagnostic=null,cleanupStatus='unconfirmed';
...
export async function main(argv) {
  ...
  const api=Object.assign({},...await Promise.all(['packages/task-application/leader-ports.mjs','packages/task-application/review-assessment-contract.mjs',
    'packages/task-application/review-assessment.mjs','packages/task-generic-files/layout.mjs','packages/agent-observation/normalization.mjs'].map(load)));
  process.env.MARSHAL_AGENT_EXECUTABLE=opts['--agent'];
  const {default:config,QWEN_MANAGED_ARGS}=await load('packages/task-generic-files/qwen-review-service-config.mjs');
