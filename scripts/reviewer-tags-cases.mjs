...
const authority='数据来源与恢复边界：原Markdown文件是正文及源内标签的权威来源；工具内用户创建、重命名、归并的标签及挂接关系是独立的用户创造数据，保存在authoritative.sqlite的user_tags/user_links表，不能从原笔记推导。笔记使用持久note_id登记路径别名，标签关联指向该ID；重复导入复用ID、不覆盖用户标签或关联。只有FTS与解析得到的源标签缓存属于可重建派生数据，另存index.sqlite。导入及标签整理的权威写入采用本地事务；重建期间取得本地写锁并暂停整理/导入，以稳定原笔记快照和完好的权威标签表构建临时索引，核对成功后替换旧索引，再释放锁。源文件读取前后摘要变化则本次重建失败，不报告一致；不修改原笔记。';
...
export function makeTagPair(source){
 ...
 for(const m of positive.materials.filter(m=>m.nodeId)){
  if(m.nodeId==='integrator'){
   ...
   assert.ok(m.content.includes('可从原笔记完整重建'));m.content=m.content.replace('可从原笔记完整重建','可从原笔记与工具权威标签联合重建');m.content+='\n\n## 权威数据与恢复接缝\n'+authority+'\n'+recovery+'\n';
