...
import {makePairs,digest} from './reviewer-component-cases.ts';
...
if(mode==='prepare') {
 ...
 const {default:installedConfig,QWEN_MANAGED_ARGS}=await import(pathToFileURL(path.join(options['--installed'],'packages/task-generic-files/qwen-review-service-config.ts')).href);
 let config=installedConfig;const requestedModel=options['--model']??null;
 if(requestedModel!==null){
  assert.equal(requestedModel,'qwen3.8-max','本轮仅授权已配置模型');
  const {createAcpProvider}=await import(pathToFileURL(path.join(options['--installed'],'packages/agent-provider-acp/index.ts')).href);
  const {createGenericFilesReviewWireConfig}=await import(pathToFileURL(path.join(options['--installed'],'packages/task-generic-files/review-wire.ts')).href);
 ...
 const {receipt}=await import(pathToFileURL(path.join(options['--installed'],'packages/task-application/leader-ports.ts')).href);
